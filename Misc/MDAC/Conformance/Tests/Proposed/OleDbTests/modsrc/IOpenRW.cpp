...
public:
	...
	int Variation_39();
	// @cmember Parameters - pIndexID - Index Rowset
	int Variation_40();
...
BEG_TEST_CASE(TCOpenRowset, COpenRowset, L"IOpenRowset::OpenRowset test")
	...
	TEST_VARIATION(40, 		L"Parameters - pIndexID - Index Rowset")
	...
	for(ULONG iDSO=0; iDSO<cDSOs; iDSO++)
	{
		...
		if(SupportedProperty(DBPROP_MULTIPLECONNECTIONS, DBPROPSET_DATASOURCE, *ppDSO))
		{
			...
			if(iDSO == 0)
			{
				...
				{
					//Set it to FALSE
					::SetProperty(DBPROP_MULTIPLECONNECTIONS, DBPROPSET_DATASOURCE, &cPropSets, &rgPropSets, DBTYPE_BOOL, VARIANT_FALSE);
					TESTC_(hr = ((IDBProperties*)*ppDSO)->SetProperties(cPropSets, rgPropSets),S_OK);
...
//*-----------------------------------------------------------------------
// @mfunc Parameters - pIndexID - Index Rowset
//
// @rdesc TEST_PASS or TEST_FAIL
//
int TCOpenRowset::Variation_40()
{
    ...
    if(GetProperty(DBPROP_OPENROWSETSUPPORT, DBPROPSET_DATASOURCEINFO, g_pIDBCreateSession, &ulValue))
	{
		...
		TESTC(!(ulValue & ~(DBPROPVAL_ORS_TABLE | DBPROPVAL_ORS_INDEX | DBPROPVAL_ORS_INTEGRATEDINDEX | DBPROPVAL_ORS_STOREDPROC)));
	}
	else
	{
...
int TCOpenRowset::Variation_74()
{
	...
	TESTC_PROVIDER(Rowset.CreateRowset(SELECT_DBSCHEMA_TABLE, IID_IRowset, m_pTable, DBACCESSOR_ROWDATA, DBPART_ALL, ALL_COLS_BOUND)==S_OK);
	...
	while(hr = Rowset.GetNextRows(&hRow)==S_OK)
	{
			...
			wcscmp((WCHAR*)&VALUE_BINDING(*pTypeBinding, Rowset.m_pData),L"TABLE")==0)
		{
			...
			if(STATUS_BINDING(*pTableBinding, Rowset.m_pData)==DBSTATUS_S_OK)
			{
				...
				if(SUCCEEDED(hr = pIOpenRowset()->OpenRowset(NULL, &TableID, NULL, IID_IRowset, m_cPropSets, m_rgPropSets, (IUnknown**)&pIRowset)))
				{
					...
					HROW hRowA;
					CRowset RowsetA;
					TESTC_(hr = RowsetA.CreateRowset(pIRowset, USE_SUPPORTED_SELECT_ALLFROMTBL, IID_IRowset, NULL, DBACCESSOR_ROWDATA, DBPART_ALL, ALL_COLS_BOUND, FORWARD, NO_COLS_BY_REF, DBTYPE_EMPTY, BLOB_LONG),S_OK);

					//Now actually loop through this rowset of data...
					while(TRUE)
					{
						//Obtain the next row
						TEST2C_(hr = RowsetA.GetNextRows(&hRowA),S_OK,DB_S_ENDOFROWSET)
						if(hr==DB_S_ENDOFROWSET)
							break;
						
						//Obtain the data for this row.
						TESTC_(hr = RowsetA.GetRowData(hRowA, &RowsetA.m_pData),S_OK);

						//Release this row
						TESTC_(hr = RowsetA.ReleaseRows(hRowA),S_OK);
					}
				...
				{
					//The are valid reasons the table could not be opened.
					//Since the Schemas are just a snapshot the table may have existed on the server
					//we we obtained schemainfo, but by the time we got to this row it may have
					//been removed by another user.  We may not have permissions to open the table,
					//or it may be in another LCID than what were running.  The only table that should
					//always succeed is on of our tables, otherwise this is allowed to fail, but displays
					//the warning and tablename...
					TableID = m_pTable->GetTableID();
					if(TableID.uName.pwszName && wcscmp(TableID.uName.pwszName, pwszTableName)==0)
					{
						TERROR("Unable to open table "<< pwszQualTableName);
						CHECK(hr, S_OK);
					}
					else
					{
						TWARNING("Unable to open table "<< pwszQualTableName);
						TEST3C_(hr, S_OK, DB_E_NOTABLE, DB_SEC_E_PERMISSIONDENIED);
					}
...
CLEANUP:
	if(pwszQualTableName && FAILED(hr))
		TERROR("Failure for Table "<< pwszQualTableName);
    Rowset.ReleaseRows(hRow);
	SAFE_RELEASE(pIRowset);
	SAFE_FREE(pwszQualTableName);
	FreeProperties();
