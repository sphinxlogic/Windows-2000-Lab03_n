...
class TCIRowsetNewRow : public CRowsetObject
{
	...
	protected:
		...
		ULONG	GetNotUpdatable();

		//@mfunc: Get first updatable column
		ULONG	GetFirstUpdatable();

		//@mfunc: Get long and updatable columns
		ULONG	GetLongAndUpdatable();
...
)
{
	...
	{		
		...
		hr=GetAccessorAndBindings(	m_pICommand,dwAccessorFlags,&m_hAccessor,&m_rgBinding,&m_cBinding,&m_cRowSize,
									...
									cColsToBind,(LONG *)rgColsToBind,NULL,eColsMemProvOwned,eParamIO,blobType);
...
//
//--------------------------------------------------------------------
ULONG	TCIRowsetNewRow::GetNotUpdatable()
{
...
//
//--------------------------------------------------------------------
ULONG	TCIRowsetNewRow::GetFirstUpdatable()
{
	ULONG cbCols = 0;
	ULONG *rgCols = NULL;
	ULONG ulUpdatableCol = 0;
...
//
//--------------------------------------------------------------------
ULONG	TCIRowsetNewRow::GetLongAndUpdatable()
{
...
int Rowset::Variation_6()
{
	...
	TESTC(pDataCpy != NULL);
	memcpy(pDataCpy, pData, m_cRowSize);
	
	rgBindings = (DBBINDING*) PROVIDER_ALLOC(m_cBinding * sizeof(DBBINDING));
	TESTC(rgBindings != NULL);
	memcpy(rgBindings, m_rgBinding, m_cBinding * sizeof(DBBINDING));
...
int Forward_Query_Buffered::Variation_1()
{
	...
	//make the nullable and updatable columns NULL data
	dwAddr=(DWORD)pInsertedRowData;
...
{		   
	...
	//create an accessor to bind updatable columns
	TESTC_PROVIDER(GetRowsetAndAccessor(SELECT_ORDERBYNUMERIC,3,rgDBPROPID,
							2,g_rgDBPropID,ON_ROWSET_ACCESSOR,FALSE,DBACCESSOR_ROWDATA,
							DBPART_VALUE|DBPART_STATUS|DBPART_LENGTH,
							UPDATEABLE_COLS_BOUND));
CLEANUP:
	return fTestPass;
...
int Invalid_Keyset_Cursor_Immediate::Variation_2()
{
	...
	memcpy(m_pData, pData, m_cRowSize);
...
int Invalid_Keyset_Cursor_Immediate::Variation_6()
{
	...
	//copy the old data
	if(!memcpy(m_pData, pData, m_cRowSize))
		goto CLEANUP;
	...
	if(m_bIndexExists)
	{
		...
		*(DBSTATUS *)((DWORD)pData+m_rgBinding[0].obStatus)=DBSTATUS_S_OK; 
		...
		if(!COMPARE(memcmp(m_pData, pData, m_cRowSize), 0))
			goto CLEANUP;
...
int Invalid_Keyset_Query_Immediate::Variation_3()
{
	...
	BOOL		fTestPass=TRUE;
	HRESULT		hr = E_FAIL;
	ULONG		cRowsInTableBeforeInsert = 0, cRowsInTableAfterInsert = 0;
...
{
	void			*pData=NULL, *pValidationData=NULL;
	BOOL			fTestPass=TRUE;
...
int Valid_Keyset_Query_Buffered::Variation_1()
{
	...
	//make the length as bogus value
	dwAddr=(DWORD)pData;
