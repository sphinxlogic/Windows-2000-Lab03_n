...
#define TEST_TABLE_ENTRY(a,b,c,d,e,f,g,h,i,j,k,l) \
  {a,\
   (b) ? (sizeof(b) / sizeof(*((DBEXPECTEDPROPERTY *)(b)))) : 0,(b),\
   &c,d,L#d,e,\
   (f) ? (sizeof(f) / sizeof(*((DBEXPECTEDPROPERTY *)(f)))) : 0,(f),\
   (h) ? (sizeof(h) / sizeof(*((DBEXPECTEDPROPERTY *)(h)))) : 0,(h),\
   g,\
   i,j,\
   &k,l,\
   L#a, __LINE__}
      ...
      HRESULT GetProperties
	   ...
	   // @cmember Calls IRowsetInfo::GetReferencedRowset; verifies whenever
		...
		HRESULT GetReferencedRowset
			(
			   ULONG                iOrdinal,        // @parm [IN] The bookmark or chapter column for which to get the related rowset
	 ...
	 IUnknown ** ppSpecification)
{
	...
	if (hr == S_FALSE) {
		CHECK((long)*ppSpecification, NULL);
	...
	do 
	{
		...
		if(fPrepare == STATE_PREPARED)
		{
			...
			if(VerifyInterface(m_pICommand, IID_ICommandPrepare,
							COMMAND_INTERFACE, (IUnknown **) &m_pICmdPrepare))
		...
		if(cProperties > 0)
		{
			...
			for(ULONG uIndex = 0; uIndex < cProperties; uIndex++)
			{
				...
				if(!SupportedProperty(rgProperties[uIndex].dwPropertyID, 
											DBPROPSET_ROWSET, m_pIDBInitialize))
					ulNotSupported++;
			}
...
int TCGeneral::Variation_1()
{
	...
	//Since SetProperties just uses the last value set, you checking mecanism if 
	//wrong.  IE:  if you do
	//
