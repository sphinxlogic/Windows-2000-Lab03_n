...
BOOL SupportedInterface(IID riid, EINTERFACE eInterface)
{
    ...
    {
		//This is a mandatory interface on ROW object. So, it should be
		//sufficient to verify that ROW objects are supported. ROW objects
		//are supported if IGetRow is supported on a Rowset.
		ASSERT(g_pTable);
		QTESTC_(hr = g_pTable->CreateRowset(USE_OPENROWSET, IID_IRowset, 0, NULL, (IUnknown**)&pIRowset), S_OK);
		QTESTC(VerifyInterface(pIRowset, IID_IGetRow, ROWSET_INTERFACE, (IUnknown**)&pIUnknown));
		//To avoid calling ValidInterface at the end of this function.
		hr = E_FAIL;
    }
    else if( riid==IID_IRowsetNotify || riid==IID_IDBAsynchNotify)
    {
...
{ 
	...
	while(cEvents)
	{
		...
		else if(dwRet >= WAIT_OBJECT_0 && dwRet < WAIT_OBJECT_0 + cEvents)
		{
...
BOOL ShouldInitialize(IUnknown* pDataSource)
{
	...
	if((GetModInfo()->GetDebugMode() & DEBUGMODE_DIALOGS) || 
		!GetProperty(DBPROP_INIT_PROMPT, DBPROPSET_DBINIT, pDataSource, &ulValue) || ulValue==0 || ulValue==DBPROMPT_NOPROMPT)
		return TRUE;
...
HRESULT GetStorageData(DBCOUNTITEM cBindings, DBBINDING* rgBindings, void* pData, void* pBuffer, DBLENGTH* pcBytes, REFIID riid, IUnknown** ppIUnknown)
{	
	//Verify input params
	TBEGIN
	ASSERT(cBindings && rgBindings && pData);
...
HRESULT GetStorageData(REFIID riid, IUnknown* pIUnknown, void* pBuffer, DBLENGTH* pcBytes)
{	
	//Verify input params
	TBEGIN
	HRESULT hr = E_FAIL;
	...
	if(pIUnknown && pBuffer && pcBytes)
	{
		...
		TEST2C_(hr = StorageRead(riid, pIUnknown, pBuffer, (ULONG)*pcBytes, (ULONG*)pcBytes), S_OK, S_FALSE);
		hr = S_OK;
	}	
...
BOOL VerifyPropStatus(ULONG cPropSets, DBPROPSET* rgPropSets, DBPROPID dwPropertyID, GUID guidPropertySet, DBPROPSTATUS dwInputStatus)
{
	ASSERT(cPropSets && rgPropSets);
	TBEGIN
	DBPROP* pProp = NULL;
...
HRESULT CRowset::GetColInfo(DBORDINAL* pcColumns, DBCOLUMNINFO** prgInfo, WCHAR** ppStringsBuffer)
{
	...
	//GetColumnInfo	
	TESTC_(hr = pIColumnsInfo->GetColumnInfo(pcColumns, prgInfo, &pLocalStringsBuffer),S_OK)
...
HRESULT CRowset::GetStorageData(HROW hRow, HACCESSOR hAccessor, void* pBuffer, DBLENGTH* pcBytes, REFIID riid, IUnknown** ppIUnknown)
{
	TBEGIN
...
HRESULT CRowsetUpdate::GetOrgStorageData(HROW hRow, HACCESSOR hAccessor, void* pBuffer, DBLENGTH* pcBytes, REFIID riid, IUnknown** ppIUnknown)
{
	TBEGIN
	ASSERT(hRow && hAccessor);
	HRESULT hr = S_OK;
...
BOOL CListener::IsCancelable(DBREASON eReason, DBEVENTPHASE ePhase, IUnknown* pIUnknown)
{
	...
	{
		case DBREASON_ROWSET_FETCHPOSITIONCHANGE:
			GetProperty(DBPROP_NOTIFYROWSETFETCHPOSITIONCHANGE, DBPROPSET_ROWSET, pIUnknown, &dwCancelable);
			break;

		case DBREASON_ROWSET_RELEASE:
			GetProperty(DBPROP_NOTIFYROWSETRELEASE, DBPROPSET_ROWSET, pIUnknown, &dwCancelable);
			break;

		case DBREASON_COLUMN_SET:
			GetProperty(DBPROP_NOTIFYCOLUMNSET, DBPROPSET_ROWSET, pIUnknown, &dwCancelable);
			break;

		case DBREASON_ROW_DELETE:
			GetProperty(DBPROP_NOTIFYROWDELETE, DBPROPSET_ROWSET, pIUnknown, &dwCancelable);
			break;

		case DBREASON_ROW_FIRSTCHANGE:
			GetProperty(DBPROP_NOTIFYROWFIRSTCHANGE, DBPROPSET_ROWSET, pIUnknown, &dwCancelable);
			break;

		case DBREASON_ROW_INSERT:
			GetProperty(DBPROP_NOTIFYROWINSERT, DBPROPSET_ROWSET, pIUnknown, &dwCancelable);
			break;

		case DBREASON_ROW_RESYNCH:
			GetProperty(DBPROP_NOTIFYROWRESYNCH, DBPROPSET_ROWSET, pIUnknown, &dwCancelable);
			break;

		case DBREASON_ROW_UNDOCHANGE:
			GetProperty(DBPROP_NOTIFYROWUNDOCHANGE, DBPROPSET_ROWSET, pIUnknown, &dwCancelable);
			break;

		case DBREASON_ROW_UNDOINSERT:
			GetProperty(DBPROP_NOTIFYROWUNDOINSERT, DBPROPSET_ROWSET, pIUnknown, &dwCancelable);
			break;

		case DBREASON_ROW_UNDODELETE:
			GetProperty(DBPROP_NOTIFYROWUNDODELETE, DBPROPSET_ROWSET, pIUnknown, &dwCancelable);
			break;

		case DBREASON_ROW_UPDATE:
			GetProperty(DBPROP_NOTIFYROWUPDATE, DBPROPSET_ROWSET, pIUnknown, &dwCancelable);
			break;

		case DBREASON_ROWSET_CHANGED:
			GetProperty(DBPROP_NOTIFYROWSETCHANGED, DBPROPSET_ROWSET, pIUnknown, &dwCancelable);
			break;
	...
	switch(ePhase)
	{
		case DBEVENTPHASE_OKTODO:
			return (BOOL)(dwCancelable & DBPROPVAL_NP_OKTODO);
		
		case DBEVENTPHASE_ABOUTTODO:
			return (BOOL)(dwCancelable & DBPROPVAL_NP_ABOUTTODO);

		case DBEVENTPHASE_SYNCHAFTER:
			return (BOOL)(dwCancelable & DBPROPVAL_NP_SYNCHAFTER);
