...
// CE fails if DBPROPVAL_UP_CHANGE bit is not set, so this #define will work around.  We don't
// want this defined normally as it masks a bug.
// #define CURSOR_ENGINE
...
class CAccessor : public CRowsetObject
{
	...
	protected:	
		...
		HRESULT	GetRowsAndData(IUnknown * pIUnknown, HACCESSOR hAccessor,
			ULONG cbRowSize, BYTE ** ppData=NULL, IRowset ** ppIRowset=NULL, HROW ** prghRows=NULL);
		...
		BOOL	VerifyBindings(
			...
			ULONG cCreateBindings,			//@parm [IN] Count of bindin structs in array
		...
		HRESULT UseRowAccessorAndVerify(
				...
				ULONG cbRowSize,		//@parm [IN] Size of buffer needed for row 
				ULONG ulRowNum,			//@parm [IN] Row number needed for MakeData
				LONG * rgColumnsOrd,	//@parm [IN] Back end ordinals corresponding to rowset cols
				ULONG cColumns,			//@parm [IN] Number of ordinals in rgColumnsOrd array
				DBBINDING * rgBinding,	//@parm [IN] Array of bindings for this accessor	
				ULONG cBinding,			//@parm [IN] Number of bindings in rgBinding array
		...
		HRESULT UseParamAccessorAndVerify(
				...
				ULONG cbRowSize,		//@parm [IN] Size for one row of parameter data
				ULONG ulRowNum,			//@parm [IN] Row number needed for MakeData								
				DBBINDING * rgBinding,	//@parm [IN] Array of bindings for this accessor	
				ULONG cBinding,			//@parm [IN] Number of bindings in rgBinding array
		...
		void SetParamIO(ULONG cBindings, DBBINDING * rgBindings, DBPARAMIO	eParamIO);

		//@cmember Changes bindings to the specified memory owner for
		//all the types for which dwMemOwner applies
		void AdjustMemOwner(DBMEMOWNERENUM eMemOwner, ULONG cBindings, DBBINDING * rgBindings);
...
{	
	...
	if(CRowsetObject::Init())	
	{
		...
		if (SUCCEEDED(hr=CreateRowsetObject(SELECT_ORDERBYNUMERIC)))
		{
...
hr=CreateRowsetObject(SELECT_ORDERBYNUMERIC);
...
//
void CAccessor::SetParamIO(ULONG cBindings, DBBINDING * rgBindings, DBPARAMIO	eParamIO)
{	
	while (cBindings)
	{
...
HRESULT	CAccessor::GetRowsAndData(IUnknown * pIUnknown, HACCESSOR hAccessor,
	ULONG cbRowSize, BYTE ** ppData, IRowset ** ppIRowset, HROW ** pprghRows)
{
...
void CAccessor::AdjustMemOwner(DBMEMOWNERENUM eMemOwner, ULONG cBindings, DBBINDING * rgBindings)
{
	ULONG	i;
...
public:
	...
	HRESULT	SetDeferredProperties(BOOL fDeferred, BOOL fCacheDeferred, ULONG cBindings, DBBINDING * rgBindings);
...
private:
	...
	ULONG m_cbRowSize ;  // Some safe size.
	...
	ULONG m_cColumnsRowsetInfoObtained;
		...
		if (rgBinding && rgBinding[0].iOrdinal == 0)
		{
				...
				(rgBinding[0].dwPart & DBPART_VALUE))
			{
				...
				if (rgBinding[0].wType & DBTYPE_BYREF)
				{
					...
					memcpy(rgpBookmarks[0], (BYTE *)*(ULONG **)(pData+rgBinding[0].obValue), rgcbBookmarks[0]);
				}
				else
...
HRESULT CAccessor::UseParamAccessorAndVerify(
				...
				ULONG cBindings, 		//@parm [IN] Number of bindings in rgBinding array)
...
{	
	...
	for (iRow = 0; iRow < cRows; iRow++)
	{
		...
		TESTC_(m_hr = pICommand->Execute(NULL, IID_IRowset, &Param, 
			&cRowsAffected, (IUnknown **)&rgpRowset[iRow]), hrExecute);
...
HRESULT	TCDeferredColumns::SetDeferredProperties(
	...
	ULONG cBindings,		//Count of bindings in rgBindings. This is ignored if fCacheDeferred == FALSE.
...
)
{
	...
	if (CHECK(m_hr = CreateRowsetObject(SELECT_ORDERBYNUMERIC), S_OK))
	{	
		...
		DBACCESSORFLAGS dwAccessorFlags)
{
	...
	if (eDeferMode == MUST_DEFERR)
	{
		...
		if (SUCCEEDED(hr = CommandRowsetObject.CreateRowsetObject(SELECT_ORDERBYNUMERIC)))
		{
...
{	
	...
	if(g_fParamAccessor)
	{
		...
		if (g_fCmdSupported)
		{
			...
			{
				cBindings--;
				memcpy(prgBindings, &m_rgBindings[1], cBindings * sizeof(DBBINDING));
			}
			else
				memcpy(prgBindings, m_rgBindings, cBindings * sizeof(DBBINDING));
...
int TCCrtRtnValsAfterGetRows::Variation_11()
{
	...
	*/
	...
	if (g_fCmdSupported)
	{
		...
		if (fResults &= CHECK(hr=CreateRowsetObject(SELECT_ORDERBYNUMERIC), S_OK))
...
int TCCrtRtnValsAfterGetRows::Variation_24()
{
	...
	if (!m_rgBindings[0].iOrdinal)
	{
		rgNoBkMkBindings++;
		cNoBkMkBindings--;
		cbRowSize -= sizeof(DBSTATUS)+sizeof(ULONG)+m_rgBindings[0].cbMaxLen;
	}
...
int TCCrtRtnValsAfterGetRows::Variation_30()
{
	...
	//of the variations.  
	CHECK(CreateRowsetObject(SELECT_ORDERBYNUMERIC), S_OK);
...
{ 
	...
	TESTC_(SetProperty(DBPROP_IRowsetChange, DBPROPSET_ROWSET, &m_cPropSets, &m_rgPropSets,
		DBTYPE_BOOL, VARIANT_FALSE), S_OK);

	if (FAILED(hr = CreateRowsetObject(SELECT_ORDERBYNUMERIC)))
	{
...
CLEANUP:
	...
	CHECK(SetProperty(DBPROP_IRowsetChange, DBPROPSET_ROWSET, &m_cPropSets, &m_rgPropSets,
		DBTYPE_BOOL, VARIANT_FALSE, DBPROPOPTIONS_OPTIONAL), S_OK);
...
#ifdef CURSOR_ENGINE
	odtLog << L"Currently specifying DBPROPVAL_UP_CHANGE even though technically not required.\n";
	m_prgDBProps[1].vValue.lVal |= DBPROPVAL_UP_CHANGE;
#endif
...
int TCCreateValidRowAccessors::Variation_12()
{
	...
	//NOTE: This assumes that DBTYPE_STR will be a mandatory coersion.
	for (ulBindIdx=0; ulBindIdx<m_cBindings; ulBindIdx++)
	{
		...
		ROUND_UP(ulOffset, ROUND_UP_AMOUNT);
		...
		cbRowSize = rgMultiBind[0].cbMaxLen + rgMultiBind[1].cbMaxLen+4*sizeof(ULONG);
...
{ 
	...
	TESTC_(CreateRowsetObject(SELECT_ORDERBYNUMERIC), S_OK);
...
{	
	...
	// }}
	{
		...
		if (CHECK(CreateRowsetObject(SELECT_ORDERBYNUMERIC), S_OK))
		{			
			...
			m_rgSearchableCols = (LONG *)m_pIMalloc->Alloc(m_pTable->CountColumnsOnTable() * sizeof(LONG));
...
{	
	...
	if (!CHECK(m_pTable->ExecuteCommand(SELECT_ORDERBYNUMERIC, IID_IRowset,
		NULL, NULL, NULL, NULL, EXECUTE_NEVER, 
		0, NULL, NULL, NULL, &m_pICommand), S_OK))
		goto CLEANUP;
	...
	if (!CHECK(m_hr = m_pTable->ExecuteCommand(SELECT_ORDERBYNUMERIC, IID_IRowset,
...
int TCCreateValidParamAccessors::Variation_12()
{
	...
	if (MAX_BIND_LIMIT > m_cBindings)
	{
		...
		{
			ULONG ulOffset = sizeof(DBSTATUS)+sizeof(ULONG)+m_rgBindings[iBinding-1].cbMaxLen;
		...
		m_cbRowSize+=(m_rgBindings[m_cBindings].cbMaxLen+sizeof(DBSTATUS)+sizeof(ULONG))*cAdditionalBindings;
	...
	pParamCols = (LONG *)m_pIMalloc->Alloc(m_cBindings*sizeof(LONG));
	if (!pParamCols)
		goto CLEANUP;

	// Copy the searchable columns
	memcpy(pParamCols, m_rgSearchableCols, m_cSearchableCols*sizeof(LONG));
...
*/
	...
	odtLog << L"Created " << m_cBindings << L" parameters.\n";
...
{ 
	...
	//Just set the command so we can do IColumnsInfo to generate accessor
	TESTC_(m_pTable->ExecuteCommand(SELECT_ORDERBYNUMERIC, IID_IRowset,
		NULL, NULL, NULL, NULL, EXECUTE_NEVER, 
		0, NULL, NULL, NULL, &m_pICommand), S_OK);
	...
	UseParamAccessorAndVerify(m_hAccessor, DBACCESSOR_PARAMETERDATA, m_cbRowSize, g_uiRowNum,	
		m_rgBindings, m_cBindings, m_pICommand, DB_E_BADBINDINFO);
...
int TCAccessorSequencing::Variation_4()
{
	...
	if (!CHECK(m_pTable->ExecuteCommand(SELECT_ORDERBYNUMERIC,
...
{ 
	...
	memcpy(pBinding, m_rgBindings, m_cBindings * sizeof(DBBINDING));
	...
	if (g_fCmdSupported)
	{
		...
		TESTC(memcmp(pBinding, m_rgBindings, m_cBindings * sizeof(DBBINDING)) == 0);
...
{ 
	...
	TESTC_(m_pTable->ExecuteCommand(SELECT_ORDERBYNUMERIC,
...
BOOL TCBookMarkRowset::Init()
{
	...
	// }}
	{
		...
		//Set m_pIAccessor on a 'select *' rowset with bookmarks
		m_hr=CreateRowsetObject(SELECT_ORDERBYNUMERIC);	
...
//	CopyTestCaseInfo(m_pSetRowset);
	...
	{		
		...
		hr=m_pSetRowset->CreateRowsetObject(SELECT_ORDERBYNUMERIC);
...
int TCCommandAccessorTransactions::TestTxn(ETXN	eTxn, BOOL	fRetaining)
{
	...
	if (!StartTransaction(SELECT_ORDERBYNUMERIC, (IUnknown **)&pCmdIAccessor, 
...
int TCRowsetAccessorTransactions::TestTxn(ETXN	eTxn, BOOL	fRetaining)
{
	...
	if (!StartTransaction(SELECT_ORDERBYNUMERIC, (IUnknown **)&pIAccessor, 
						0, NULL, NULL, ISOLATIONLEVEL_READUNCOMMITTED))
		goto CLEANUP;
...
int TCAddRefAccessor::Variation_2()
{
	...
	//if we don't have command, skip the test;
	if(!g_fCmdSupported)
	{
		if( !m_pCmdIAccessor)
			return TEST_PASS;
	...
	if (!CHECK (m_pIAccessor->AddRefAccessor (m_hAccessor, &cRowsetRefCount), S_OK))
		goto CLEANUP;
...
CLEANUP:
	...
	while (cRowsetRefCount > 1)
		m_pIAccessor->ReleaseAccessor (m_hAccessor, &cRowsetRefCount);
...
int TCAddRefAccessor::Variation_3()
{
	...
	//  Do it  for the copied accessor.
	if (!CHECK (m_pIAccessor->AddRefAccessor (m_hAccessor, NULL), S_OK))
		goto CLEANUP;
...
BOOL TCAccessorOnAlteredTable::Init()
{
	...
	// }}
	{
		...
		m_cbRowSize = ulPrec1 + ulPrec2 + 2*sizeof(DBSTATUS) + 2*sizeof(ULONG);
...
int TCAccessorOnAlteredTable::Variation_1()
{
	...
	BYTE * pData=NULL;
	BOOL fSuccess = FALSE;
	HACCESSOR hRevAccessor = DB_NULL_HACCESSOR;
	ULONG ulTemp;
	...
	if (!CHECK(m_pATIRowset->RestartPosition(NULL), DB_S_COLUMNSCHANGED))
		goto CLEANUP;
	...
	m_rgATDbBindings[1].obStatus = m_rgATDbBindings[0].obValue + m_rgATDbBindings[0].cbMaxLen;
	m_rgATDbBindings[1].obLength = m_rgATDbBindings[1].obStatus + sizeof(DBSTATUS);
	m_rgATDbBindings[1].obValue = m_rgATDbBindings[1].obLength + sizeof(ULONG);

	// Now create a new accessor with the ordinals reversed
	TESTC_(m_pATIAccessor->CreateAccessor(DBACCESSOR_ROWDATA, m_cATDbBindings, m_rgATDbBindings, m_cbRowSize,
		&hRevAccessor, NULL), S_OK);
	...
	TESTC_(m_pATIRowset->GetData(*phRow, hRevAccessor, pData), S_OK);
...
BOOL TCAccessorOnAlteredTable::Terminate()
{
	...
	if (m_pIColumnsRowsetInfo && m_cColumnsRowsetInfoObtained > 0)
			m_pIColumnsRowsetInfo->ReleaseRows(m_cColumnsRowsetInfoObtained, (ULONG *)m_rgColumnsRowsetInfohRows,
				NULL, NULL, NULL);
