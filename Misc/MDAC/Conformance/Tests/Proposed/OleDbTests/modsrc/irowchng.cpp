...
class TCIRowsetChange : public CRowsetObject
{
	...
	protected:
		...
		BOOL	QueryBasedUpdate(IUnknown *pIUnknown=NULL);
		...
		BOOL	GetNullableAndUpdatable(ULONG	*pcbCol,
										ULONG	**prgColNum);

 		//@mfunc: Get the Not-Nullable and Updatable column
		BOOL	GetNotNullableAndUpdatable(	ULONG	*pcbCol,
											ULONG	**prgColNum);

		//@mfunc: Get the Numeric and Updatable column
		BOOL	GetNumericAndUpdatable(	ULONG	*pcbCol,
										ULONG	**prgColNum);

		//@mfunc: Get the Fixed Length and Updatable column
		BOOL	GetFixedLengthAndUpdatable(	ULONG	*pcbCol,
											ULONG	**prgColNum);

		//@mfunc: Get the non key non BLOB
		BOOL	GetNonKeyNonBLOB(	ULONG	*pcbCol,
									ULONG	**prgColNum);

		//@mfunc: Get the BLOB
		BOOL	GetBLOB(	ULONG	*pcbCol,
							ULONG	**prgColNum);

		//@mfunc: Get the BLOB
		BOOL	GetImage(	ULONG	*pcbCol,
							ULONG	**prgColNum);

		//@mfunc: Get the Variable Length and Updatable column
		BOOL	GetVariableLengthAndUpdatable(	ULONG	*pcbCol,
												ULONG	**prgColNum,
												BOOL	fLong=TRUE,
												BOOL	fBytes=TRUE);

		//@mfunc: Get the Variable Length and Updatable column
		BOOL	GetStr(	ULONG	*pcbCol,
						ULONG	**prgColNum);


		//@mfunc: Get not updatable column
		ULONG	GetNotUpdatable(	ULONG	*pcbCol,
									ULONG	**prgColNum);

		BOOL	GetUpdatableCols(	ULONG	*pcbCol,
									ULONG	**prgColNum);

 
		ULONG	GetAllButFirst(	ULONG	*pcbCol,
								ULONG	**prgColNum);
	...
	public:
		...
		{	
			...
			m_ulTableRows		=0;
			m_ulProvUpdFlags	=0;
			m_ulUpdFlags		=DBPROPVAL_UP_CHANGE;
...
BOOL TCIRowsetChange::Init()
{
	...
	if( !SettableProperty(DBPROP_UPDATABILITY,DBPROPSET_ROWSET,g_pIDBCreateSession) )
	{
		...
		GetProperty(DBPROP_UPDATABILITY,DBPROPSET_ROWSET,m_pIAccessor,&m_ulProvUpdFlags);
		ReleaseRowsetObject();
	}
...
)
{
	...
	if(cProperties || cPropertiesUnset)
	{
		...
		if (-1!=g_lMaxPendRows)
		{
			...
			rgDBPropSet[0].rgProperties[cProp].vValue.vt	= VT_I4;
			rgDBPropSet[0].rgProperties[cProp].colid		= DB_NULLID;
			rgDBPropSet[0].rgProperties[cProp].vValue.lVal	= g_lMaxPendRows;
			cProp++;
		}
	...
	switch(eAccessorLocation)
	{
		...
		case ON_ROWSET_ACCESSOR:  
				...
				eColsByRef,NULL,NULL,NULL,dbTypeModifier,cColsToBind,(LONG *)rgColsToBind,
...
)
{
	...
	hr = GetAccessorAndBindings(pIUnknown,dwAccessorFlags,&m_hAccessor,
							&m_rgBinding,&m_cBinding,&m_cRowSize,dwPart,eColsToBind,eBindingOrder,
							eColsByRef,NULL,&cCnt,NULL,dbTypeModifier,cColsToBind,(LONG *) rgColsToBind,
							NULL, eColsMemProvOwned,eParamIO,blobType);
...
BOOL	TCIRowsetChange::QueryBasedUpdate(IUnknown *pIUnknown)
{
	IRowsetInfo			*pIRowsetInfo=NULL;
	ULONG				cProperty=0;
	...
	if(pIUnknown)
	{
		TESTC_(pIUnknown->QueryInterface(IID_IRowsetInfo,(LPVOID *)&pIRowsetInfo),S_OK);
	}
	else
	{
		TESTC_(m_pIRowset->QueryInterface(IID_IRowsetInfo,(LPVOID *)&pIRowsetInfo),S_OK);
...
)
{
	...
	//initialization
	*pcbCol=0;
	*prgColNum=(ULONG *)PROVIDER_ALLOC(sizeof(ULONG) * m_cRowsetCols);
...
//
//---------------------------------------------------------------------------------
BOOL	TCIRowsetChange::GetUpdatableCols(	ULONG	*pcbCol,
											ULONG	**prgColNum)
{
	ULONG	cColsCount=0;
...
//
//--------------------------------------------------------------------
BOOL	TCIRowsetChange::GetFixedLengthAndUpdatable(	ULONG	*pcbCol,
														ULONG	**prgColNum)
{
...
//
//--------------------------------------------------------------------
BOOL	TCIRowsetChange::GetNonKeyNonBLOB(	ULONG	*pcbCol,
											ULONG	**prgColNum)
{
	ULONG	cColsCount = 0;
...
//
//--------------------------------------------------------------------
BOOL	TCIRowsetChange::GetBLOB(	ULONG	*pcbCol,
									ULONG	**prgColNum)
{
	ULONG	cColsCount = 0;
...
//
//--------------------------------------------------------------------
BOOL	TCIRowsetChange::GetImage(	ULONG	*pcbCol,
									ULONG	**prgColNum)
{
	ULONG	cColsCount = 0;
...
//
//--------------------------------------------------------------------
ULONG	TCIRowsetChange::GetAllButFirst(ULONG	*pcbCol,
										ULONG	**prgColNum)
{
	ULONG	cColsCount = 0;
...
//
//--------------------------------------------------------------------
ULONG	TCIRowsetChange::GetNotUpdatable(	ULONG	*pcbCol,
											ULONG	**prgColNum)
...
BEG_TEST_CASE(CacheDeferred, TCIRowsetChange, L"test DBPROP_CACHEDEFERRED")
	TEST_VARIATION(1, 		L"Kagera does not support DBPROP_CACHDEFERRED")
END_TEST_CASE()
#undef THE_CLASS
// }} TCW_TESTCASE_END
// }} TCW_TEST_CASE_MAP_END
...
protected:
	...
	BOOL		PrepareForSetData(
		ULONG			cSelectedColumn,	// [in]	the ordinal of the selected column (1 based)
...
public:
	...
	int Variation_9();
	// @cmember SetData(IGNORE)/SetData()/GPF
	int Variation_10();
	// }} TCW_TESTVARS_END
} ;
...
int Boundary::Variation_1()
{
	...
	m_ulUpdFlags		= DBPROPVAL_UP_CHANGE|DBPROPVAL_UP_INSERT;

	// Check to see if supported
	if( m_ulProvUpdFlags && (m_ulProvUpdFlags & m_ulUpdFlags) )
		m_ulUpdFlags = m_ulProvUpdFlags;
...
int Boundary::Variation_7()
{
	...
	m_rgBinding[0].iOrdinal	= ULONG_MAX;
	TESTC_(pIAccessor->CreateAccessor(DBACCESSOR_ROWDATA,
...
int Boundary::Variation_10()
{
	...
	m_cRowsObtained		= 0;
	m_rgPropertyIDs[0]	= DBPROP_IRowsetChange;
	m_ulUpdFlags		= DBPROPVAL_UP_CHANGE|DBPROPVAL_UP_INSERT|DBPROPVAL_UP_DELETE;;//DBPROPVAL_UP_CHANGE;
...
int Rowsets::Variation_2()
{
	IUnknown	*pIRowset			= NULL;
	BOOL		fTestPass			= TEST_SKIPPED;
	BOOL		fEscapeClauses		= FALSE;
	BOOL		fODBCExt			= FALSE;
	BOOL		fANSI				= FALSE;
	ULONG		ulSQLSupportMask	= 0;
...
// {{ TCW_VAR_PROTOTYPE(1)
//*-----------------------------------------------------------------------
// @mfunc Kagera does not support DBPROP_CACHDEFERRED
//
// @rdesc TEST_PASS or TEST_FAIL
//
...
int Forward_Query::Variation_5()
{
	...
	// Initialize 
	m_ulUpdFlags = DBPROPVAL_UP_INSERT;
...
int Forward_Cursor::Variation_13()
{
	...
	rgColsToBind=(ULONG *)PROVIDER_ALLOC(sizeof(ULONG) * m_cBinding);
...
int Forward_Cursor::Variation_14()
{
	...
	m_ulUpdFlags = DBPROPVAL_UP_DELETE | DBPROPVAL_UP_INSERT | DBPROPVAL_UP_CHANGE;
...
{ 
	...
	TESTC_(pTypesRowset->GetNextRows(NULL,0,1,&cRows, &pHRow),S_OK);
...
int Forward_Cursor::Variation_27()
{
	...
	rgDBPropSet[0].rgProperties[1].dwPropertyID		= DBPROP_UPDATABILITY;                    
   	...
   	rgDBPropSet[0].rgProperties[1].vValue.vt		= VT_I4;
   	rgDBPropSet[0].rgProperties[1].colid			= DB_NULLID;
	V_I4(&rgDBPropSet[0].rgProperties[1].vValue)	= m_ulUpdFlags;
...
int Forward_Cursor::Variation_28()
{
	...
	ULONG		cRows					= 0;
	HROW		*pHRow					= NULL;
	ULONG		cNotUpdateCol			= 0;
	ULONG		*rgNotUpdateColNumber	= NULL;
	ULONG		cUpdateCol				= 0;
	ULONG		*rgUpdateColNumber		= NULL;
	ULONG		cCol					= 0;
	ULONG		rgColNumber[2]			= {NULL,NULL};
	ULONG		cCount					= 0;
	ULONG		cBinding				= 0;
	void		*pData					= NULL;
	void		*pGetData				= NULL;
...
//*-----------------------------------------------------------------------
// @mfunc set no data in al BLOB column
//
// @rdesc TEST_PASS or TEST_FAIL
//
int Forward_Cursor::Variation_29()
{
	...
	DBPROPID	rgPropertyIDs[2];
	ULONG		cColNum			= 0;
	ULONG		*rgColNumberNum	= NULL;
	ULONG		cColBLOB			= 0;
	ULONG		*rgColNumberBLOB	= NULL;
	ULONG		rgColNumber[2]		= {0,0};
	...
	if ( IsNumericType(m_rgBinding[0].wType))
	{
		...
		*(BYTE *)((DWORD)pData+m_rgBinding[1].obValue)	= '/0';
		*(ULONG *)((DWORD)pData+m_rgBinding[1].obLength)= 0;
	}
	else
	{
		//fill the BLOB buffer with empty string
		*(BYTE *)((DWORD)pData+m_rgBinding[0].obValue)	= '/0';
...
int Keyset_Remove_Buffered::Variation_2()
{
	...
	//transmitt the changes, now everythign should work ok
...
BOOL Bookmark_Forward::Init()
{
	...
	// Initialize 
	m_ulUpdFlags = DBPROPVAL_UP_DELETE|DBPROPVAL_UP_CHANGE|DBPROPVAL_UP_INSERT;
...
int Bookmark_Forward::Variation_2()
{
	...
	if ((!m_pIRowsetChange) || (!m_pIRowsetUpdate) || (!m_pIRowsetLocate))
		goto CLEANUP;
...
int Invalid_Keyset_Cursor_Buffered::Variation_7()
{
	...
	*(ULONG *)((DWORD)pData+m_rgBinding[0].obLength)=m_rgBinding[0].cbMaxLen+1;
	...
	{	 
		if(!COMPARE(*(DBSTATUS *)((DWORD)pData+m_rgBinding[0].obStatus), 
...
int Valid_Keyset_Cursor_Immediate::Variation_4()
{
	...
	*((BYTE *)((DWORD)pData+m_rgBinding[cIndex].obValue+4))='\0';
	...
	if(!memcmp( (void *)((DWORD)pData+m_rgBinding[cIndex].obValue),
...
{
	BOOL		fTestPass=TEST_SKIPPED;
	ULONG		cCnt=g_ulNextRow++;
	ULONG		cCol;
	ULONG		*rgColNumber=NULL;
	void		*pData=NULL;
	ULONG		cRows;
	HROW		*pHRow=NULL;
	ULONG		cUpdatedRows, cMaxPendingRows;
	HROW		*pUpdatedHRow=NULL;
...
{
	cStatus=*(DBSTATUS *)((DWORD)pSetData+rgBinding[i].obStatus);
}
...
BOOL SetDefault::GetDefaultColumns()
{
	...
	ULONG		nCol				= 0;
	ULONG		cCol				= 0;
	ULONG		cNumColonTable		= m_pTable->CountColumnsOnTable();
	...
	memset(m_rgbDefault, 0, (sizeof(BOOL)*cNumColonTable));
...
{
	ULONG		iCol	= 0;
	ULONG		cCol	= g_pTable->CountColumnsOnTable();
	CCol		col;
...
)
{
	...
	CCol			col;
	ULONG			cOrdinalPos			= 0;
	ULONG			nCols				= m_pTable->CountColumnsOnTable();
	...
	memcpy(pDataCC, pData, m_cRowSize);
...
{ 
	...
	ULONG				cSeed				= g_ulNextRow++;
	ULONG				cColsOnTable		= 0;
	DBPROPID			rgPropertyIDs[1];
	...
	{ 
		...
		else
		{
			...
			memset(m_rgbDefault, 0, (sizeof(BOOL)*cColsOnTable));
...
{ 
	...
	ULONG			cColumnDesc		= g_pTable->CountColumnsOnTable();
	ULONG			cOrdinalPos		= 0;
	ULONG			cBinding		= 0;
	CCol			col;
	// buffer for row data
	...
	if (m_fCustomTables)
	{
		...
		if (!fPropNULLSet)
		{
			SetProperty(DBPROP_COL_NULLABLE, DBPROPSET_COLUMN, 
				...
				VT_BOOL, VARIANT_FALSE, DBPROPOPTIONS_REQUIRED, DB_NULLID);
		}
		m_pCustomTable->SetColumnDesc(rgColumnDesc, cColumnDesc);
...
// {{ TCW_VAR_PROTOTYPE(10)
//*-----------------------------------------------------------------------
// @mfunc SetData()/SetData(IGNORE)/GPF
//
// @rdesc TEST_PASS or TEST_FAIL 
//
...
{ 
	...
	memcpy(pDataCopy, pDataIn, m_cRowSize);
	SAFE_ALLOC(pDataReIn, BYTE, m_cRowSize);
	memset(pDataReIn, 0, m_cRowSize);
...
{  
	...
	// Initialize UPDATABILITY flag for class
	m_ulUpdFlags = DBPROPVAL_UP_DELETE | DBPROPVAL_UP_CHANGE;
...
{  
	...
	// Initialize UPDATABILITY flag for class
	m_ulUpdFlags = DBPROPVAL_UP_DELETE | DBPROPVAL_UP_INSERT;
