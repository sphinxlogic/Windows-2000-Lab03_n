	...
	*pcRowIndex = g_cTotalEntries;
	...
	switch (*((DBSTATUS *)(dwAddrData + m_rgBinding[cBndgParseName].obStatus)))
	{
		case DBSTATUS_S_ISNULL:
			break;
		case DBSTATUS_S_OK:
			wszParseName = (WCHAR*)(dwAddrData + m_rgBinding[cBndgParseName].obValue);
			break;
		default:
			return FALSE;
	}
	...
	// get source type
	if (!COMPARE(*((DBSTATUS *)(dwAddrData + m_rgBinding[cBndgType].obStatus)), DBSTATUS_S_OK))
		return FALSE;
	ulSrcType = *((unsigned short*)(dwAddrData + m_rgBinding[cBndgType].obValue));
	...
	for(cCount=0; cCount<m_cBinding; cCount++)
	{
		...
		if ((m_rgBinding[cCount].dwPart) & DBPART_LENGTH)
		{
			...
			{
				if (*((ULONG *)(dwAddrData + m_rgBinding[cCount].obLength)) < m_rgBinding[cCount].cbMaxLen)
					fErrorOccured = TRUE;
				continue;			
			}
		...
		{	
			...
			pConsumerData=(void *)(dwAddrData + m_rgBinding[cCount].obValue);
...
BOOL CSourcesRowset::ReadParseName(const CLSID clsid)
{
	...
	// enumerators.
	while(m_pIRowset->GetNextRows(NULL,0,1,&cRow,&pHRow) == S_OK)
	{
		// initialize fResult
		fResult = FALSE;
	...
	for(ULONG i=0; i<(g_cOLEDBProvNames+g_cOLEDBProvMDPNames); i++)
	{
								...
								g_rgwszParseName[i],&chEaten,&pIMoniker), S_OK))
		{
			...
			if (pIMoniker)
			{
				...
				if (wcsicmp(g_rgwszParseName[i], wszClsid))
				{
					fResult = TRUE;
					goto LOOP;
				}
...
ULONG CSourcesRowset::GetNextRowsWithoutRelAllRows()
{
	...
	if (SettableProperty(DBPROP_CANHOLDROWS, DBPROPSET_ROWSET, m_pISrcRow, ENUMERATOR_INTERFACE))
		SetProperty(DBPROP_CANHOLDROWS, DBPROPSET_ROWSET, &cPropSet, &rgPropSet, VT_BOOL, VARIANT_FALSE, DBPROPOPTIONS_REQUIRED, DB_NULLID);
	...
	while((m_hr=pIRowset->GetNextRows(NULL,0,FETCH_ROW_STD,&cRow,&pHRow)) == S_OK)
	{
		...
		TESTC_(m_hr=pIRowset->GetNextRows(NULL,0,FETCH_ROW_STD, &cRow,&pHRow), DB_E_ROWSNOTRELEASED);
...
public:
	...
	int Variation_3();
	// @cmember riid = IID_NULL, invalid property set -- E_INVALIDARG
	int Variation_4();
...
BEG_TEST_CASE(TCSourcesRowset_RootEnumerator, CSourcesRowset, L"ISourcesRowset::GetSourcesRowset test for root enumerator")
	...
	TEST_VARIATION(4, 		L"riid = IID_NULL, invalid property set -- E_INVALIDARG")
...
// {{ TCW_VAR_PROTOTYPE(4)
//*-----------------------------------------------------------------------
// @mfunc riid = IID_NULL, invalid property set -- E_INVALIDARG
//
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSourcesRowset_RootEnumerator::Variation_4()
{
	...
	m_hr=m_pISrcRow->GetSourcesRowset(NULL, IID_NULL,3, 
								NULL, (IUnknown **) &m_pIRowset);
	if (E_INVALIDARG != m_hr)
...
int TCSourcesRowset_RootEnumerator::Variation_12()
{
	...
	// Dirty the output params
	pIUnkInner = (IRowset*)0x12345678;

	// Create an inner rowset object on IID_IRowset
	hr = m_pISrcRow->GetSourcesRowset(	pIUnkOuter, IID_IRowset, 0, NULL, &pIUnkInner);

	if (E_NOINTERFACE==hr || CHECK(hr, DB_E_NOAGGREGATION))
		fResult = TEST_PASS;
	else
		odtLog<<L"Incorrect HRESULT for non support of aggregation.\n";
...
int TCSourcesRowset_RootEnumerator::Variation_14()
{
	...
	// We have only one pointer on enumerator object
	m_pISrcRow->AddRef();
	m_pISrcRow->Release();//
	...
	m_hr=m_pISrcRow->GetSourcesRowset(pIUnkOuter, IID_IUnknown,
													0, NULL, &pIUnkInner);
	
	// Check the return code
	if (FAILED(m_hr))
	{
		TESTC_(m_hr, DB_E_NOAGGREGATION);
		TESTC(NULL == pIUnkInner);
	}
	else
		TESTC_(m_hr, S_OK);
...
CLEANUP:
	...
	// Make sure the ref count on enumerator object is not changed
	m_pISrcRow->AddRef();
	m_pISrcRow->Release();//	
...
int TCSourcesRowset_ProviderEnumerator::Variation_12()
{
	...
	// Create an outer rowset object
	TESTC_(m_hr=m_pISrcRow->GetSourcesRowset(NULL, IID_IRowset, 0, NULL, &pIUnkOuter), S_OK);
...
int TCSourcesRowset_ProviderEnumerator::Variation_14()
{
	...
	// Create an inner rowset object on IID_IUnknown
	m_hr=m_pISrcRow->GetSourcesRowset(pIUnkOuter, IID_IUnknown, 0, NULL, &pIUnkInner);
...
int TCSourcesRowset_ProviderEnumerator::Variation_40()
{
	...
	// its sure that the prop is of type bool
	m_rgDBPropSets[0].rgProperties[count-1].vValue.vt = VT_BOOL;
