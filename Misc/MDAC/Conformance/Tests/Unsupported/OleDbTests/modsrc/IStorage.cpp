...
public:
	...
	virtual HRESULT StorageRead(IUnknown* pIUnknown, void* pBuffer, ULONG cBytes, ULONG* pcBytesRead = NULL, ULONG ulOffset = 0);
	virtual HRESULT StorageWrite(IUnknown* pIUnknown, void* pBuffer, ULONG cBytes, ULONG* pcBytesWrote = NULL, ULONG ulOffset = 0);

	virtual HRESULT StorageSeek(IUnknown* pIUnknown, LONG lOffset, DWORD dwOrigin = STREAM_SEEK_SET, ULONG* pulOffset = NULL);
	virtual HRESULT StorageSetSize(IUnknown* pIUnknown, ULONG ulSize);
...
//
////////////////////////////////////////////////////////////////////////////
HRESULT TCStorage::StorageRead(IUnknown* pIUnknown, void* pBuffer, ULONG cBytes, ULONG* pcBytesRead, ULONG ulOffset)
{
	//Delegate
...
//
////////////////////////////////////////////////////////////////////////////
HRESULT TCStorage::StorageWrite(IUnknown* pIUnknown, void* pBuffer, ULONG cBytes, ULONG* pcBytesWrote, ULONG ulOffset)
{
	//Delegate
...
//
////////////////////////////////////////////////////////////////////////////
HRESULT TCStorage::StorageSetSize(IUnknown* pIUnknown, ULONG cbSize)
{
	HRESULT hr = S_OK;
	ULARGE_INTEGER largeSize = { cbSize };
...
ULONG TCStorage::Thread_StorageRead(LPVOID pv)
{
	...
	ULONG		cBytes		= (ULONG)THREAD_ARG2;		//[in]
	ULONG*		pcBytesRead	= (ULONG*)THREAD_ARG3;		//[out]
	...
	//Call Read
	TEST2C_(::StorageRead(*pIID, pIUnknown, pBuffer, cBytes, pcBytesRead),S_OK,S_FALSE);
...
ULONG TCStorage::Thread_StorageWrite(LPVOID pv)
{
	...
	IID*		pIID		= (IID*)THREAD_ARG1;		//[in]
	ULONG		cBytes		= (ULONG)THREAD_ARG2;		//[in]
	ULONG*		pcBytesWrote= (ULONG*)THREAD_ARG3;		//[out]
	...
	TESTC_(::StorageWrite(*pIID, pIUnknown, pBuffer, cBytes, pcBytesWrote),STG_E_ACCESSDENIED);
...
{ 
	...
	TESTC(stagstg.grfMode >= 0 /*& STGM_READ*/);
...
int TCSeqStream_Read::Variation_23()
{
	...
	//Get the Data, ISequentialStream - should actually succeed
	TESTC_(RowsetA.GetStorageData(hRow, hAccessor, pBuffer, &cBytes, IID_IUnknown, (IUnknown**)&pIUnknown),S_OK)
	TESTC(cBytes != 0);
...
int TCSeqStream_Read::Variation_45()
{
	...
	//Get the Data, ISequentialStream - should actually succeed
	TESTC_(RowsetA.GetStorageData(hRow, hAccessor, m_pBuffer, &cBytes, m_riidStorage, NULL),S_OK)

	//Once ISeqStream is released we can read again
	TESTC_(RowsetA.GetStorageData(hRow, hAccessor, m_pBuffer2, &cBytes, m_riidStorage, NULL),S_OK)
...
int TCSeqStream_Read::Variation_62()
{
	...
	TESTC_(::GetStorageData(cBindings, rgBindings, pData, m_pBuffer, &cBytes, m_riidStorage, NULL),S_OK)
	...
	TESTC_(::GetStorageData(cBindings, rgBindings, pData2, m_pBuffer2, &cBytes, m_riidStorage, NULL),S_OK)
	TESTC(memcmp(m_pBuffer, m_pBuffer2, cBytes)==0)
...
int TCSeqStream_Read::Variation_67()
{
	...
	TESTC_(RowsetA.GetOrgStorageData(hRow, hAccessor, m_pBuffer, &cBytes, m_riidStorage, NULL),S_OK)

	//GetOriginalData again
	TESTC_(RowsetA.GetOrgStorageData(hRow, hAccessor, m_pBuffer2, &cBytes, m_riidStorage, NULL),S_OK)
...
int TCSeqStream_Read::Variation_68()
{
	...
	//GetOriginalData, ISequentialStream - should actually succeed
	TESTC_(RowsetA.GetOrgStorageData(rghRows[ROW_ONE], hAccessor, m_pBuffer, &cBytes, m_riidStorage, (IUnknown**)&pIUnknown),S_OK)
	...
	if(hr == S_OK)
	{
		...
		hr = RowsetA.GetOrgStorageData(rghRows[ROW_TWO], hAccessor, m_pBuffer2, &cBytes, m_riidStorage, (IUnknown**)&pIUnknown2);
...
int TCSeqStream_Read::Variation_69()
{
	...
	//GetOriginalData, ISequentialStream - should actually succeed
	TESTC_(RowsetA.GetOrgStorageData(rghRows[ROW_ONE], hAccessor, m_pBuffer, &cBytes, m_riidStorage, NULL),S_OK)

	//GetNextRow, while a storage object is open
	TESTC_(RowsetA.GetNextRows(&rghRows[ROW_TWO]),S_OK)

	//GetOrginalData again, already an open storage object 
	hr = RowsetA.GetOrgStorageData(rghRows[ROW_TWO], hAccessor, m_pBuffer2, &cBytes, m_riidStorage, NULL);
	TEST2C_(hr, S_OK, DB_E_NOTFOUND)
...
int TCSeqStream_Read::Variation_78()
{
	...
	//Get the Data, ISequentialStream - should actually succeed
	TESTC_(RowsetA.GetStorageData(hRowA, hAccessorA, m_pBuffer, &cBytes, m_riidStorage, (IUnknown**)&pIUnknownA),S_OK)
	TESTC_(RowsetB.GetStorageData(hRowB, hAccessorB, m_pBuffer2, &cBytes, m_riidStorage, (IUnknown**)&pIUnknownB),S_OK)
...
int TCSeqStream_Read::Variation_80()
{
	...
	ULONG cBytes = ULONG_MAX;
	ULONG rgcBytesRead[THREE_THREADS] = { 0, 0, 0 };
	IUnknown* pIUnknown = NULL;
...
int TCSeqStream_Write::Variation_43()
{
	...
	TESTC_(pCStorage->Seek(0),S_OK);
	TESTC_(pCStorage->Read(m_pBuffer, sizeof(void*)*DATA_SIZE, &cBytes),S_FALSE);
...
int TCSeqStream_Transactions::Variation_1()
{
	...
	if(m_fAbortPreserve) 
	{
		...
		TESTC_(StorageRead(m_riidStorage, pIUnknown, pBuffer, 10, &cBytes),S_OK)
		TESTC(cBytes == 10)
	}
	else
	{
		//Make sure the storage object is invalid
		TESTC_(StorageRead(m_riidStorage, pIUnknown, pBuffer, 10, &cBytes),E_UNEXPECTED)
		TESTC(cBytes == 0)
	}
...
int TCSeqStream_Transactions::Variation_6()
{
	...
	{
		//Make sure the storage object is valid
		TESTC_(StorageWrite(m_riidStorage, pIUnknown, pBuffer, 10, &cBytes),STG_E_ACCESSDENIED)
		TESTC(cBytes == 0)
	}
	else
	{
		//Make sure the storage object is invalid
		TESTC_(StorageWrite(m_riidStorage, pIUnknown, pBuffer, 10, &cBytes),E_UNEXPECTED)
		TESTC(cBytes == 0)
	}
