...
ULONG DisplaySize(CCol ColInfo);
...
ULONG GetDataSize(DBTYPE wType, ULONG cbData)
{
...
void Repack(ULONG cBind, DBBINDING * pBIND, ULONG * pcbRowSize)
{
	ULONG iBind =0;
	ULONG ulOffset = 0;
...
CLEANUP:
	...
	WCHAR * BuildTokenString(enum TOKEN_ENUM eToken, ULONG * pcColsAdded, ULONG ** prgColsAdded)
	{
		CCol TempCol;
		ULONG iCol, cCols, iStartCol;
		WCHAR * pwszStr=NULL;
		WCHAR * pwszName=NULL;
		...
		ULONG iParam=0;
		BOOL fError=TRUE;
		ULONG cColsAdded = 0,  * pColsAdded = NULL;
		const ulColNameMax = m_ulMaxParamName;

		cCols = m_pTable->CountColumnsOnTable();

		TEST_ALLOC(ULONG, pColsAdded, 0, cCols * sizeof(ULONG));
		...
		switch(eToken)
		{
			...
			case T_OFFSET_NO_LONG_PARM_EQ_LIST:
				...
				TEST_ALLOC(WCHAR, pwszStr, 0, m_cParams * (3 + 2*ulColNameMax) * sizeof(WCHAR));
			...
			case T_PARM_MARKER_LIST_RET:
				...
				TEST_ALLOC(WCHAR, pwszStr, 0, m_cParams * (wcslen(pwszParamMarker) + sizeof(L",")));
			...
			case T_NO_LONG_PARM_DEF_LIST_DFLT:
				{
					...
					ULONG cbMem	= m_cParams * (2 + 3*ulColNameMax) * sizeof(WCHAR);
					...
					TEST_ALLOC(WCHAR, pwszStr, 0, cbMem);
			...
			case T_NULL_COL_LIST:
				...
				TEST_ALLOC(WCHAR, pwszStr, 0, cCols * (3 + 2 * ulColNameMax) * sizeof(WCHAR));
			...
			case T_NO_LONG_PARM_MARKER_LIST:
				...
				TEST_ALLOC(WCHAR, pwszStr, 0, cCols * (ulColNameMax+wcslen(pwszParamMarker)+sizeof(L",")));
...
CLEANUP:
	...
	WCHAR * GetSyntax(enum TOKEN_ENUM eToken, ULONG * pcColsWanted = NULL, ULONG ** prgColsWanted = NULL)
	{
		ULONG iSyntax=0;
		ULONG iANSISyntax=0;
		ULONG iToken=0;
...
CLEANUP:
	...
	WCHAR * MakeParamName(CCol ColInfo, WCHAR wchFirst)
	{
		...
		if (m_ulCreateFlags & CREATE_LONG_NAMES || m_ulCreateFlags & CREATE_MAX_NAMES)
		{
			...
			if (GetModInfo()->GetLocaleInfo())
			{
				...
				GetModInfo()->GetLocaleInfo()->SetUnicodeSeed(ColInfo.GetColNum());
	...
	inline void SetColMap(ULONG cParams, ParamStruct * pColMap)
	{
		m_cParams = cParams;
		m_pColmap = pColMap;
	}
...
public:
	...
	BOOL FindRow (ULONG rlRowNum, CTable *pTable=NULL, ICommand * pICommand = NULL, IRowset ** ppIRowset=NULL, HROW ** pphRows=NULL);
	CMyError m_MyError;
};
...
//--------------------------------------------------------------------
BOOL
CIVerifyRow::FindRow(ULONG ulRowNum, CTable *pTable, ICommand * pICommand, IRowset ** ppIRowset, HROW ** pphRows)
{
	...
	TEST_CHECK(pTable->CreateSQLStmt(SELECT_ROW_WITH_LITERALS, NULL, &pwszSqlStmt,NULL, NULL, ulRowNum), S_OK);

	// Set command text
	TEST_CHECK(pICommandText->SetCommandText(DBGUID_DBSQL , pwszSqlStmt), S_OK);

	TEST_CHECK(pICommand->Execute(NULL, IID_IRowset, NULL, NULL, (IUnknown **)&pIRowset), S_OK);
...
public:
	...
	HRESULT CompareDBParamInfo( 
		ULONG			cCols,			// [in]	 Number of columns in rgColumnsInfo.  This is expected to match cParams
		LONG *			prgParamColMap,	// [in]  Parameter to column mapping array
		ULONG 			cParams, 		// [in]  Number of parameter information to compare
	...
	BOOL MakeDataForCommand(ULONG cRowNum);
	...
	HRESULT FillInputBindingsForArray(
		...
		ULONG *			rgRowNums,		// Array of rownumbers for which data has to be generated.
		ULONG 			cParamColMap,	
		LONG *			rgParamColMap	// Array of Column number mappings for the bindings.
	);
	...
	WCHAR * GetStandardTypeName(DBTYPE wType);
	...
	HRESULT SetParameterInfoIfNeeded(ULONG cParams, ULONG * pParamOrdinals, DBPARAMBINDINFO * pParamBindInfo, 
		ICommand * pICommand = NULL);

	HRESULT PrepareForExecute(WCHAR * pwszExecuteStmt, ULONG cParams, ULONG * pParamOrdinals,
		DBPARAMBINDINFO * pPARAMBIND, BOOL * pfCanDerive, WCHAR * pwszProcName=NULL,
		WCHAR * pwszCreateProcStmt=NULL, BOOL fIsFunction = FALSE);

	HRESULT ValidateGetParameterInfo(ULONG cExpParams, ULONG cParamSets, ULONG ulRowNum, ULONG * pExpOrdinals,
		DBPARAMBINDINFO * pExpParamBind, DBBINDING * pBINDING, ULONG cbRowSize, ParamStruct * pParamAll,
		BYTE * pData, enum ROWSET_ENUM eRowset, ULONG cColumns, ULONG * rgColumns,BOOL fCanDerive,
	...
	HRESULT ExecuteAndVerify(ULONG cParams, ULONG cParamSets, ParamStruct * pParamAll,
			ULONG ulRowNum,	DBBINDING * pBINDING, ULONG cbRowSize, BYTE * pData, enum ROWSET_ENUM eRowset,
			ULONG cColumns, ULONG * rgColumns, enum VERIFY_ENUM eVerifyMethod, BOOL fRelease,
			ICommand * pICommand = NULL, HRESULT hrExpected = S_OK, DBBINDING * pBindMatch=NULL,
			BYTE * pDataMatch = NULL, ULONG * pcRowsExpected = NULL);

	HRESULT VerifyRowset(REFIID riidRowset, IUnknown * pUnkRowset, ULONG ulStartingRow,
		ULONG cRowsetCols, ULONG * rgTableColOrds, BOOL fRelease, BOOL fRestart = FALSE,
		CTable * pTable = NULL, ULONG * pcRowsExpected=NULL);

	HRESULT VerifyOutParams(ULONG cParams, ULONG cParamSets, ULONG ulRowNum, DBBINDING * pBinding,
		ULONG cbRowSize, BYTE * pData, ParamStruct * pParamAll, enum VERIFY_ENUM eVerifyMethod, 
	...
	BOOL CreateProcBindings(
		...
		ULONG ulTableRow,					// [IN]  Row number in table to select, insert, or update
		ULONG * pcParams,					// [OUT] Count of params created
		ULONG * pcbRowSize,					// [OUT] Count of bytes for a single row of parameters
		DBBINDING ** ppDBBINDINFO,			// [OUT] Binding array for CreateAccessor
		ULONG ** ppParamOrdinals,			// [OUT] Array of cParams ordinals
		...
		ULONG * pcColumns = NULL,			// [OUT] Column count for rowset returned by proc or exec
		ULONG ** prgColumnsOrd = NULL,		// [OUT] Array of mappings of rowset columns to underlying table columns
		ULONG ** prgParamColOrd = NULL,		// [OUT] Array of mappings of parameters to underlying table columns
	...
	HRESULT ExecuteStoredProcedure(ICommandText *pICommandText, DBPARAMS *pDbParams, 
		ULONG ulRowNum, ULONG DataValues, ULONG BindingType);
	...
	BOOL StorePassByRefPointers(ULONG cRows, BYTE * pData);
...
protected:
	...
	ULONG *m_rgStoredProcParamColMap;
	...
	// @cmember BOOL to specify whether the backend DBMS is Microsoft SQL Server or not.
	BOOL	m_bSqlServer;

	// @cmember BOOL to specify whether the backend DBMS is Oracle or not.
	BOOL	m_bOracle;

	// @cmember BOOL to specfiy if multiple parameter sets were supported or not.
	BOOL	m_bMultipleParamSets;

	// @cmember BOOL to Specify if Output parameters are supported.
	ULONG	m_ulOutParamsSupported;

	// @cmember BOOL to Specify if default values for columns are supported (DBPROP_COL_DEFFAULT)
	BOOL	m_fDefaultVal;

	// @cmember BOOL to whether to give valid ppRowset to Execute
	BOOL	m_fRowsetTest;
	
	// @cmember BOOL to Specify case of identifiers
	ULONG	m_ulIdentifierCase;

	// @cmember BOOL to Specify case of quoted identifiers
	ULONG	m_ulQuotedIdentifierCase;
	...
	HRESULT VerifyParamInfo(ULONG cParamsExp, ULONG * rgParamOrdinals, DBPARAMBINDINFO * rgDbParamBindInfo,
		ULONG cParams, DBPARAMINFO * rgParamInfo, LPOLESTR pNamesBuffer, ULONG idxStart=0, BOOL fDerived = FALSE);
	
	BOOL ReverseArray(void * rgArray, ULONG cElements, ULONG ulElementSize);

	BOOL ScrambleArray(void * rgArray, ULONG cElements, ULONG ulElementSize);

	WCHAR ** CreateParameterNames(WCHAR ** ppwszParameterNames, enum NAME_ENUM eNameType, ULONG * pcParamNames=NULL, 
	...
	SHORT m_sDriverODBCVer;

	LONG * m_prgParamOrdinals;

	WCHAR ** m_prgwszParameterNames;

	static ULONG m_cInsertRowNum;
...
ULONG CICmdWParams::m_cInsertRowNum = TOTAL_NUMBER_OF_ROWS + 1;
		...
		
{
	...
	m_bOracle = FALSE;
	m_fRowsetTest = TRUE;
	m_ulOutParamsSupported = DBPROPVAL_OA_NOTSUPPORTED;
...
{
	ULONG			ulOutParamsSupported;
	ULONG			ulMultParamSets=0;
	WCHAR *			pwszDriverODBCVer;

	// Needed for DBPROP_OUTPUTPARAMETERAVAILABILITY
	if (GetProperty(DBPROP_OUTPUTPARAMETERAVAILABILITY, 
				   DBPROPSET_DATASOURCEINFO,m_pIDBInitialize, &ulOutParamsSupported))
		m_ulOutParamsSupported = ulOutParamsSupported;

	// Needed for DBPROP_MULTIPLEPARAMSETS
	m_bMultipleParamSets = GetProperty(DBPROP_MULTIPLEPARAMSETS, 
	...
	if (GetProperty(KAGPROP_DRIVERODBCVER, DBPROPSET_PROVIDERDATASOURCEINFO, 
		...
		m_sDriverODBCVer=_wtoi(pwszDriverODBCVer);
	else
		m_sDriverODBCVer=0;	// Unknown/not Kagera provider
	...
	{
		if (rgPropertySets[0].rgProperties[0].dwPropertyID == DBPROP_DBMSNAME)
		{
			if (!wcscmp (V_BSTR(&(rgPropertySets[0].rgProperties[0].vValue)), L"Microsoft SQL Server"))
				m_bSqlServer = TRUE;
			if (!wcscmp (V_BSTR(&(rgPropertySets[0].rgProperties[0].vValue)), L"Oracle"))
				m_bOracle = TRUE;
			...
			if (!wcscmp (V_BSTR(&(rgPropertySets[0].rgProperties[0].vValue)), L"ACCESS"))
			{
				if (!COMPARE(m_ulOutParamsSupported, DBPROPVAL_OA_NOTSUPPORTED))
				{
			...
			if (!wcscmp (V_BSTR(&(rgPropertySets[0].rgProperties[0].vValue)), L"ACCESS") ||
				GetModInfo()->GetClassContext() == CLSCTX_LOCAL_SERVER)
			{
				if (!COMPARE(m_bMultipleParamSets, FALSE))
				{
			...
			if (GetModInfo()->GetClassContext() == CLSCTX_LOCAL_SERVER)
			{
				...
				m_fRowsetTest = FALSE;		// Interface Remoting does not yet support a rowset pointer if output params
			}
		}
...
CICmdWParams::Init()
{
	...
	if (COLEDB::Init())
	{
		...
		m_rgParamColMap = (LONG *)m_pIMalloc->Alloc(m_pTable->CountColumnsOnTable() * sizeof(LONG));
		...
		m_rgParamOrdinals = (ULONG *)m_pIMalloc->Alloc(m_pTable->CountColumnsOnTable() * sizeof(ULONG));
		...
		memset(m_rgDbParamBindInfo, 0, m_pTable->CountColumnsOnTable() * sizeof(DBPARAMBINDINFO));
		...
		for (i = 1; i <= m_pTable->CountColumnsOnTable(); i++)
		{
			...
			if (TempCol.GetUpdateable() )
			{
				...
				AddParam(m_cDbParamBindInfo, i, DBPARAMIO_INPUT, NULL, FALSE, NULL, NULL, m_rgDbParamBindInfo, m_pParamStruct);
		...
		TEST_CHECK(GetAccessorAndBindings(m_pCmdIAccessor, 
						...
						(LONG *)m_rgParamColMap,  (DBORDINAL *)m_rgParamOrdinals ,NO_COLS_OWNED_BY_PROV,
...
//-------------------------------------------------------------------------------
BOOL
CICmdWParams::MakeDataForCommand(ULONG ulRowNum)
{
	...
	if (!CHECK(FillInputBindings((CTable *)m_pThisTestModule->m_pVoid, DBACCESSOR_PARAMETERDATA,
				m_cBindings, m_rgBindings, &m_pData, ulRowNum, m_cParamColMap, m_rgParamColMap), S_OK))
	{
		return FALSE;
	}
	...
	HACCESSOR * phLocalAccessor)				//	[Out]
{
	...
	TEST_ALLOC(ULONG, *prgParamColumnMap, 0, sizeof(ULONG));
	...
	)
{
	...
	if (!GetSqlBindInfo(pwszDataSourceType, wType, wszCreateStatement, wszInsertStatement, wszDropStatement,
			...
			(ULONG **)&rgParamColumnMap, &hLocalAccessor))
	{
	...
	TEST_CHECK((hr = FillInputBindings(m_pTable, DBACCESSOR_PARAMETERDATA, cParams,
			rgDbBindInfo, (BYTE **)&pData, ulRowNum, cParams, (LONG *)rgParamColumnMap)), S_OK);
	...
	*((ULONG *)((BYTE *)pData + rgDbBindInfo->obLength)) = sizeof (WCHAR) *10;
...
{
	ULONG iCol, cbRowSize = 0;
	BOOL bResult = FALSE;
	DBBINDING * pBinding = NULL;
	DBPARAMBINDINFO * pParamBindInfo = NULL;
	ParamStruct * pParamStruct = NULL;
	ULONG * prgParamOrdinals = NULL;
	HRESULT hr = E_FAIL;

	TEST_ALLOC(DBBINDING, pBinding, 0, sizeof(DBBINDING) * cCols);
	TEST_ALLOC(DBPARAMBINDINFO, pParamBindInfo, 0, sizeof(DBPARAMBINDINFO) * cCols);
	TEST_ALLOC(ParamStruct, pParamStruct, 0, sizeof(ParamStruct) * cCols);
	TEST_ALLOC(ULONG, prgParamOrdinals, 0, sizeof(ULONG) * cCols);
	...
	for (iCol = 0; iCol < cCols; iCol++)
	{
		...
		AddParam(iCol, prgParamColMap[iCol], DBPARAMIO_INPUT, NULL, FALSE, &cbRowSize, pBinding, pParamBindInfo,  pParamStruct);
	}

	hr = VerifyParamInfo(cCols, prgParamOrdinals, pParamBindInfo, cParams, rgParamInfo, pNamesBuffer);
...
ULONG DisplaySize(CCol ColInfo)
{
	ULONG ulDispSize = ColInfo.GetMaxSize();
...
WCHAR * CICmdWParams::GetStandardTypeName(DBTYPE wType)
{
...
)
{
	...
	// We assume the standard table and column info unless told otherwise
	if (!pTable)
		pTable = m_pTable;
	...
	if (pDBPARAMBINDINFO)
	{
		...
		// We don't expect return params to be nullable.
		if (TempCol.GetNullable() && !fReturnParam)
...
HRESULT CICmdWParams::PrepareForExecute(WCHAR * pwszExecuteStmt, ULONG cParams, ULONG * pParamOrdinals,
...
HRESULT CICmdWParams::ValidateGetParameterInfo(ULONG cExpParams, ULONG cParamSets, ULONG ulRowNum, ULONG * pExpOrdinals,
	DBPARAMBINDINFO * pExpParamBind, DBBINDING * pBINDING, ULONG cbRowSize, ParamStruct * pParamAll,
	BYTE * pData, enum ROWSET_ENUM eRowset, ULONG cColumns, ULONG * rgColumns, BOOL fCanDerive, 
	enum VERIFY_ENUM eVerifyMethod,	ICommand * pICommand)
{
	...
	if (SUCCEEDED(hrGet))
	{
		...
		TEST_ALLOC(BYTE, pDataCopy, 0, cbRowSize*cParamSets*sizeof(BYTE));
		memcpy(pDataCopy, pData, cbRowSize*cParamSets*sizeof(BYTE));
		...
		if (eVerifyMethod != VERIFY_NONE)
			memcpy(pData, pDataCopy, cbRowSize*cParamSets*sizeof(BYTE));
	...
	if (cExpParams > 1)
	{
		...
		FAIL_CHECK(pICmdWParams->SetParameterInfo(1, &pExpOrdinals[0], &pExpParamBind[1]), hrSet);
...
HRESULT CICmdWParams::VerifyOutParams(ULONG cParams, ULONG cParamSets, ULONG ulRowNum, DBBINDING * pBinding,
	ULONG cbRowSize, BYTE * pData, ParamStruct * pParamAll, enum VERIFY_ENUM eVerifyMethod, DBBINDING * pBindMatch,
	BYTE * pDataMatch)
{
	BOOL fResult = TRUE;
	ULONG iParam, iRow, cOutParams = 0, cCols = 0;
	ULONG * rgColumns = NULL;
	...
	if (eVerifyMethod == VERIFY_USE_TABLE)
	{
		...
		TEST_ALLOC(ULONG, rgColumns, 0, sizeof(ULONG)*cParams);
	...
	else if (eVerifyMethod == VERIFY_USE_PDATA)
	{
		...
		for (iParam = 0; iParam < cParams; iParam++)
		{
			...
			if (pBinding[iParam].dwPart & DBPART_STATUS)
				ulStatusOut = *(ULONG *)(pData+pBinding[iParam].obStatus);
			if (pBindMatch[iParam].dwPart & DBPART_STATUS)
				ulStatusExp = *(ULONG *)(pDataMatch+pBindMatch[iParam].obStatus);

			if (pBinding[iParam].dwPart & DBPART_LENGTH)
				ulLengthOut = *(ULONG *)(pData+pBinding[iParam].obLength);
			if (pBindMatch[iParam].dwPart & DBPART_LENGTH)
				ulLengthExp = *(ULONG *)(pDataMatch+pBindMatch[iParam].obLength);
					...
					(pBindMatch[iParam].dwPart & DBPART_VALUE))
				{
					...
					if (ulLength)
						FAIL_COMPARE(RelativeCompare(pValueOut, pValueExp, wType, (USHORT)ulLength, 
							pBinding[iParam].bPrecision, pBinding[iParam].bScale, ulLength), 0);
				}
			}
		}
	}
	else if (eVerifyMethod == VERIFY_NULL)
	{
		for (iParam = 0; iParam < cParams; iParam++)
		{
			...
			{
				DBSTATUS ulStatusOut = *(ULONG *)(pData+pBinding[iParam].obStatus);
		...
		ULONG * pcRowsExpected)
{
	...
	if (!m_fRowsetTest && eRowset == ROWSET_NONE)
	{
		ppIRowset = NULL;
		iid = IID_NULL;
	}
	...
	// If we failed but didn't expect it, then exit.  For success codes this will allow
	...
	if (m_ulOutParamsSupported == DBPROPVAL_OA_ATEXECUTE)
	...
	if (pRowset)
		FAIL_VAR(VerifyRowset(IID_IRowset, pRowset, ulRowNum, cColumns, rgColumns, fRelease, FALSE,
			NULL, pcRowsExpected), S_OK);

	if (m_ulOutParamsSupported == DBPROPVAL_OA_ATROWRELEASE)
		FAIL_VAR(VerifyOutParams(cParams, cParamSets, ulRowNum, pBINDING, cbRowSize, pData, pParamAll,
			eVerifyMethod, pBindMatch, pDataMatch), S_OK);
...
HRESULT CICmdWParams::VerifyRowset(REFIID riidRowset, IUnknown * pUnkRowset, ULONG ulStartingRow,
	ULONG cRowsetCols, ULONG * rgTableColOrds, BOOL fRelease, BOOL fRestart, CTable * pTable,
	ULONG * pcRows)
{
	BOOL fResult = TRUE;
	IRowset * pIRowset = NULL;
	...
	TEST_ALLOC(BYTE, pData, 0, cbRowSize * sizeof(BYTE));
...
BOOL CICmdWParams::CreateProcBindings(
	...
	ULONG cParamSets,					// [IN]	 Number of sets of parameters to be created
	DBTYPE wReturnType,					// [IN]  Return parameter type
	ULONG ulTableRow,					// [IN]  Row number in table or rowset to select, insert, or update
	ULONG * pcParams,					// [OUT] Count of params created
	ULONG * pcbRowSize,					// [OUT] Count of bytes for a single row of parameters
	DBBINDING ** ppDBBINDINFO,			// [OUT] Binding array for CreateAccessor
	ULONG ** ppParamOrdinals,			// [OUT] Array of cParams ordinals
	...
	BYTE ** ppData,						// [OUT] Pointer to data for the parameters
	ParamStruct ** ppParamAll,			// [OUT] Pointer to array of param structs to save name, column
	ULONG * pcColumns,					// [OUT] Column count for rowset returned by proc or exec
	ULONG ** prgColumnsOrd,				// [OUT] Array of mappings of rowset columns to underlying table columns
	ULONG ** prgParamColOrd,			// [OUT] Array of mappings of parameters to underlying table columns
	ULONG ulCreateFlags					// [IN]  Flags used to control creation of parameters
)
{
	ULONG iCol, cParams=0, cCols, cColList=0, cParamList=0, cLimit=0, cbRowSize=0, iParam, iParamList=0,
		iLimit=0, iParamRow, ulMaxDispSize=0, iDestParam, cFinalParams, iDefaultParam;
	CCol TempCol;
	BOOL fReturn = FALSE;
	...
	if ((eProcType == T_EXEC_PROC_SELECT_OUT ||
	...
	TEST_ALLOC(ParamStruct, pColList,	0, cCols*sizeof(ParamStruct));
	TEST_ALLOC(ParamStruct, pParamList,	0, cCols*sizeof(ParamStruct));
	TEST_ALLOC(ParamStruct, pLimit,		0, cCols*sizeof(ParamStruct));
	...
	TEST_ALLOC(ULONG, pulColOrds,0, cParams*sizeof(ULONG));
	TEST_ALLOC(ULONG, *ppParamOrdinals, 0, cParams*sizeof(ULONG));
	...
	for (iParam=0; iParam < cParams; iParam++)
	{
		...
		if (bHasReturnParam(eProcType) && !iParam)
		{
			...
			&cbRowSize, *ppDBBINDINFO, *ppDBPARAMBINDINFO, pParamAll, NULL, TRUE))
				goto CLEANUP;
		...
		else if (cParamList)
		{
			if (!AddParam(iParam,  pParamList[iParamList].ulColIndex, pParamList[iParamList].eParamIO,
				pParamList[iParamList].pwszParamName, fBindByName, &cbRowSize, *ppDBBINDINFO, *ppDBPARAMBINDINFO, pParamAll))
					goto CLEANUP;
		...
		else if (cLimit)
		{
			if (!AddParam(iParam, pLimit[iLimit].ulColIndex, pLimit[iLimit].eParamIO, 
				pLimit[iLimit].pwszParamName, fBindByName,  &cbRowSize, *ppDBBINDINFO, *ppDBPARAMBINDINFO, pParamAll))
					goto CLEANUP;
	...
	if (eProcType == T_EXEC_PROC_SELECT_INOUT)
	{
		...
		TEST_ALLOC(BYTE, pData, 0, 3 * cbRowSize);
		...
		for (iParam=0; iParam < cParams; iParam++)
		{
			LONG lCompResult;
			BYTE * pFirst, * pSecond;
			ULONG cbFirst, cbSecond;
			ULONG ulStFirst, ulStSecond;
			ULONG * pLengthFirst = (ULONG *)(pData+cbRowSize+(*ppDBBINDINFO)[iParam].obLength);
			ULONG * pLengthSecond = (ULONG *)(pData+(*ppDBBINDINFO)[iParam].obLength);
			...
			*/
			...
			ulStFirst	= *(ULONG *)(pData+cbRowSize+(*ppDBBINDINFO)[iParam].obStatus);
			ulStSecond	= *(ULONG *)(pData+(*ppDBBINDINFO)[iParam].obStatus);

			// Get the previous row info (only used for ulTableRow > 1)
			pPrev		= pData+2*cbRowSize+(*ppDBBINDINFO)[iParam].obValue;
			ulStPrev	= *(ULONG *)(pData+2*cbRowSize+(*ppDBBINDINFO)[iParam].obStatus);
			cbPrev		= *(ULONG *)(pData+2*cbRowSize+(*ppDBBINDINFO)[iParam].obLength);

			// Either of the fields may be NULL.
			if (ulStFirst == DBSTATUS_S_ISNULL || ulStSecond == DBSTATUS_S_ISNULL)
			{
			...
			else
			{
				...
				lCompResult = RelativeCompare(pFirst, pSecond, (*ppDBBINDINFO)[iParam].wType, (USHORT)cbSecond, 
					(*ppDBBINDINFO)[iParam].bPrecision, (*ppDBBINDINFO)[iParam].bScale, cbFirst);
			...
			if (!fSingletonSelect)
			{
				...
				else
				{
					if (ulStFirst == DBSTATUS_S_OK && lCompResult != RelativeCompare(pPrev, pSecond,
						...
						(*ppDBBINDINFO)[iParam].bScale, cbFirst))
						fSingletonSelect = TRUE;
				}
			}
		}
	...
	{
		TEST_ALLOC(BYTE, *ppData, 0, cbRowSize*cParamSets);
		...
		for (iParamRow=0; iParamRow < cParamSets; iParamRow++)
		{
			...
			TEST_CHECK (FillInputBindings(m_pTable, DBACCESSOR_PARAMETERDATA, cParams,
				*ppDBBINDINFO, &pData, ulTableRow+iParamRow, cParams, (LONG *)pulColOrds, PRIMARY), S_OK);
			...
			for (iParam=0; iParam < cParams; iParam++)
			{
				...
				if ((*ppDBBINDINFO)[iParam].eParamIO == DBPARAMIO_OUTPUT)
				{
					...
					ULONG ulParamLen = (*ppDBBINDINFO)[iParam].cbMaxLen;
					memset(pParamBuf, 0xCA, ulParamLen);
	...
	for(iParam=0; iParam < cParams; iParam++)
	{
		...
		else
		{
			if (iParam > iDestParam)
			{
				...
				memcpy(&(*ppParamOrdinals)[iDestParam], &(*ppParamOrdinals)[iParam], sizeof(ULONG));
				memcpy(&pulColOrds[iDestParam], &pulColOrds[iParam], sizeof(LONG));
...
//--------------------------------------------------------------------------
HRESULT 
CICmdWParams::ExecuteStoredProcedure(ICommandText *pICommandText, DBPARAMS *pDbParams, ULONG ulRowNum, ULONG DataValues,  ULONG BindingType)
{
	BYTE *pData = NULL;
	HRESULT hr = S_OK;
	ULONG i = 0;
	...
	if (!CHECK((hr = FillInputBindings(m_pTable, DBACCESSOR_PARAMETERDATA, m_cStoredProcBindings, m_rgStoredProcBindings,
				(BYTE **)&pData, (ulRowNum-1),  m_cStoredProcParamColMap, (LONG *)m_rgStoredProcParamColMap)), S_OK))
	{
		return hr;
	}
...
BOOL
CICmdWParams::StorePassByRefPointers(ULONG cRows, BYTE * pData)
{
	ULONG i=0;
...
CICmdWParams::ReleaseInputBindingsMemoryByRef(ULONG cRows, BYTE * pData, BOOL fFreeProviderMemory)
{
	...
	for (i = 0; i < m_cStoredProcBindings; i++)
	{
		...
		if (*(ULONG *)(pData + m_rgStoredProcBindings[i].obStatus) == DBSTATUS_S_OK)
		{
...
//--------------------------------------------------------------------------
BOOL CICmdWParams::ReverseArray(void * rgArray, ULONG cElements, ULONG ulElementSize)
{
...
//--------------------------------------------------------------------------
BOOL CICmdWParams::ScrambleArray(void * rgArray, ULONG cElements, ULONG ulElementSize)
{
	BYTE * pTemp=new BYTE[ulElementSize];
	BYTE * prgStart=(BYTE *)rgArray;
...
WCHAR ** CICmdWParams::CreateParameterNames(WCHAR ** ppwszParameterNames, enum NAME_ENUM eNameType,
	ULONG * pcParamNames, ULONG fColTypes)
{
	...
	if (!(ppwszParameterNames))
	{
		...
		memset(ppwszParameterNames, 0, (m_pTable->CountColumnsOnTable()+1) * sizeof (LPOLESTR));
	}
...
HRESULT CICmdWParams::VerifyParamInfo(ULONG cParamsExp, ULONG * rgParamOrdinals, DBPARAMBINDINFO * rgDbParamBindInfo,
	ULONG cParams, DBPARAMINFO * rgParamInfo, LPOLESTR pNamesBuffer, ULONG idxStart, BOOL fDerived)
{
	CCol TempCol;
	ULONG iOrdinal, idxMatch;
	...
	for (ULONG idx=0; idx < cParams; idx++)	
	{
		...
		if (m_bSqlServer && fDerived &&
			wcscmp(rgDbParamBindInfo[idxMatch].pwszName, L"RETURN_VALUE") &&
		...
		if (!MYCOMPARE((rgParamInfo[idx].dwFlags & (DBPARAMFLAGS_ISINPUT | DBPARAMFLAGS_ISOUTPUT)) > 0, TRUE, wszMsg))
			cFailures++;
		...
		for (iFlag = 0; iFlag < NUMELEM(g_rgParamFlags); iFlag++)
		{
			...
			ulAllFlags |= g_rgParamFlags[iFlag].ulFlag;

			// Set up message in case flag value did not match expected
			if (rgParamInfo[idx].dwFlags & g_rgParamFlags[iFlag].ulFlag)
			...
			if (!MYCOMPARE(rgParamInfo[idx].dwFlags & g_rgParamFlags[iFlag].ulFlag, 
				dwFlags & g_rgParamFlags[iFlag].ulFlag, wszMsg))
				cFailures++;
		}
		...
		if (pNamesBuffer || rgParamInfo[idx].pwszName)
		{
			...
			{	
				...
				// Need to convert out internal names to upper or lower case before compare if provider does so
				...
				if (!MYCOMPARE(wcscmp(rgParamInfo[idx].pwszName, rgDbParamBindInfo[idxMatch].pwszName),0, L"Parameter names did not match.\n"))  
					cFailures++;
			}
			else
			{
		...
		if (bType)
		{
			...
			// bPrecision
			if (!MYCOMPARE(rgParamInfo[idx].bPrecision, rgDbParamBindInfo[idxMatch].bPrecision, L"Precision did not match.\n"))  
				cFailures++;

			// bScale
			if (!MYCOMPARE(rgParamInfo[idx].bScale, rgDbParamBindInfo[idxMatch].bScale, L"Scale did not match.\n"))  
				cFailures++;
		}
	}
...
// {{ TCW_TEST_CASE_MAP(TCGetParameterInfo)
//--------------------------------------------------------------------
// @class Test case for GetParameterInfo.
//
class TCGetParameterInfo : public CICmdWParams { 
private:
...
public:
	...
	DECLARE_TEST_CASE_FUNCS(TCGetParameterInfo,CICmdWParams);
...
// {{ TCW_TESTCASE(TCGetParameterInfo)
#define THE_CLASS TCGetParameterInfo
BEG_TEST_CASE(TCGetParameterInfo, CICmdWParams, L"Test case for GetParameterInfo.")
...
// {{ TCW_TEST_CASE_MAP(TCCommandExecute)
//--------------------------------------------------------------------
// @class Test case for ICommand::Execute (For Parameter related test cases
//
class TCCommandExecute : public CICmdWParams { 
private:
	// @cmember Static array of variations
	DECLARE_TEST_CASE_DATA();
	...
	ULONG *m_rgSPParamColMap;
	ULONG *m_rgSPParamColOrdinals;
	...
	HRESULT ExecuteSP(DBPARAMS *pDbParams, ULONG ulRowNum);

	
public:
	// {{ TCW_DECLARE_FUNCS
	// @cmember Execution Routine
	DECLARE_TEST_CASE_FUNCS(TCCommandExecute,CICmdWParams);
	// }} TCW_DECLARE_FUNCS_END
...
// {{ TCW_TESTCASE(TCCommandExecute)
#define THE_CLASS TCCommandExecute
BEG_TEST_CASE(TCCommandExecute, CICmdWParams, L"Test case for ICommand::Execute (For Parameter related test cases")
...
// {{ TCW_TEST_CASE_MAP(TCSetParameterInfo)
//--------------------------------------------------------------------
// @class Test case for SetParameterInfo
//
class TCSetParameterInfo : public CICmdWParams { 
private:
	// @cmember Static array of variations
	DECLARE_TEST_CASE_DATA();
...
public:
	// {{ TCW_DECLARE_FUNCS
	// @cmember Execution Routine
	DECLARE_TEST_CASE_FUNCS(TCSetParameterInfo,CICmdWParams);
	// }} TCW_DECLARE_FUNCS_END
...
// {{ TCW_TESTCASE(TCSetParameterInfo)
#define THE_CLASS TCSetParameterInfo
BEG_TEST_CASE(TCSetParameterInfo, CICmdWParams, L"Test case for SetParameterInfo")
...
public:
	...
	LPVOID MakeData(CCol ColInfo, ULONG * pcbDataSize, ULONG ulSeed);
...
TEST_MODULE(7, ThisModule, gwszModuleDescrip)
	TEST_CASE(1, TCGetParameterInfo)
	TEST_CASE(2, TCMapParameterNames)
	TEST_CASE(3, TCCommandExecute)
	TEST_CASE(4, TCSetParameterInfo)
...
// {{ TCW_TC_PROTOTYPE(TCGetParameterInfo)
//*-----------------------------------------------------------------------
//| Test Case:		TCGetParameterInfo - Test case for GetParameterInfo.
//|	Created:			03/30/96
//*-----------------------------------------------------------------------
...
//
int TCGetParameterInfo::Variation_1()
{
	BOOL fResult = TRUE;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCGetParameterInfo::Variation_2()
{
	BOOL fResult = FALSE;
	...
	CHECK(CompareDBParamInfo(m_cBindings,  m_rgParamColMap, m_cParams, m_rgParamInfo, m_pNamesBuffer), S_OK);
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCGetParameterInfo::Variation_3()
{
	BOOL fResult = FALSE;
	LONG cRowsAffected = 0;
	IRowset * pRowset = NULL;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCGetParameterInfo::Variation_4()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCGetParameterInfo::Variation_5()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCGetParameterInfo::Variation_6()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCGetParameterInfo::Variation_7()
{
	// 
	FreeDescParams();
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCGetParameterInfo::Variation_8()
{
	FreeDescParams();
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCGetParameterInfo::Variation_9()
{
	FreeDescParams();
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCGetParameterInfo::Variation_10()
{
	FreeDescParams();
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCGetParameterInfo::Variation_11()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCGetParameterInfo::Variation_12()
{
	BOOL fResult = TRUE;
	...
	if (SupportedProperty(DBPROP_ACCESSORDER, DBPROPSET_ROWSET, m_pThisTestModule->m_pIUnknown,SESSION_INTERFACE))
		hrSetProp = SetRowsetProperty(m_pICommand, DBPROPSET_ROWSET, DBPROP_ACCESSORDER, DBPROPVAL_AO_RANDOM);
...
CLEANUP:
	...
	if (hrSetProp == S_OK)
		CHECK(SetRowsetProperty(m_pICommand, DBPROPSET_ROWSET, DBPROP_ACCESSORDER, DBPROPVAL_AO_RANDOM, DBPROPOPTIONS_OPTIONAL), S_OK);
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCGetParameterInfo::Variation_13()
{
	...
	if (m_ulOutParamsSupported == DBPROPVAL_OA_NOTSUPPORTED)
	{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCGetParameterInfo::Variation_14()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCGetParameterInfo::Variation_15()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCGetParameterInfo::Variation_16()
{
	BOOL fResult = TRUE;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCGetParameterInfo::Variation_17()
{
	...
	WCHAR * pwszExecProcStmt=NULL;
	WCHAR * pwszExecStmt=NULL;
	WCHAR * pwszProcName=L"\"''Stra# _* & % n  geProc\"";
	BYTE * pData=NULL;
	ParamStruct * pParamAll=NULL;
	ULONG cColumns = 0;
	ULONG * prgColumnsOrd = NULL;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCGetParameterInfo::Variation_18()
{
	BOOL fResult = TRUE;
...
// @rdesc TEST_PASS or TEST_FAIL 
//
int TCGetParameterInfo::Variation_19()
{ 
...
// @rdesc TRUE or FALSE
//
BOOL TCGetParameterInfo::Terminate()
{
	if (m_pICommandPrepare)
...
BOOL TCMapParameterNames::Init()
{
	...
	// }}
	{
		...
		if (m_sDriverODBCVer > 0 && m_sDriverODBCVer < 3)
			m_hrExpect=E_NOTIMPL;
		
		memset(&m_iNotTouched, NOT_TOUCHED_ORD, sizeof(LONG));

		m_fUseProc = (m_fProcedureSupport && m_ulOutParamsSupported != DBPROPVAL_OA_NOTSUPPORTED && m_Syntax.IsKnown());
		...
		if (m_fUseProc)
		{
			...
			m_prgParamOrdinals = (LONG *)pParamOrdinals;
		...
		if (!m_prgParamOrdinals && !(m_prgParamOrdinals=(LONG *)PROVIDER_ALLOC(m_cParamNames*sizeof(LONG))))
			return FALSE;

		// Copy the ordinals we had in base class init.
		memcpy(m_prgParamOrdinals, m_rgParamOrdinals, m_cParamNames * sizeof(ULONG));
...
int TCMapParameterNames::Variation_1()
{
	...
	LONG *rgParamOrdinals = NULL;
	BOOL fSuccess = FALSE;

	if ((rgParamOrdinals = (LONG *)m_pIMalloc->Alloc(cParamNames *sizeof (LONG))) == NULL)
	{
		odtLog << wszMemoryAllocationError;
		return TEST_FAIL;
	}
...
int TCMapParameterNames::Variation_5()
{
	...
	LONG cParamNames = 3;
	WCHAR *rgParamNames[] = {L"ONE", L"TWO", L"THREE" };
	LONG rgParamOrdinals[] = { LONG_MAX, LONG_MAX, LONG_MAX };
	HRESULT hr = E_FAIL;
...
int TCMapParameterNames::Variation_6()
{
	...
	LONG * pParamOrdinals = NULL;
	ULONG iParam;
	HRESULT hr = E_FAIL;

	TEST_ALLOC(LONG, pParamOrdinals, NOT_TOUCHED_ORD, m_cParamNames*sizeof(LONG));

	// Copy the name array
	TEST_ALLOC(WCHAR *, prgwszParameterNames, 0, m_cParamNames*sizeof(WCHAR *));
	memcpy(prgwszParameterNames, m_prgwszParameterNames, m_cParamNames*sizeof(WCHAR *));
...
int TCMapParameterNames::Variation_10()
{
	...
	prgwszParameterNames = new WCHAR *[m_cParamNames];
...
int TCMapParameterNames::Variation_11()
{
	...
	TEST_ALLOC(LONG, pParamOrdinals, NOT_TOUCHED_ORD, m_cParamNames*sizeof(LONG));
	TEST_ALLOC(DBPARAMBINDINFO, pParamBindInfo, NOT_TOUCHED_ORD, m_cParamNames*sizeof(DBPARAMBINDINFO));
	memcpy(pParamBindInfo, m_pParamBindInfo, m_cParamNames*sizeof(DBPARAMBINDINFO));
	...
	TEST_ALLOC(WCHAR *, rgwszParamNames, 0, m_cParamNames*sizeof(WCHAR *));
	memcpy(rgwszParamNames, m_prgwszParameterNames, m_cParamNames * sizeof(WCHAR *));
	...
	// can derive.
	TEST_CHECK(hr = m_pICmdWParams->SetParameterInfo(m_cParamNames, (ULONG *)m_prgParamOrdinals, pParamBindInfo),
...
CLEANUP:
	...
	if (!CHECK(hr = m_pICmdWParams->SetParameterInfo(m_cParamNames, (ULONG *)m_prgParamOrdinals, m_pParamBindInfo),
		DB_S_TYPEINFOOVERRIDDEN))
...
CLEANUP:
	...
	// Reset param info
	if (!COMPARE(SUCCEEDED(m_pICmdWParams->SetParameterInfo(m_cParamNames, (ULONG *)m_prgParamOrdinals, m_pParamBindInfo)),
		TRUE))
		testresult = TEST_FAIL;
...
// {{ TCW_TC_PROTOTYPE(TCCommandExecute)
//*-----------------------------------------------------------------------
//| Test Case:		TCCommandExecute - Test case for ICommand::Execute (For Parameter related test cases
//|	Created:			04/10/96
//*-----------------------------------------------------------------------
...
//--------------------------------------------------------------------
BOOL 
TCCommandExecute::DropSP()
{
	WCHAR * pwszDropString = NULL;
	WCHAR * pwszDropProc = NULL;
	BOOL fSuccess = FALSE;

	if (!(pwszDropProc = m_Syntax.GetSyntax(T_DROP)))
		goto CLEANUP;
...
// @rdesc TRUE or FALSE
//--------------------------------------------------------------------
BOOL TCCommandExecute::CreateSPText()
{
	...
	TEST_ALLOC(ULONG, m_rgSPParamColMap, 0, cParams * sizeof(ULONG));
...
//--------------------------------------------------------------------
HRESULT 
TCCommandExecute::ExecuteSP(DBPARAMS *pDbParams, ULONG ulRowNum)
{
	BYTE *pData = NULL;
	HRESULT hr = S_OK;
	...
	if (!CHECK((hr = FillInputBindings(m_pTable,
				...
				(BYTE **)&pData, (ulRowNum-1),  m_cSPParamColMap,(LONG *)m_rgSPParamColMap)), S_OK))
	{
		return hr;
	}
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_1()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_2()
{
	ULONG cBindings;
	DBBINDING *rgBindings = NULL;
	LONG *rgParamColMap = NULL;
	BOOL fResult = FALSE;
	void *pData = NULL;
	...
	rgParamColMap = (LONG *)m_pIMalloc->Alloc(cBindings * sizeof(LONG));
	...
	memcpy(rgBindings, m_rgBindings, m_cBindings*sizeof(DBBINDING));
	memcpy(rgParamColMap, m_rgParamColMap, m_cBindings*sizeof (LONG));
	...
	// Get to the end.
	ulOffset = rgBindings[cBindings -1].obValue + sizeof(ULONG);
	...
	if (rgBindings[cBindings -1].dwPart & DBPART_VALUE)
		if (rgBindings[cBindings - 1].cbMaxLen > sizeof(ULONG))
			ulOffset += rgBindings[cBindings - 1].cbMaxLen - sizeof(ULONG);
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_3()
{
	HACCESSOR hAccessor = DB_NULL_HACCESSOR;
	ULONG cBindings = m_cBindings - m_cBindings/2;
	DBPARAMS ExecDbParams;
	void *pData = NULL;
	LONG cRowsAffected;
	IRowset * pRowset = NULL;
	...
	if (!CHECK(FillInputBindings((CTable *)m_pThisTestModule->m_pVoid,
				...
				(BYTE **)&pData, ulRowNum,  m_cParamColMap,m_rgParamColMap), S_OK))
	{
		goto CLEANUP;
	}
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_4()
{
	HACCESSOR hAccessor = DB_NULL_HACCESSOR;
	ULONG cBindings = m_cBindings - m_cBindings/2;
	IRowset * pRowset = NULL;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_5()
{
	...
	ULONG				pcColumns		= 0;		// Count of columns
	...
	*((ULONG *)((BYTE *)pData + DbBinding.obLength)) = sizeof (LONGLONG);
...
// accessor.
int TCCommandExecute::Variation_6()
{
	HACCESSOR hAccessor = DB_NULL_HACCESSOR;
	DBPARAMS ExecDbParams;
	LONG cRowsAffected;
	...
	TEST_CHECK(FillInputBindings((CTable *)m_pThisTestModule->m_pVoid,	DBACCESSOR_PARAMETERDATA,
		...
		m_cParamColMap,m_rgParamColMap), S_OK);
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_7()
{
	...
	TEST_CHECK(FillInputBindings((CTable *)m_pThisTestModule->m_pVoid,
				...
				(BYTE **)&pData,(ulRowNum = NextInsertRowNum()),  m_cParamColMap,m_rgParamColMap), S_OK);
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_8()
{
	HACCESSOR hAccessor = DB_NULL_HACCESSOR;
	DBPARAMS ExecDbParams;
	void *pData = NULL;
	...
	TEST_CHECK(FillInputBindings(m_pExtraTable, DBACCESSOR_PARAMETERDATA,  m_cBindings,
		 m_rgBindings, (BYTE **)&pData, ulRowNum,  m_cParamColMap, m_rgParamColMap), S_OK);
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_9()
{
	HACCESSOR hAccessor = DB_NULL_HACCESSOR;
	DBPARAMS ExecDbParams;
	void *pData = NULL;
	...
	TEST_CHECK(FillInputBindings((CTable *)m_pThisTestModule->m_pVoid, DBACCESSOR_PARAMETERDATA,
		m_cBindings, m_rgBindings, (BYTE **)&pData, (ulRowNum = NextInsertRowNum()),  m_cParamColMap, m_rgParamColMap), S_OK);
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_10()
{
	BOOL fResult = FALSE;
	BYTE * pData = NULL;
	...
	TEST_CHECK(FillInputBindings(m_pTable,
				DBACCESSOR_PARAMETERDATA, m_cSPBindings, m_rgSPBindings,
				(BYTE **)&pData, ulRowNum,  m_cSPParamColMap,(LONG *)m_rgSPParamColMap), S_OK);
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_11()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_12()
{
	BOOL				fSuccess		= FALSE;	// Variation passed	or failed
	HRESULT				hr				= E_FAIL;	// HRESULT
	ICommand*			pICommand		= NULL;		// ICommandPrepare Object
	...
	// Allocate memory
	pData = (void *)m_pIMalloc->Alloc( (2 * sizeof (ULONG) + (DbBinding.cbMaxLen) + (sizeof (WCHAR))) );
	if (pData == NULL)
	{
		goto CLEANUP;
	}
	...
	*((ULONG *)((BYTE *)pData + DbBinding.obLength)) = sizeof (WCHAR);
	*((DBSTATUS *)((BYTE *)pData + DbBinding.obStatus)) = DBSTATUS_S_OK ;
	...
	{
		TEST_CHECK (hr, DB_E_ERRORSOCCURRED);
		TEST_COMPARE (*((DBSTATUS *)((BYTE *)pData + DbBinding.obStatus)), DBSTATUS_E_CANTCONVERTVALUE);
	}
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_13()
{
	HACCESSOR hAccessor = DB_NULL_HACCESSOR;
	DBPARAMS ExecDbParams;
	void *pData = NULL;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_14()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_15()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_16()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_17()
{
	BOOL fResult = TRUE;
	HRESULT hr = E_FAIL;
	ULONG iCol, cBinding = 0, cbRowSize = 0, ulRowNum;
	CCol TempCol;
	HACCESSOR hParamAccessor = DB_NULL_HACCESSOR;
	DBBINDING * pBinding = NULL;
	DBPARAMBINDINFO rgParamBindInfo[2];
	ULONG rgParamOrdinals[2] = {1, 2};
	DBPARAMS 	Param;
	...
	for (iCol = 1; iCol <= m_pTable->CountColumnsOnTable(); iCol++)
	{
		...
		IF_CHECK(hr, S_OK)
		{
			...
			IF_CHECK(hr, S_OK)
			{
				...
				SetParameterInfoIfNeeded(2, rgParamOrdinals, rgParamBindInfo, m_pICommand);
							...
							IF_CHECK(hr,S_OK)
							{
								...
								IF_COMPARE(cRowsAffected == 1 || cRowsAffected == DB_COUNTUNAVAILABLE, TRUE)
								{
									...
									IF_CHECK(hr, S_OK)
									{
										...
										ABORT_CHECK(SetParameterInfoIfNeeded(1, rgParamOrdinals, rgParamBindInfo, m_pICommand), S_OK);
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_18()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_19()
{
	// NULL input param is illegal in the sense it will never return any rows.  There is currently a spec bug that addresses this issue.
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_20()
{
	/*
	NULL input param is illegal in the sense it will never return any rows and therefore will not
	populate the output param.  There is currently a spec bug that addresses this issue.

	We should add a test case for populating output params when the search clause doesn't return any rows
	after the spec bug is resolved.  In the meantime this variation can be skipped as it doesn't provide any
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_21()
{
	...
	ULONG cParams, cParamSets = 1, cbRowSize, iParam;
	ULONG ulRowNum = 2;
	DBBINDING * pBINDING=NULL;
	ULONG * pParamOrdinals=NULL;
	DBPARAMBINDINFO * pPARAMBIND=NULL;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_22()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_23()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_24()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_25()
{
	// S_OK for non-NULL ouput data, DB_S_ERRORSOCCURRED for some NULL output data.
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_26()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_27()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_28()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_29()
{
	// Error for NULL data, S_OK for input param, error for variable length binary output data
	BOOL fResult = TRUE;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_30()
{
	BOOL fResult = TRUE;
	
	ULONG cParams, cParamSets = 1, cbRowSize, iParam;
	ULONG ulRowNum = 3, cbByRef = 0, ulOffSet=0;
	ULONG cRowsObtained=0;
	HROW * prghRow = NULL;
	DBBINDING * pBINDING=NULL;
	ULONG * pParamOrdinals=NULL;
	...
	for(iParam=0; iParam < cParams; iParam++)
	{
		...
		cbByRef+=sizeof(DBSTATUS)+sizeof(ULONG);
	...
	// values.  
	TEST_ALLOC(BYTE, pDataBYREF, 0, cbByRef);
	...
	for(iParam=0; iParam < cParams; iParam++)
	{
		...
		pBINDINGBYREF[iParam].obLength = ulOffSet+sizeof(DBSTATUS);
		pBINDINGBYREF[iParam].obValue = ulOffSet+sizeof(DBSTATUS)+sizeof(ULONG);
		// Copy the status
		...
		else
		{
			...
			memcpy(&VALUE_BINDING(pBINDINGBYREF[iParam], pDataBYREF), 
				&VALUE_BINDING(pBINDING[iParam], pData), pBINDINGBYREF[iParam].cbMaxLen); 
		}

		ulOffSet+=sizeof(DBSTATUS)+sizeof(ULONG)+pBINDINGBYREF[iParam].cbMaxLen;
	}
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_31()
{
	BOOL fResult = TRUE;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_32()
{
//	return TestStoredProcBindings (RETURN_VALUE_TYPE, VALID_IN_OUT_DATA, LENGTH_VALUE_STATUS, REGULAR_PARAMNAMES);

	// TODO: Since TCGetParameterInfo::Variation_16 only tests one return type, use this variation
	//		 to test other return data types if supported by the provider (Oracle does).	

	odtLog << L"This is already tested in TCGetParameterInfo::Variation_16.\n";
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_33()
{
	BOOL fResult = TRUE;
	...
	ULONG * pMultiParamOrdinals=NULL;
	ULONG * pMultiColMap = NULL;
	ULONG cMultiParams = 0;
	ULONG cbMultiRowSize = 0;
	ULONG iParam;
	HRESULT hrSet = S_OK;
	...
	TEST_ALLOC(ULONG, pMultiParamOrdinals, 0, cMultiParams*sizeof(ULONG));
	TEST_ALLOC(ParamStruct, pMultiParamAll, 0, cMultiParams*sizeof(ParamStruct));
	TEST_ALLOC(ULONG, pMultiColMap, 0, cMultiParams*sizeof(ULONG));
	...
	for (iParam = 0; iParam < cParams; iParam++)
	{
		...
		if (cMultiParams > 0)
		{
			pMultiBinding[cMultiParams].obLength = pMultiBinding[cMultiParams-1].obValue +
				pMultiBinding[cMultiParams-1].cbMaxLen;
			pMultiBinding[cMultiParams].obStatus = pMultiBinding[cMultiParams].obLength + sizeof(ULONG);
			pMultiBinding[cMultiParams].obValue = pMultiBinding[cMultiParams].obStatus + sizeof(DBSTATUS);
		}
		...
		cbMultiRowSize+=pMultiBinding[cMultiParams].cbMaxLen+sizeof(ULONG)+sizeof(DBSTATUS);
		cMultiParams++;
	...
	ABORT_CHECK (FillInputBindings(m_pTable, DBACCESSOR_PARAMETERDATA, cMultiParams,
		pMultiBinding, &pData, ulRowNum, cMultiParams, (LONG *)pMultiColMap, PRIMARY), S_OK);
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_34()
{
	BOOL fResult = FALSE;
	LONG cRowsAffected = 0;
	ULONG ulRowNum;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_35()
{

	BOOL fResult = FALSE;
	LONG cRowsAffected = 0;
	ULONG cNumRows, i;
	ULONG *rgRowNums=NULL;
	BYTE *pData = NULL;
	
	if (!m_bMultipleParamSets)
	{
	...
	TEST_ALLOC(ULONG, rgRowNums, 0, cNumRows * sizeof (ULONG));
	...
	TEST_ALLOC(BYTE, pData, 0, cNumRows * m_cbRowSize * sizeof (BYTE));

	// Create the Parameter array.
	TEST_CHECK (FillInputBindingsForArray (m_pTable, DBACCESSOR_PARAMETERDATA, m_cbRowSize, m_cBindings, m_rgBindings,
		(BYTE **)&pData, cNumRows, rgRowNums, m_cParamColMap, m_rgParamColMap), S_OK);
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_36()
{
	BOOL fResult = FALSE;
	LONG cRowsAffected = 0;
	ULONG cNumRows, i;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_37()
{
	...
	for (i=0; i < m_cBindings; i++)
	{
		...
		if	(lSaveIndex == -1 && 
			(m_rgBindings[i].wType == DBTYPE_I1 ||
			m_rgBindings[i].wType == DBTYPE_UI1 ||
			m_rgBindings[i].wType == DBTYPE_I2 ||
			m_rgBindings[i].wType == DBTYPE_UI2 ||
			m_rgBindings[i].wType == DBTYPE_I4 ||
			m_rgBindings[i].wType == DBTYPE_UI4)) 
		{
			// Change the binding to the next larger integer type
			lSaveIndex = i;
			dbSaveType = m_rgBindings[i].wType;
			cbMaxLen = m_rgBindings[i].cbMaxLen;

			// The next type is always twice as large
			m_rgBindings[i].cbMaxLen *= 2;

			// Adjust the row size accordingly
			cbRowSize-=cbMaxLen;
			cbRowSize+=m_rgBindings[i].cbMaxLen;

			switch (m_rgBindings[i].wType)
			{
				case DBTYPE_I1:
				case DBTYPE_UI1:
					m_rgBindings[i].wType = DBTYPE_I2;
				...
				case DBTYPE_I4:
				case DBTYPE_UI4:
					m_rgBindings[i].wType = DBTYPE_I8;
					pInt = (void *)&cInt64Max;
					break;
			}
		}
	...
	TEST_ALLOC(BYTE, pData, 0, cNumRows * cbRowSize * sizeof (BYTE));

	// Create the Parameter array.  
	TEST_CHECK (FillInputBindingsForArray (m_pTable, DBACCESSOR_PARAMETERDATA, cbRowSize, m_cBindings, m_rgBindings,
		(BYTE **)&pData, cNumRows, rgRowNums, m_cParamColMap, m_rgParamColMap), S_OK);
	...
	for (j = 1; j < cNumRows; j+=2)
	{
		...
		*((ULONG *)(((BYTE *)pRowData) + m_rgBindings[lSaveIndex].obLength)) = m_rgBindings[lSaveIndex].cbMaxLen;
		memcpy((void *)(((BYTE *)pRowData) + m_rgBindings[lSaveIndex].obValue), pInt, m_rgBindings[lSaveIndex].cbMaxLen);
	}
	...
	{
		pRowData = (BYTE *)pData + j*cbRowSize;
		*((ULONG *)(((BYTE *)pRowData) + m_rgBindings[lSaveIndex].obLength)) = sizeof (BYTE);
		memcpy((void *)(((BYTE *)pRowData) + m_rgBindings[lSaveIndex].obValue), &cTinyInt25, sizeof(BYTE));
	}

	// We Will have to create a new accessor.
	TEST_CHECK (m_pCmdIAccessor->CreateAccessor ( DBACCESSOR_PARAMETERDATA, m_cBindings, m_rgBindings, 
	...
	for (j=0; j < cNumRows; j++)
	{
		for (i=0; i < m_cBindings; i++)
		{
			...
			ULONG ulExpectedStatus;
			ULONG ulStatus = *((DBSTATUS *)(((BYTE *)pData + j*cbRowSize) + m_rgBindings[i].obStatus));
			BOOL fComp = TRUE;
...
CLEANUP:
	...
	{
		// RESTORE Bindings.
		m_rgBindings[lSaveIndex].wType = dbSaveType;
		m_rgBindings[lSaveIndex].cbMaxLen = cbMaxLen;
	}
...
int TCCommandExecute::Variation_38()
{
	...
	TEST_ALLOC(WCHAR *, ppwszProviderTypeNames, 0, cColumns * sizeof(WCHAR *));
	...
	TEST_CHECK(GetAccessorAndBindings(m_pCmdIAccessor, DBACCESSOR_PARAMETERDATA,
				...
				NULL, NULL,
				&pStringsBuffer, DBTYPE_EMPTY, cUpdateableCols, (LONG *)rgUpdateableCols ,
	...
	TEST_ALLOC(ULONG, rgParamOrdinals, 0, cSaveTableCols * sizeof(ULONG));
	TEST_ALLOC(ULONG, rgUpdateableCols, 0, cSaveTableCols * sizeof(ULONG));
	TEST_ALLOC(DBPARAMBINDINFO, pParamBindInfo, 0, cSaveTableCols * sizeof(DBPARAMBINDINFO));
	TEST_ALLOC(ParamStruct, pParamStruct, 0, cSaveTableCols * sizeof(ParamStruct));
	...
	TEST_CHECK (FillInputBindingsForArray (pSaveTable, DBACCESSOR_PARAMETERDATA, cbRowSize, cBindings, rgLocalBindings,
		(BYTE **)&pData, cNumRows, rgRowNums, cUpdateableCols, (LONG *)rgUpdateableCols), S_OK);
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_39()
{
	...
	memset(ppwszProviderTypeNames, 0, cColumns * sizeof(WCHAR *));
	...
	for (i = 1; i <= (LONG)pSaveTable->CountColumnsOnTable(); i++)
	{
		...
		{
			// Build Information for SetParameterInfo.
			AddParam(cDbParamBindInfo, i, DBPARAMIO_INPUT, NULL, FALSE, NULL, NULL, pParamBindInfo, pParamStruct);
			rgUpdateableCols[cUpdateableCols++] = TempCol.GetColNum();  // Column number which is the next Updateable col.
			rgParamOrdinals[cDbParamBindInfo] = cDbParamBindInfo + 1;  // Parameter ordinal number.
			cDbParamBindInfo++;
	...
	TEST_CHECK(GetAccessorAndBindings(m_pCmdIAccessor, DBACCESSOR_PARAMETERDATA,
				...
				UPDATEABLE_COLS_BOUND, FORWARD, NO_COLS_BY_REF, 
				NULL, NULL,
				NULL, DBTYPE_EMPTY, cUpdateableCols, (LONG *)rgUpdateableCols ,
				(DBORDINAL *)rgParamOrdinals ,NO_COLS_OWNED_BY_PROV, DBPARAMIO_INPUT, BLOB_LONG ), S_OK);
	...
	for (i=0; i < cNumRows; i++)
	{
		TEST_COMPARE(cRow.FindRow(rgRowNums[i], pSaveTable), TRUE);
	}
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_40()
{
	BOOL fResult = FALSE;
	LONG cRowsAffected = 0;
	IRowset * pRowset = NULL;
	...
	for (i=0; i < cNumRows; i++)
	{
		pLength = ((ULONG *)((BYTE *)pData + (i*m_cbRowSize) + m_rgBindings[VarColumn].obLength));
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_41()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_42()
{
	ULONG cBindings=1;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_43()
{
	BOOL fResult = TRUE;
	IRowset *	pIRowset = NULL;
	BYTE *		pData = NULL;
	...
	//Get memory to hold array of all col numbers.  NOTE:  This 
	//is the max possible, we won't necessarily use them all.
	rgSearchableCols = (LONG *)m_pIMalloc->Alloc(m_pTable->CountColumnsOnTable() * sizeof(LONG));
	if (!rgSearchableCols)
		return FALSE;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_44()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_45()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_46()
{
...
//	return TestStoredProcBindings (INPUT_OUTPUT, VALID_IN_OUT_DATA, LENGTH_VALUE_STATUS, REGULAR_PARAMNAMES, TRUE);
	odtLog << L"This is already tested in other variations now, for example, TCGetParameterInfo::Variation_13 and 14.\n";
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_47()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCCommandExecute::Variation_48()
{
	odtLog << L"Not yet implemented.\n";
	return TEST_PASS;
}
...
// @rdesc TEST_PASS or TEST_FAIL 
//
int TCCommandExecute::Variation_49()
{ 
	BOOL fResult = FALSE;
	LONG cRowsAffected = 0;
	ULONG ulRowNum;
	...
	memcpy(pSaveBinding, m_rgBindings, m_cBindings * sizeof(DBBINDING));
	...
	for (iCol = 1; iCol <= m_pTable->CountColumnsOnTable(); iCol++)
	{
		...
		cbRowSize+=sizeof(ULONG)+sizeof(DBSTATUS)+m_rgBindings[iBind].cbMaxLen;
		iBind++;
	}
	...
	//This sets it to one row of data.
	if (!CHECK(FillInputBindings((CTable *)m_pThisTestModule->m_pVoid, DBACCESSOR_PARAMETERDATA,
				m_cBindings, m_rgBindings, &m_pData, ulRowNum = NextInsertRowNum(), m_cParamColMap, m_rgParamColMap), S_OK))
	{
		return FALSE;
	}
	...
	// Verify If we have inserted the row properly.
	TEST_COMPARE(cRow.FindRow(ulRowNum, m_pTable, m_pICommand, &pIRowset, &phRows), TRUE);

	// FindRow merely finds the appropriate row.  We need to validate the inserted data for the big bindings.
	TEST_COMPARE(pIRowset==NULL, FALSE);
	TEST_COMPARE(phRows==NULL, FALSE);

	// Allocate a buffer to retrieve the row using the same size as the parameter buffer
	SAFE_ALLOC(pData, BYTE, cbRowSize);

	// Retrieve the row
	TEST_CHECK(pIRowset->GetData(*phRows, m_DbParamsAll.hAccessor, pData), S_OK);

	// Validate information retrieved
	TEST_COMPARE(CompareBuffer(pData, m_pData, m_cBindings, m_rgBindings, NULL, TRUE, FALSE, COMPARE_ONLY), TRUE);
...
CLEANUP:
	...
	if (pSaveBinding)
		memcpy(m_rgBindings, pSaveBinding, m_cBindings * sizeof(DBBINDING));
...
// @rdesc TEST_PASS or TEST_FAIL 
//
int TCCommandExecute::Variation_50()
{ 
	...
	memcpy(pBindings, m_rgBindings, m_cBindings*sizeof(DBBINDING));
...
CLEANUP:
	...
	ReleaseDataForCommand();
	memcpy(m_rgBindings, pBindings, m_cBindings * sizeof(DBBINDING));
	SAFE_FREE(pBindings);
...
// @rdesc TEST_PASS or TEST_FAIL 
//
int TCCommandExecute::Variation_51()
{ 
	BOOL fResult = TRUE;
	...
	hr = pIOpenRowset->OpenRowset(NULL, &dbidProc, NULL, IID_IRowset, 
...
// @rdesc TEST_PASS or TEST_FAIL 
//
int TCCommandExecute::Variation_52()
{ 
	/*
	...
	*/
	...
	for (iCol = 1; iCol <= cCols; iCol++)
	{
		...
		TESTC(!wcsicmp(DefaultCol.GetColName(), OrigCol.GetColName()));
	...
	for (iCol = 1; iCol <= cCols; iCol++)
	{
		...
		{
			// The column has a default value, screw up the value and length.
	...
	// Verify If we have inserted the row properly.
	TEST_COMPARE(cRow.FindRow(ulRowNum, pDefaultTable, m_pICommand, &pIRowset), TRUE);

	// FindRow merely finds the appropriate row.  We need to validate the inserted data for the default bindings.
	TEST_COMPARE(VerifyRowset(IID_IRowset, pIRowset, ulRowNum, m_cParamColMap, (ULONG *)m_rgParamColMap,
		FALSE, TRUE, pDefaultTable), S_OK);
...
// @rdesc TEST_PASS or TEST_FAIL 
//
int TCCommandExecute::Variation_53()
{ 
	BOOL fResult = TRUE;
	...
	ULONG cParams2, cParamSets2 = 1, cbRowSize2;
	DBBINDING * pBINDING2 = NULL;
	ULONG * pParamOrdinals2 = NULL;
...
// @rdesc TEST_PASS or TEST_FAIL 
//
int TCCommandExecute::Variation_54()
{ 
	BOOL fResult = TRUE;
...
// @rdesc TEST_PASS or TEST_FAIL 
//
int TCCommandExecute::Variation_55()
{ 
	BOOL fResult = FALSE;
	LONG cRowsAffected = 0;
	ULONG iParam;
...
// @rdesc TEST_PASS or TEST_FAIL 
//
int TCCommandExecute::Variation_56()
{ 
	...
	// Call OpenRowset on the stored proc
	hrOpen = pIOpenRowset->OpenRowset(NULL, &dbidProc, NULL, IID_IRowset, 
		0, NULL, (IUnknown **)&pIRowset);
	...
	if (hrOpen == S_OK)
		FAIL_VAR(VerifyRowset(IID_IRowset, pIRowset, ulRowNum, cColumns, prgColumnsOrd, TRUE), S_OK);
...
// @rdesc TEST_PASS or TEST_FAIL 
//
int TCCommandExecute::Variation_57()
{ 
	...
	ABORT_CHECK (FillInputBindings(m_pTable, DBACCESSOR_PARAMETERDATA, cParams,
		pBINDING, &pData, ulNullRow, cParams, (LONG *)prgParamColOrd, PRIMARY), S_OK);
	...
	// Now refill the parameter data buffer because we filled it with non-null data
	ABORT_CHECK (FillInputBindings(m_pTable, DBACCESSOR_PARAMETERDATA, cParams,
		pBINDING, &pData, ulNonNullRow, cParams, (LONG *)prgParamColOrd, PRIMARY), S_OK);
...
// @rdesc TEST_PASS or TEST_FAIL 
//
int TCCommandExecute::Variation_58()
{ 
	BOOL fResult = FALSE;
	LONG cRowsAffected = 0;
	ULONG ulRowNum;
	...
	ULONG cParams, cParamSets = 1, cbRowSize, cRowsObtained = 100;
	HROW * prghRows = NULL;
	DBBINDING * pBINDING=NULL;
	ULONG * pParamOrdinals=NULL;
	DBPARAMBINDINFO * pPARAMBIND=NULL;
	...
	for (iBind = 0; iBind < cParams; iBind++)
	{
		...
		ULONG cbMaxLen = pBind->cbMaxLen;
		ULONG ulDataSize = LENGTH_BINDING(*pBind, pData);
	...
	if (SupportedProperty(DBPROP_UPDATABILITY, DBPROPSET_ROWSET, m_pThisTestModule->m_pIUnknown,SESSION_INTERFACE))
		CHECK(SetRowsetProperty(m_pICommand, DBPROPSET_ROWSET, DBPROP_UPDATABILITY, DBPROPVAL_UP_DELETE), S_OK);
	...
	ABORT_CHECK(m_pICommand->Execute(NULL, IID_IRowset, &ExecDbParams, 
		&cRowsAffected, (IUnknown **)&pIRowset), S_OK);
	...
	for (iBind = 0; iBind < cBindings; iBind++)
	{
			...
			sStatus == DBSTATUS_S_OK)
		{
			...
			ULONG ulLength = LENGTH_BINDING(m_rgBindings[iBind], m_pData);
			ULONG ulNewLength;
...
// @rdesc TEST_PASS or TEST_FAIL 
//
int TCCommandExecute::Variation_59()
{ 
	BOOL fResult = TEST_FAIL;
	...
	// FindRow merely finds the appropriate row.  We need to validate the inserted data for the default bindings.
	TESTC_(VerifyRowset(IID_IRowset, pIRowset, ulRowNum, cColumns, prgColumnsOrd, FALSE, TRUE, m_pTable, &cRows), S_OK);
...
// @rdesc TEST_PASS or TEST_FAIL 
//
int TCCommandExecute::Variation_60()
{ 
	BOOL fResult = TEST_FAIL;
	...
	TESTC_(FillInputBindings(m_pTable, DBACCESSOR_PARAMETERDATA, cParams,
		pBINDING, &pData, ulRowNum, cParams, (LONG *)prgParamColOrd, PRIMARY), S_OK);
...
// @rdesc TRUE or FALSE
//
BOOL TCCommandExecute::Terminate()
{
...
// {{ TCW_TC_PROTOTYPE(TCSetParameterInfo)
//*-----------------------------------------------------------------------
//| Test Case:		TCSetParameterInfo - Test case for SetParameterInfo
//|	Created:			05/03/96
//*-----------------------------------------------------------------------
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_1()
{
	BOOL fResult = TRUE;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_2()
{
	BOOL fResult = FALSE;
	LONG cRowsAffected = 0;
	ULONG ulRowNum = 0;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_3()
{
	...
	WCHAR *pwszSqlSelectAllFromTbl = NULL;
	ULONG ulRowNum = 0;
	ULONG cTableCols = m_pTable->CountColumnsOnTable();
	ParamStruct * pParamStruct = NULL;
	IRowset * pRowset = NULL;
	...
	ABORT_ALLOC(ULONG, rgParamOrdinals, 0, cTableCols * sizeof(ULONG));
	ABORT_ALLOC(ULONG, rgUpdateableCols, 0, cTableCols * sizeof(ULONG));
	ABORT_ALLOC(DBPARAMBINDINFO, rgDbParamBindInfo, 0, cTableCols * sizeof(DBPARAMBINDINFO));
	ABORT_ALLOC(DBBINDING, rgLocalBindings, 0, cTableCols * sizeof(DBBINDING));
	ABORT_ALLOC(ParamStruct, pParamStruct, 0, cTableCols * sizeof(ParamStruct));
	...
	for (i = 1; i <= m_pTable->CountColumnsOnTable(); i++)
	{
		...
		if (TempCol.GetUpdateable())
		{
			...
			rgUpdateableCols[cUpdateableCols++] = TempCol.GetColNum();  // Column number which is the next Updateable col.
			rgParamOrdinals[cDbParamBindInfo] = cDbParamBindInfo + 1;  // Parameter ordinal number.
			AddParam(cDbParamBindInfo++, i, DBPARAMIO_INPUT, NULL, FALSE, &cbRowSize, rgLocalBindings, rgDbParamBindInfo, pParamStruct);
		}
	...
	ABORT_CHECK(FillInputBindings((CTable *)m_pTable,
				...
				(BYTE **)&pData, (ulRowNum = NextInsertRowNum()) , cUpdateableCols, (LONG *)rgUpdateableCols), S_OK);

	DbParamsAll.cParamSets = 1;
	DbParamsAll.hAccessor = hLocalAccessor;
	DbParamsAll.pData = pData;

	ABORT_CHECK(m_pICommand->Execute(NULL, IID_IRowset,
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_4()
{
	BOOL fResult = FALSE;
	HRESULT hr;
	...
	CHECK(CompareDBParamInfo(m_cDbParamBindInfo, m_rgParamColMap, m_cParams, m_rgParamInfo, m_pNamesBuffer), S_OK);
	...
	{
		TEST_CHECK (m_cParams, m_cDbParamBindInfo);
		TEST_CHECK (hr, S_OK);
	}
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_5()
{
	BOOL fResult = FALSE;
	ULONG cParams = 0;
	ULONG *rgParamOrdinals = NULL;
	...
	rgParamOrdinals = (ULONG *)m_pIMalloc->Alloc((m_cDbParamBindInfo - cParams)* sizeof(ULONG));
	...
	if (m_bSetParameterInfo)
		...
		CHECK(CompareDBParamInfo(cParams, m_rgParamColMap, m_cParams, m_rgParamInfo, m_pNamesBuffer), S_OK);
	else
	{
		...
		CHECK(CompareDBParamInfo(m_cParams, m_rgParamColMap, m_cParams, m_rgParamInfo, m_pNamesBuffer), S_OK);
	}
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_6()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_7()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_8()
{
	HRESULT hr;
	BOOL fResult = FALSE;
	LONG cRowsAffected = 0;
	...
	rgParamOrdinals = (ULONG *)m_pIMalloc->Alloc (m_cDbParamBindInfo*sizeof (ULONG));
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_9()
{
	ULONG cParams = 2;
	ULONG ulNumParams=0;
	HRESULT hr = E_FAIL, hrSet = E_FAIL;
	ULONG rgParamOrdinals[2] = { 1, ULONG_MAX };
	LPWSTR wszTypeInfo[] = { L"DBTYPE_I1", L"DBTYPE_I2" };
	WCHAR *pwszSqlSelectAllFromTbl=NULL;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_10()
{
	ICommand *pICommand = NULL;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_11()
{
	...
	TEST_ALLOC(DBPARAMBINDINFO, pParamBindInfo, 0, m_cBindings * sizeof(DBPARAMBINDINFO));
	memcpy(pParamBindInfo, m_rgDbParamBindInfo, m_cBindings *  sizeof(DBPARAMBINDINFO));
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_12()
{
	
	ULONG cDeleteParams = 2;
	ULONG rgDeleteOrdinals[2];
	BOOL fResult = FALSE;
	HRESULT hr;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_13()
{
	BOOL fResult = TRUE;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_14()
{
	BOOL fResult = TRUE;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_15()
{
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_16()
{
	ULONG cParams = 1;
	HRESULT hr;
	ULONG rgParamOrdinals[1] = { 1 };
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_17()
{
	BOOL fResult = FALSE;
	...
	ABORT_CHECK(m_pICommand->Execute(NULL, IID_IRowset, &ExecParams, &cRowsAffected, (IUnknown **)&pIRowset), S_OK);
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_18()
{
	BOOL fResult = FALSE;
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_19()
{
	BOOL fResult = FALSE;
	HRESULT hr = E_FAIL;
	...
	ReverseArray(m_rgParamOrdinals, m_cDbParamBindInfo, sizeof(ULONG));
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_20()
{
	BOOL fResult = FALSE;
	...
	ScrambleArray(m_rgParamOrdinals, m_cDbParamBindInfo, sizeof(ULONG));
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_21()
{
	BOOL fResult = TRUE;
	ULONG idxDupSource=0;	// First parameter is the source of the duplicate name
	ULONG idxDupTarget=m_cDbParamBindInfo-1;	// Last parameter is the duplicate
...
// @rdesc TEST_PASS or TEST_FAIL
//
int TCSetParameterInfo::Variation_22()
{

	BOOL fResult = TRUE;
	ULONG idxBadParam=m_cDbParamBindInfo-1;	// We'll use the last parameter
	LPOLESTR pwszSavedName;
...
// @rdesc TEST_PASS or TEST_FAIL 
//
int TCSetParameterInfo::Variation_23()
{ 
	...
	memcpy(rgDbParamBindInfo, m_rgDbParamBindInfo,  m_cDbParamBindInfo * sizeof(DBPARAMBINDINFO));
...
// @rdesc TRUE or FALSE
//
BOOL TCSetParameterInfo::Terminate()
{
	HRESULT hr;
...
int TCBugRegressions::Variation_1()
{
	...
	if (! (m_bSqlServer  && ( m_ulOutParamsSupported != DBPROPVAL_OA_NOTSUPPORTED) ))
	{
...
//		wcscpy (((WCHAR *)((BYTE *)pData + DbBindings[i].obValue)), wszInputStr);
		*((ULONG *)((BYTE *)pData + DbBindings[i].obLength)) = sizeof (VARIANT);
...
{ 
	...
	ULONG * prgColumnsOrd = NULL;
	BOOL fCanDerive = FALSE;
	ULONG ulSqlSupport = DBPROPVAL_SQL_NONE;
	...
	TESTC_(VerifyRowset(IID_IRowset, pIRowset, ulRowNum, cColumns, (ULONG *)prgColumnsOrd,
...
{ 
	...
	for (iParam = 0; iParam < cParams; iParam++)
	{
		...
		if (S_OK == hrConv)
		{
			...
			hrMakeData = m_pTable->MakeData(wszData, ulRowNum, pParamAll[iParam].ulColIndex, PRIMARY, TRUE);
	...
	// Create a new pData based on the new bindings
	TESTC_(FillInputBindings(m_pTable, DBACCESSOR_PARAMETERDATA, cParams,
		pBINDING64, &pData64, ulRowNum, cParams, (LONG *)pParamColOrd, PRIMARY), S_OK);
	...
	memcpy(pData64Match, pData64, cbRowSize64);
	...
	for (iParam = 0; iParam < cParams; iParam++)
	{
		...
		if (pBINDING64[iParam].eParamIO == DBPARAMIO_OUTPUT)
		{
			...
			memset(&VALUE_BINDING(pBINDING64[iParam], pData64), 
				0xCA,
				GetDataSize(pBINDING64[iParam].wType, pBINDING64[iParam].cbMaxLen));
		}
	}

	// In order to repro this bug you must do a partial SetParameterInfo.
...
//	hrSetParam = m_pICmdWParams->SetParameterInfo(cParams, pParamOrdinals, pPARAMBIND);
	...
	// If Prepare is called the bug won't repro either
//	if (m_pICommandPrepare)
...
LPVOID TCIUnknown::MakeData(CCol ColInfo, ULONG * pcbDataSize, ULONG ulSeed)
{
	...
	TEST_ALLOC(WCHAR, pwszDataItem, 0, DisplaySize(ColInfo)+sizeof(WCHAR));

	// Make a data item for this column
	TEST_CHECK(m_pTable->MakeData(pwszDataItem, ulSeed, ColInfo.GetColNum(), SECONDARY, wType, TRUE), S_OK);
	...
	REFIID	iidVerifyObject)
{
	...
	ULONG					iCol, cRefCount, cRowsObtained, cbReadProv, cbReadSrc;
	LONG					cRowsAffected;
	IRowset *				pIRowset=NULL;
	ISequentialStream *		pISeqStreamRow=NULL;
	IUnknown *				pIParamObject=NULL;
	HACCESSOR				hRowAccessor, hRowAccessorInLine, hParamAccessor;
	HROW					rghRows[1];
	...
	rgParamBind[0].obLength	=0;
	rgParamBind[0].obStatus	=rgParamBind[0].obLength+sizeof(ULONG);
	rgParamBind[0].obValue	=rgParamBind[0].obStatus+sizeof(ULONG);
	...
	if (!(m_fSupportInterface = m_ulStorageSupport & ulStorageInterface))
		odtLog << L"ISequentialStream is not supported.\n";
	...
	for (iFlag=0; iFlag < sizeof(g_StorageFlags)/sizeof(g_StorageFlags[0]); iFlag++)
	{
		...
		if (ParamStorageObject.iid == IID_ISequentialStream && g_StorageFlags[iFlag].dwMode != STGM_READ)
			continue;
		
		odtLog << g_StorageFlags[iFlag].wszMode << L"\n";

		// Storage object
		ParamStorageObject.dwFlags	=g_StorageFlags[iFlag].dwMode;

		// Create a parameter accessor for this parameter
		m_hr = m_pIAccessor->CreateAccessor(DBACCESSOR_PARAMETERDATA, 1, rgParamBind, sizeof(ParameterData),
		...
		{
			case E_NOINTERFACE:
				if (COMPARE(m_fSupportInterface, FALSE))
					m_TestResult = TEST_PASS;
				goto CLEANUP;
				break;
			case DB_E_ERRORSOCCURRED:
		...
		for (iCol=1; iCol <= m_pTable->CountColumnsOnTable(); iCol++)
		{
			...
			// Make sure column is updateable and a BLOB column
			// Remove testing of non-BLOB columns until spec bug and provider issues
			// are resolved.
//			if (!ColInfo.GetUpdateable() || !IsBLOB(ColInfo.GetProviderType()))
			if (!ColInfo.GetUpdateable()) // || !IsBLOB(ColInfo.GetProviderType()))
				continue;
			...
			// Due to Luxor bug, don't try data type "numeric"
			if (!wcscmp(ColInfo.GetProviderTypeName(), L"numeric") || 
				!wcscmp(ColInfo.GetProviderTypeName(), L"decimal"))
			{
				odtLog << L"Remove skipping of numerics once Luxor bug #38598 is fixed.\n";
				continue;
			}
			...
			ParameterData.pIUnknown = (IUnknown *)pIParamObject;

			// Set the length of the data
			ParameterData.ulDataSize = cbReadSize;

			// Set the status on input
			if (eTest == NULL_DATA)
				ParameterData.ulStatus = DBSTATUS_S_ISNULL;
			else if (eTest == DEFAULT_DATA)
				ParameterData.ulStatus = DBSTATUS_S_DEFAULT;
			else
				ParameterData.ulStatus = DBSTATUS_S_OK;
			...
			// storage object
			if (pIParamObject && NULL_DATA != eTest)
			...
			m_hr = m_pICommand->Execute(NULL, IID_IRowset, &Params, &cRowsAffected, (IUnknown **)&pIRowset);

 			if (pIParamObject)
			{
				// Make sure the ref count is what we expect. The param object will be gone.
//				if (FAILED(m_hr) || !COMPARE(pIParamObject->Release(), 0))
				if (!COMPARE(pIParamObject->Release(), 0))
				{
					...
					// Now we don't delete the object, leaking it to prevent a crash
					// in a case where other objects using this storage object try
					// to do some cleanup when the dll shuts down.
					//	delete pIParamObject;  // So we don't fail the next column
				}
			...
			switch(m_hr)
			{
				...
				case DB_E_ERRORSOCCURRED:
					if (eTest == NULL_DATA && !ColInfo.GetNullable())
					{
						...
						FAIL_COMPARE(ParameterData.ulStatus, DBSTATUS_E_INTEGRITYVIOLATION);
					}
					else if (!IsBLOB(ColInfo.GetProviderType()))
					{
						...
						FAIL_COMPARE(ParameterData.ulStatus, DBSTATUS_E_CANTCONVERTVALUE);
					}
					else
					{
			...
			swprintf(wszCommand, wszSelectFormat, ColInfo.GetColName(), m_pTable->GetTableName());
			ABORT_CHECK(m_pICommandText->SetCommandText(DBGUID_DBSQL, wszCommand), S_OK);
			ABORT_CHECK(m_pICommand->Execute(NULL, IID_IRowset, NULL, &cRowsAffected, (IUnknown **)&pIRowset), S_OK);
			ABORT_CHECK(pIRowset->GetNextRows(NULL, 0, 1, &cRowsObtained, &prghRows), S_OK);

			// Set Inline result so we know whether to release inline objects
			hrInLine=E_FAIL;

			// Try to get the data using the IUnknown accessor
			hrObject=pIRowset->GetData(*prghRows, hRowAccessor, (void *)&RowData);
			...
			if (!SUCCEEDED(hrObject))
			{
				...
				hrInLine=pIRowset->GetData(*prghRows, hRowAccessorInLine, (void *)&RowData);
			...
			if (SUCCEEDED(hrObject) || SUCCEEDED(hrInLine))
			{
				...
				if (COMPARE(RowData.ulStatus, ulExpStatus))
				{
					switch(RowData.ulStatus)
					{
						case DBSTATUS_S_ISNULL:
						...
						case DBSTATUS_S_OK:
							if (hrObject == S_OK)
							{
								...
								pISeqStreamRow = (ISequentialStream *)RowData.pIUnknown;

								// Initialize our source ISequentialStream to point to beginning
								m_pISeqStream->Init(pvSrcData, cbSrcData, cbReadSize);
								...
								do
								{
									...
									ABORT_CHECK(m_pISeqStream->Read(pvCmpBuf, cbSrcData, &cbReadSrc), S_OK);
							...
							else
							{
									...
									eTest == EMPTY_STORAGE_OBJECT))
								{
									...
									FAIL_COMPARE(RowData.ulDataSize, 0);
								}
								else
								{
									// Have to verify using in-line data.  
									if (COMPARE(RowData.ulDataSize, cbReadSize))
									{
										BYTE * pvData = RowData.bData;
										...
										if (ColInfo.GetProviderType() == DBTYPE_WSTR)
										{
											FAIL_COMPARE(((WCHAR *)RowData.bData)[cbReadSize/sizeof(WCHAR)], L'\0');
										}
										if (ColInfo.GetProviderType() == DBTYPE_STR)
										{
											FAIL_COMPARE(((CHAR *)RowData.bData)[cbReadSize], '\0');
										}
									}
									else
										m_TestResult = TEST_FAIL;
...
CLEANUP:
	...
	m_pIAccessor->ReleaseAccessor(hRowAccessor, &cRefCount);
	FAIL_COMPARE(cRefCount, 0);
