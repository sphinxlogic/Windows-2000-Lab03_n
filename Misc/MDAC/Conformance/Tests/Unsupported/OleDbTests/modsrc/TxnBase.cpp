...
BOOL	CTxnChgRowset::Init() 
{
	...
	if (CTxnRowset::Init())
	{
		...
		{		
			...
			m_rgUpdateableCols = (LONG *)PROVIDER_ALLOC(m_pTable->CountColumnsOnTable() * sizeof(ULONG));
...
// @mfunc Insert
//--------------------------------------------------------------------
BOOL	CTxnChgRowset::Insert(ULONG ulRowNum, HROW * phNewRow)
{
	BYTE *				pData = NULL;
	BOOL				fResults = FALSE;
	...
	//Now Insert the row
	if (pIRowsetChange->InsertRow(NULL, m_hUpdateAccessor,
		pData, &hNewRow) == S_OK)		
		fResults = TRUE;	
...
// @mfunc Change
//--------------------------------------------------------------------
BOOL	CTxnChgRowset::Change(ULONG ulDeleteRowNum, ULONG ulInsertRowNum, HROW * phRow)
{
	BOOL				fResults = FALSE;
	HROW				hFoundRow = DB_NULL_HROW;
	BYTE *				pData = NULL;
...
{	
	...
	if (m_pRORowset1)
	{
		(CTxnRowset *)m_pRORowset1->Terminate();
		delete m_pRORowset1;
	}
