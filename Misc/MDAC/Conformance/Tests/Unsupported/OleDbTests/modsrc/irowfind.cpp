...
ULONG				g_ulColNum;					// Column number to test FindValue match on.
DBTYPE				g_wColType;					// DBTYPE of that column
LONG				g_lRowLast;					// the index of last row in g_pCTable.
LONG				g_lNullCollation;			// contains the value of the NULL_COLLATION property
...
class TCIRowsetFind : public CRowsetObject
{
	...
	protected:
		...
		(
			...
			ULONG			cbBookmark,			// Length of bookmark.
			LONG			lRowsToFetch,		// maps to cRows
			LONG			lOffset,			// maps to lOffset
			ULONG			ulColToMatch,		// Which column to match
			ULONG			ulRowToMatch,		// Is there a row where the find should happen? 0 - no match
			HRESULT			hrExpected,			// Expected HRESULT
			ULONG			ulRowsExpected,		// Expected count of rows
...
)
{
	...
	if ( hr == S_OK && wVariantType != DBTYPE_NULL )
	{
		...
		m_pFindValue = (BYTE *)PROVIDER_ALLOC(cbDataLength+sizeof(DBSTATUS)+sizeof(ULONG));				
	...
	{
		m_pFindValue = (BYTE *)PROVIDER_ALLOC(sizeof(DBSTATUS)+2*sizeof(ULONG));
		...
		*(ULONG *)(m_pFindValue+offsetof(FindValueInfo, cbLength)) = 0xFFFFFFFF;
	}
	else
	{
		*(DBSTATUS *)(m_pFindValue+offsetof(FindValueInfo, dbsStatus)) = DBSTATUS_S_OK;
		...
		{
		case DBTYPE_WSTR:
			*(ULONG *)(m_pFindValue+offsetof(FindValueInfo, cbLength)) = cbDataLength-sizeof(WCHAR);
			break;
		case DBTYPE_STR:
			*(ULONG *)(m_pFindValue+offsetof(FindValueInfo, cbLength)) = cbDataLength-sizeof(char);
			break;
		default:
			*(ULONG *)(m_pFindValue+offsetof(FindValueInfo, cbLength)) = cbDataLength;
			break;		
		}
	}
	...
	if ( FAILED( m_pIAccessor->CreateAccessor(DBACCESSOR_ROWDATA, 1, &Binding, sizeof(ULONG)+sizeof(DBSTATUS)+cbDataLength, 
...
)
{
	...
	if(hr == S_OK && wVariantType != DBTYPE_NULL)
	{
		...
		if ( fUseStream )
		{
			...
			m_pISeqStream->Write(pMakeData, cbDataLength, &cbTmp);
	...
	if ( hr == S_FALSE || wVariantType == DBTYPE_NULL)
	{
		...
		if (CompareOp != DBCOMPAREOPS_IGNORE)
		{
			...
			*(DBSTATUS *)(m_pFindValue+offsetof(FindValueInfo, dbsStatus)) = DBSTATUS_S_ISNULL;
			...
			// bogus length shouldn't matter since NULL status was set
			*(ULONG *)(m_pFindValue+offsetof(FindValueInfo, cbLength)) = 0xFFFFFFFE;
		}
	}
	else
	{
	...
	TESTC(SUCCEEDED(m_pIAccessor->CreateAccessor(DBACCESSOR_ROWDATA, 1,	&Binding,
			sizeof(ULONG)+sizeof(DBSTATUS)+cbDataLength, 	&m_hRowsetFindAccessor, NULL)))
			goto CLEANUP;
...
(		
	...
	LONG			lRowsToFetch,	// maps to cRows
	LONG			lOffset,		// maps to lOffset
	ULONG			ulColToMatch,	// Column to match
	ULONG			ulRowToMatch,	// Is there a row where the find should happen? 0 - no match
	HRESULT			hrExpected,		// Expected HRESULT
	ULONG			ulRowsExpected,	// Expected count of rows
...
)
{
	...
	ULONG	cchDataLength = 0;
	ULONG	cbDataLength = *pcbDataLength;
	...
	case DBCOMPAREOPS_CONTAINS:
		...
		case SUBOP_CONTAINS_END:
		{
			...
			case DBTYPE_STR:
				...
				ULONG cbTrim = MYABS((char *)pMakeData - szTmp);
	...
	case DBCOMPAREOPS_LE:
	{
		...
		case DBTYPE_STR:
			...
			*ppMakeData = pReAllocData;
			break;
		case DBTYPE_BSTR:
			pReAllocData = SysAllocStringLen(NULL, cchDataLength+1);
...
)
{
	...
	switch ( wColType )
	{
		...
		case DBTYPE_DBTIME:
			...
			time->hour = lSeconds / 3600;
			time->minute = (lSeconds % 3600) / 60;
			time->second = lSeconds % 60;
			break;
			}
		case DBTYPE_DBTIMESTAMP:
			{
...
)
{
	...
	ASSERT(*pcbDataLength <= 111);  // max varnumeric byte length is 111.
	...
	case DBCOMPAREOPS_EQ:
		...
		case 1:
			...
			pNewVarNum = (DB_VARNUMERIC *)PROVIDER_ALLOC(*pcbDataLength+1);
			if (!pNewVarNum)
				return FALSE;

			memset(pNewVarNum, 0, *pcbDataLength+1);
			memcpy(pNewVarNum, pVarNum, *pcbDataLength);
			AddCharToVarNumericVal(L'0',pNewVarNum, USHORT(*pcbDataLength+1)-sizeof(DB_VARNUMERIC)+1);
		...
		case 2:
			...
			pTempVar = (DB_VARNUMERIC *)PROVIDER_ALLOC(*pcbDataLength);
			if (!pTempVar)
				return FALSE;

			memcpy(pTempVar, pVarNum, *pcbDataLength);

			if (pVarNum->scale==s_MinScale)
				return FALSE;
			
			VarNumericDiv10Rem(pTempVar, pTempVar, (*pcbDataLength)-sizeof(DB_VARNUMERIC)+1, &bRemainder);
			if(bRemainder!=0)
				return FALSE; // not able to guarantee unchanged value

			// this can work, so copy the contents
			memcpy(pVarNum, pTempVar, *pcbDataLength);
	...
	case DBCOMPAREOPS_GE:
		...
		{
			VarNumericDiv10Rem(pVarNum, pVarNum, *pcbDataLength, &bRemainder);
			pVarNum->precision--;	// decrement the precision to compensate for div by 10
	...
	case DBCOMPAREOPS_LE:
		...
		if (pVarNum->scale==s_MinScale)
		{
			...
			memset(pNewVarNum, 0, *pcbDataLength+1);
			memcpy(pNewVarNum, pVarNum, *pcbDataLength);
			AddCharToVarNumericVal(L'1', pNewVarNum, USHORT(*pcbDataLength)-sizeof(DB_VARNUMERIC)+1);
			PROVIDER_FREE(pVarNum);
...
)
{
	...
	HROW *		phrow = NULL;
	ULONG		cRowsObtained, ulColToFind;
	DBTYPE		wColType = DBTYPE_EMPTY;
...
)
{
	...
	CCol	TempCol;
	ULONG	ulColNum = pCTable->CountColumnsOnTable();
	ULONG	ulRowCount = pCTable->GetRowsOnCTable();
...
int Boundary::Variation_7()
{
	BOOL fTestPass=TRUE;
	CCol TempCol;
	ULONG ulColNum = g_pCTable->CountColumnsOnTable();
	ULONG ulRowCount = g_pCTable->GetRowsOnCTable();
	ULONG ulColIndex;
...
int Boundary::Variation_8()
{
	...
	// Lets create the accessor.
	if ( FAILED( m_pIAccessor->CreateAccessor(DBACCESSOR_ROWDATA, 0, NULL, sizeof(ULONG)+sizeof(DBSTATUS), 
						&m_hRowsetFindAccessor, NULL)) )
	{
		fTestPass = TEST_FAIL;
		goto CLEANUP;
...
int OutputRowHandleAllocation::Variation_4()
{
	...
	m_hr = m_pIRowsetFind->FindNextRow(DB_NULL_HCHAPTER, m_hRowsetFindAccessor, m_pFindValue, 
													...
													LONG_MAX, &cRowsObtained, &rghRows);
...
int AccessorTests::Variation_1()
{
	...
	DBPROPID		guidProperty[1];
	HROW			*phrow = NULL;
	ULONG			cRowsObtained, ulColToFind, cbDataLength;
	USHORT			usize;
	HRESULT			hr;
	void			*pMakeData = NULL;
	CCol			TempCol;
