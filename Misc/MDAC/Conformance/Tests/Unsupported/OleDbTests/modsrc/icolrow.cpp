	...
	if( ModuleCreateDBSession(pThisTestModule) )
	{
								...
								pThisTestModule->m_pIUnknown, &wszProviderName))
		{
			if (!wcscmp((LPWSTR)wszProviderName, L"MSDASQL.DLL")) 
				g_fKagera=TRUE;
			if (!wcscmp((LPWSTR)wszProviderName, L"MSDAORA.DLL")) 
				g_fOracle=TRUE;
		}
		...
		if( FAILED(pTable->CreateTable(10)) )
			return FALSE;
...
protected:
	...
	ULONG			m_ulCatalogSupport;
	// Schema support (0 for none)
	ULONG			m_ulSchemaSupport;
...
public:
	...
	HRESULT GetAvailableColumns(
		ULONG* cOptColumns,							// [IN/OUT] Count of optional columns
	...
	BOOL Compare_DBCOLUMNFLAGS_ISBOOKMARK(LONG iOrdinal, WCHAR * pwszName, LONG dwFlags, BOOL fExpBookmarks);
...
BOOL IColRow::Init()
{
	...
	ULONG ulIdentCase = DBPROPVAL_IC_UPPER;
	ULONG ulQuotedIdentCase = DBPROPVAL_IC_UPPER;
...
)
{
	...
	LONG rgColsToBind[TABLES_COLS] = {1, 2, 3};
...
void IColRow::DumpCommandProps(IUnknown * pIUnknown, BOOL fPropertiesInError)
{
	...
	//	pICmdProp->GetProperties(1, rgPropertyIDSets, &cPropertySets, &pPropertySets);
		...
		if (0)
		{
			// Here we test that the provider ignores rgPropertyIDSets if cPropertyIDSets == 0.
			// I just happened to stumble upon this bug and needed a repro.
			hrGetProp = pICmdProp->GetProperties(0, rgPropertyIDSets, &cPropertySets, &pPropertySets);

			if (FAILED(hrGetProp))
			{
				odtLog << L"GetProperties failed!";
				return;
			}

			cPropZero = pPropertySets[0].cProperties;
			FreeProperties(&cPropertySets, &pPropertySets);

			hrGetProp = pICmdProp->GetProperties(0, NULL, &cPropertySets, &pPropertySets);
			...
			cPropZeroNULL = pPropertySets[0].cProperties;

			if (!COMPARE(cPropZero, cPropZeroNULL))
				odtLog << L"\nError: Provider doesn't ignore rgPropertyIDSets when cPropertyIDSets == 0!\n\n";
		}
...
)
{
	...
	if( SUCCEEDED(hr) )
	{
		...
		if( pColRowset && QIRowset(pColRowset) && VerifyRowset(pColRowset,iid,stmt) )
			fSucceed = TRUE;
	}
	else
	{
...
)
{
	...
	switch(eArrangeOption)
	{
		...
		case ALLDBID:
			...
			if(m_rgDBID)
			{
				memcpy(m_rgDBID,rgOriginalColumns,sizeof(DBID)*m_cDBID);
				fSucceed = TRUE;
			}
		...
		case DUPLICATEDBID:
			...
			if(m_rgDBID)
			{
				...
				for(index=cOriginalColumns; index<m_cDBID; index++)
					memcpy(&(m_rgDBID[index]), &(rgOriginalColumns[index-cOriginalColumns]), sizeof(DBID));
		...
		case INVALIDDBID:
			...
			if(m_rgDBID)
			{
				for(ULONG index=0; index<cOriginalColumns; index++)
					memcpy(&(m_rgDBID[index]), &(rgOriginalColumns[index]), sizeof(DBID));
...
)
{
	...
	ULONG			cBinding		= 0;		// count of bindings
...
)
{
	...
	if( m_fOriginalRowsetHasBookmark && !iRow)
	{
		...
		if(COMPARE((ULONG)VALUE_BINDING(rgBindings[4], pData), 0))
		{
	...
	// We can't check the value of hidden columns
	// TODO: Actually, per spec we should be able to do some minimal checking.
	...
	if (iRow+1 > m_pTable->CountColumnsOnTable() + m_fHasOrdinalZero)
	{
		...
		return fCompareExtra;
	}

	// Get Query Col that this row should match
	if( FAILED(m_pTable->GetColInfo(iRow + 1 - m_fHasOrdinalZero, col)) )
	{
...
BOOL IColRow::Compare_DBCOLUMN_NAME(CCol col, DATA * pColumn)
{
	...
	if( Compare_StatusInfo(col, DBSTATUS_S_OK, pColumn, L"DBCOLUMN_NAME", ALLOW_NULLS) )
	{
		...
		// allowed to be NULL.
		if(pColumn->sStatus == DBSTATUS_S_OK && ((WCHAR *)pColumn->bValue) && col.GetColName() )
		{
...
BOOL IColRow::Compare_DBCOLUMN_COLUMNSIZE(CCol col, DATA * pColumn)
{
	...
	if( Compare_StatusInfo(col, DBSTATUS_S_OK, pColumn, L"DBCOLUMN_COLUMNSIZE") )
	{
		...
		{
			// Check the values
			if( *((ULONG*)pColumn->bValue) == SizeOfDBTYPE(col.GetProviderType()) )
				fSuccess = TRUE;
			else
				odtLog<< L"[ " << col.GetColName() << L" ] "
					  << L"ERROR The column size for fixed type should be "<< SizeOfDBTYPE(col.GetProviderType())
					  << L", returned "<< *((ULONG*)pColumn->bValue)<< ENDL;
		}
	}
...
BOOL IColRow::Compare_DBCOLUMN_TABLENAME(CCol col, DATA * pColumn)
{
	...
	if( Compare_StatusInfo(col, DBSTATUS_S_OK, pColumn, L"DBCOLUMN_BASETABLENAME", ALLOW_NULLS) )
	{
		...
		if( m_pTable->GetTableName() && *(WCHAR*)pColumn->bValue )
		{
...
//	SizeOfDBTYPE
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
ULONG IColRow::SizeOfDBTYPE(DBTYPE dbtype)
{
 		if(DBTYPE_I1 == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(signed char);

 		if(DBTYPE_I2 == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(SHORT);

 		if(DBTYPE_I4 == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(LONG);

 		if(DBTYPE_I8 == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(LARGE_INTEGER);

 		if(DBTYPE_UI1 == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(BYTE);

 		if(DBTYPE_UI2 == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(unsigned short);

 		if(DBTYPE_UI4 == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(unsigned int);

 		if(DBTYPE_UI8 == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(ULARGE_INTEGER);

 		if(DBTYPE_R4 == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(float);

 		if(DBTYPE_R8 == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(double);

 		if(DBTYPE_CY == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(LARGE_INTEGER);

 		if(DBTYPE_DATE == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(DATE);

 		if(DBTYPE_BOOL == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(VARIANT_BOOL);

 		if(DBTYPE_IUNKNOWN == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(IUnknown *);

 		if(DBTYPE_GUID == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(GUID);

 		if(DBTYPE_NUMERIC == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(DB_NUMERIC);

 		if(DBTYPE_DBDATE == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(DBDATE);

 		if(DBTYPE_DBTIME == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(DBTIME);

 		if(DBTYPE_DBTIMESTAMP == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(DBTIMESTAMP);

 		if(DBTYPE_DECIMAL == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
			return sizeof(DECIMAL);

 		if(DBTYPE_VARIANT == (~(DBTYPE_BYREF|DBTYPE_VECTOR|DBTYPE_ARRAY) & dbtype))
...
int Cmd_GetColumnsRowset::Variation_31()
{
	...
	// Set the Command Text
	TESTC_(hr=SetCommandText(m_pIMalloc, m_pICommand, m_pTable,
								 NULL,eSELECT,SELECT_ALL_WITH_FOR_BROWSE,NULL), S_OK);

	// Prepare
	QTESTC(SUCCEEDED(hr=PrepareCommand(m_pICommand,PREPARE,1)));
...
int Cmd_GetColumnsRowset::Variation_32()
{
	...
	// Set the Command Text
	TESTC_(hr=SetCommandText(m_pIMalloc, m_pICommand, m_pTable,
								 NULL,eSELECT,SELECT_COLLISTORDERBYCOLONECOMPUTEBY,NULL), S_OK);
