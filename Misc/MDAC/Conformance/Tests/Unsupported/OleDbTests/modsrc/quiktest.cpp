...
protected:
	...
	WCHAR*		AppendToInitString(
					WCHAR*		pwszInitString,  //[IN/OUT] String to which value is to be appended.
					DBPROPID	dwPropertyID,    //[IN] Property ID of property to add to string.
					const GUID& guidPropertySet, //[IN] Property set of property to add to string.
					VARIANT*	pVariant         //[IN] Value of the property.
					);
...
//
WCHAR* CQuickTest::AppendToInitString(
	WCHAR* pwszInitString,       //[IN/OUT] String to which value is to be appended.
	DBPROPID dwPropertyID,       //[IN] Property ID of property to add to string.
	const GUID& guidPropertySet, //[IN] Property set of property to add to string.
	VARIANT* pVariant            //[IN] Value of the property.
	)
{
	...
	WCHAR* pwszKeyword	= GetStaticPropDesc(dwPropertyID, guidPropertySet);

	TESTC(pwszKeyword != NULL);

	TESTC_(VariantToString(pVariant, wszBuffer, MAX_QUERY_LEN, FALSE),
		S_OK);

	//Add this to the String
	if(pwszInitString)
	{
		pwszNewString = CreateString(L"%s; %s =%s", pwszInitString, 
			pwszKeyword, wszBuffer);
	}
	else
	{
		pwszNewString = CreateString(L"%s =%s", pwszKeyword, wszBuffer);
	}
	...
	)
{
	...
	{
		odtLog<<L"Failed to get IRowset interface.\n";
		return FALSE;
	}
...
CLEANUP:
	if(!bSuccess)
		odtLog<<L"Failed to get bookmark for row "<<ulRow<<L".\n";
	...
	)
{
	...
	*pfExists = FALSE;
	...
	if(FAILED(hr))
	{
		odtLog<<L"GetNextRows on Index_Schema Rowset failed.\n";
		CHECK(hr, S_OK);
...
TESTRESULT CQuickTest::testSetProperties1()
{
	...
	{
		odtLog<<L"Failed to create another DSO.\n";
		return TEST_FAIL;
	}
...
TESTRESULT CQuickTest::testIDataIzGetDS()
{
	...
	for(i=0; i<cPropSets; i++)
	{
		...
		for(j=0; j<pPropSet->cProperties; j++)
		{
			...
			pwszTemp = pwszInitString;
			pwszInitString = AppendToInitString(pwszInitString, 
				pProp->dwPropertyID, pPropSet->guidPropertySet, 
				&pProp->vValue);
...
TESTRESULT CQuickTest::testGetDataSource()
{
	...
	{
		odtLog<<L"Failed to get IGetDataSource.\n";
		return TEST_FAIL;
	}
...
TESTRESULT CQuickTest::testSessGetProp()
{
	...
	{
		odtLog<<L"Failed to get ISessionProperties.\n";
		return TEST_FAIL;
	}
...
TESTRESULT CQuickTest::testAddColumn()
{
	...
	for (cOldCol=cNewCol=0; cNewCol<cColDesc2; cNewCol++)
	{
		...
		else
		{
			...
			TESTC(CompareDBID(pColumnDesc->dbcid, rgColDesc2[cOldCol].dbcid));
...
CLEANUP:
	...
	if(pNewTableID)
	{
		...
		if(hr != S_OK)
		{
			odtLog<<L"DropTable failed at CLEANUP.\n";
			CHECK(hr, S_OK);
			tTestResult = TEST_FAIL;
		}
	}
...
TESTRESULT CQuickTest::testIIndexDef()
{
	...
	if(pIDSR)
	{
		...
		{
			odtLog<<L"DoesIndexExist failed.\n";
...
TESTRESULT CQuickTest::testGetColumnInfo(IUnknown* pIUnknown)
{
	...
	if(fColIDWarning)
	{
		...
		odtLog<<L"WARNING: ColID returned is of different DBKIND. This may be due to a test bug in our Privlib code.\n";
	}
...
TESTRESULT CQuickTest::testFindNextRow()
{
	...
	SAFE_ALLOC(pFindValue, BYTE, cbDataLength+sizeof(DBSTATUS)+sizeof(DBLENGTH))
...
TESTRESULT CQuickTest::testIRowsetChangeSet()
{
	...
	if(!GetProperty(DBPROP_UPDATABILITY, DBPROPSET_ROWSET, (IUnknown*)
		m_pIRowset, &updtFlag))
	{
		odtLog<<L"Failed to get the value of DBPROP_UPDATABILITY.\n";
		return TEST_SKIPPED;
	}
	...
	)
{
	...
	{
		odtLog<<L"WARNING: All parameter names were NULL. Hence, the names buffer should be NULL.\n";
		odtLog<<L"Instead of NULL, the names buffer contains "<<pwszNamesBuffer<<L".\n";
		COMPAREW(pwszNamesBuffer, NULL);
	}
	...
	for(ulIndex=0; ulIndex<cParams; ulIndex++)
	{
		...
		COMPAREW(rgLocalParamInfo[ulIndex].pTypeInfo, 
			TempCol.GetTypeInfo()) ;
		COMPAREW(rgLocalParamInfo[ulIndex].wType,TempCol.GetProviderType());
		COMPAREW(rgLocalParamInfo[ulIndex].ulParamSize,
			rgDbParamBindInfo[ulIndex].ulParamSize) ;
		COMPAREW(rgLocalParamInfo[ulIndex].bPrecision, 
			rgDbParamBindInfo[ulIndex].bPrecision) ;
		COMPAREW(rgLocalParamInfo[ulIndex].bScale, 
			rgDbParamBindInfo[ulIndex].bScale) ;
	}
...
TESTRESULT CQuickTest::testICmdWParam()
{
	...
	if(FAILED(hr))
	{
		odtLog<<L"WARNING: Executing Command with parameters failed. The syntax used may not supported by the provider.\n";
		CHECKW(hr, S_OK);
		tTestResult = TEST_SKIPPED;
		goto CLEANUP;
	}
...
BOOL CQuickTest::testIRowSchemaChange(CRowObject* pCRowObj)
{
	...
	//No need to modify rgColAccess[0].columnid. This should be ignored.
...
BEG_TEST_CASE(TCSession, CQuickTest, L"Session Interfaces")
	...
	TEST_VARIATION(15, 		L"AggregateSession")
END_TEST_CASE()
#undef THE_CLASS
// }} TCW_TESTCASE_END
// }} TCW_TEST_CASE_MAP_END
...
BOOL TCDataSource::Init()
{
	...
	// }}
	{
		if(CHECK(m_hr = CreateDataSourceObject(),S_OK))
		{
			...
			if(m_pIDBProperties == NULL)
			{
				odtLog<<L"Failed to get IDBProperties (level-0) interface.\n";
				return FALSE;
			}
...
int TCDataSource::Variation_2()
{
	...
	if(hr != S_OK)
	{
		odtLog<<L"Uninitialize (before Initialize) failed.\n";
		goto CLEANUP;
	}
...
{ 
	...
	FreeProperties(&m_cPropSets, &m_rgPropSets);
	SAFE_RELEASE(m_pIRowset);
	DBCOUNTITEM ulRows = m_pTable->GetRowsOnCTable();
...
{ 
	...
	TESTC(CheckColumnsRowset((IUnknown*) pIRowset, cOptCols, FALSE))
...
{ 
	...
	TEST2C_(hr = m_pIBindResource->Bind(NULL, m_pwszRowsetURL, 
		DBBINDURLFLAG_READ, DBGUID_ROWSET, IID_IRowset, 
...
{ 
	...
	//that their values match to the default ones stored in vrgDefault.
	//
