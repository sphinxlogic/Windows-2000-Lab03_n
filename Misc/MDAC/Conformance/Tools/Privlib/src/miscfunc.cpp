...
VALUE:
		...
		{	
			...
			// Make data based on the column number and the row number of the retrieved data in the table
			hr = pSchema->MakeData(wszData, cRow, iOrdinal, eValue, wDBType); 
			...
			{
				void* pDBTypeData = WSTR2DBTYPE(wszData, wDBType, &uswSize);
				pBackEndData = DBTYPE2VARIANT(pDBTypeData, wDBType);
			...
			if (pBinding->wType & DBTYPE_BYREF)
			{
				...
				if (!CompareDBTypeData( (void *)(*(DWORD_PTR *)pConsumerData), pBackEndData,
					...
					NULL, TRUE, wBackEndType & (~DBTYPE_BYREF), cbConsumerSize))
					fColumnError=TRUE;
					...
					{	
						...
						if(hr==S_OK || hr==S_FALSE)
						{
							...
							if (!CompareDBTypeData(pBuffer, pBackEndData, wBackEndType, (USHORT)cBytes,	
									...
									wBackEndType & (~DBTYPE_BYREF), cBytes))
								fColumnError=TRUE;
						}
						else
						{
...
{	
	...
	hr=SafeArrayAccessData(pConsumerData, (void HUGEP* FAR*)&pArrayConsumer);
	//hr=SafeArrayAccessData(pBackEndData, (void HUGEP* FAR*)&pArrayBackend);
	...
	switch (wType)
	{
		case(DBTYPE_I4):
			...
			for(ulIndex=0; ulIndex<ulElements; ulIndex++)
				if(((LONG *)pArrayConsumer)[ulIndex] != ((LONG *)(pBackEndData->pvData))[ulIndex])
					goto END;
			break;
	...
	/*
	// Loop through the safearray, compare element by element
	for(ulCnt=1;ulCnt<=ulElements;ulCnt++)
	{
		// Get a copy of the index of the element
		ulIndex=ulCnt;

		// Calculating the indices of the element in the safearray
		// one dimension at a time, starting from the most significant dimension
		for(ulDimCount=ulDim;ulDimCount >1; ulDimCount--)
		{	
			// Calculating the sum of elements in subdimensions
			ulSum -= (pUBound[ulDimCount-1]-pLBound[ulDimCount-1]+1);

			// Divide the index of the element in the whole array by the sum
			// of the elements in subdimensions
			div_result=div(ulIndex,ulSum);

			// The quot of the division minus 1 is the index of the element 
			// in the dimension
			pIndices[ulDimCount-1]= div_result.quot-1+pLBound[ulDimCount-1];

			// The remainder of the division is used to calcalate the index
			// of the element in the next dimension
			ulIndex=div_result.rem;
		}

		// The remainder of the last division is the index of the element in the
		// first dimension
		pIndices[0]=div_result.rem-1+pLBound[0];

		// Init the variant
		VariantInit(&DataOrg);
		VariantInit(&DataCpy);

		// Get the element in consumer's buffer
		hr=SafeArrayGetElement(pConsumerData,pIndices, &DataOrg);
		
		if (FAILED(hr))
			goto END;

		// Get the element at the backend
		hr=SafeArrayGetElement(pBackEndData, pIndices, &DataCpy);
		
		if(FAILED(hr))
			goto END;
		
		// If wType!= INVALID_DBTYPE, we need to make sure the vt type of the element
		// is the same as expected 
		if (wType!=INVALID_DBTYPE);
			if (wType!= VARTYPE2DBTYPE(DataOrg.vt))
				goto END;
				
		// Compare the data in the consumer's buffer with backend data
		fSame=CompareVariant(&DataOrg,&DataCpy);

		// Clear the variant
		GCHECK(VariantClear(&DataOrg),S_OK);
		GCHECK(VariantClear(&DataCpy),S_OK);
...
//				if (*(double *)pConsumerData == *(double *)pBackEndData)
		...
		case DBTYPE_VARIANT:
				fSame=CompareVariant((VARIANT *)pConsumerData,(VARIANT *)pBackEndData);
			goto END;
...
)
{
	...
	if (pVariantOrg->vt == (VT_VARIANT | VT_BYREF))
		return CompareVariant(V_VARIANTREF(pVariantOrg), V_VARIANTREF(pVariantCpy));
		
	// Check if the variant is a safearray, no need to verify the corresponding DBTYPE.
	// do not free memory
	if (V_VT(pVariantOrg) & VT_ARRAY)
		//return CompareSafeArray(V_ARRAY(pVariantOrg), V_ARRAY(pVariantCpy), INVALID_DBTYPE, NULL,FALSE);
		return CompareSafeArray(V_ARRAY(pVariantOrg), V_ARRAY(pVariantCpy), V_VT(pVariantOrg), NULL,FALSE);
	...
	{	
		...
		case VT_R8:
//			return V_R8(pVariantOrg) == V_R8(pVariantCpy);
 			return 	fabs(V_R8(pVariantOrg) - V_R8(pVariantOrg)) <= fabs(g_dDoubleTolerance * V_R8(pVariantOrg));
		...
		case VT_DATE:
			return V_DATE(pVariantOrg) == V_DATE(pVariantCpy);
		...
		case VT_DECIMAL:
			return CompareDecimal(&V_DECIMAL(pVariantOrg), &V_DECIMAL(pVariantCpy));

		// As we are not testing OLE object, return FALSE for VT_UNKNOWN
		case VT_UNKNOWN:
			return (V_UNKNOWN(pVariantOrg) == V_UNKNOWN(pVariantCpy));

		// As we are not testing OLE object, return FALSE for VT_DISPATCH
		case VT_DISPATCH:
			return (V_DISPATCH(pVariantOrg) == V_DISPATCH(pVariantCpy));

		case VT_I1:
			return V_I1(pVariantOrg) == V_I1(pVariantCpy);

		case VT_I2 | VT_BYREF:
			return *V_I2REF(pVariantOrg) == *V_I2REF(pVariantCpy);

		case VT_I4 | VT_BYREF:
			return *V_I4REF(pVariantOrg) == *V_I4REF(pVariantCpy);

		case VT_UI2 | VT_BYREF:
			return *V_UI2REF(pVariantOrg) == *V_UI2REF(pVariantCpy);

		case VT_UI4 | VT_BYREF:
			return *V_UI4REF(pVariantOrg) == *V_UI4REF(pVariantCpy);

		case VT_R4 | VT_BYREF:
			return *V_R4REF(pVariantOrg) == *V_R4REF(pVariantCpy);

		case VT_R8 | VT_BYREF:
			return *V_R8REF(pVariantOrg) == *V_R8REF(pVariantCpy);

		case VT_BOOL | VT_BYREF:
			return *V_BOOLREF(pVariantOrg) == *V_BOOLREF(pVariantCpy);

		case VT_ERROR | VT_BYREF:
			return *V_ERRORREF(pVariantOrg) == *V_ERRORREF(pVariantCpy);

		case VT_CY | VT_BYREF:
			return memcmp(V_CYREF(pVariantOrg), V_CYREF(pVariantCpy),8)==0;

		case VT_DATE | VT_BYREF:
			return *V_DATEREF(pVariantOrg) == *V_DATEREF(pVariantCpy);

		case VT_BSTR | VT_BYREF:
			if(fCaseSensitive)
				return wcscmp(*V_BSTRREF(pVariantOrg), *V_BSTRREF(pVariantCpy))==0;
			else
				return _wcsicmp(*V_BSTRREF(pVariantOrg), *V_BSTRREF(pVariantCpy))==0;

		case VT_DECIMAL | VT_BYREF:
			return memcmp(V_DECIMALREF(pVariantOrg), V_DECIMALREF(pVariantCpy),sizeof(DECIMAL))==0;
		...
		case VT_I1 | VT_BYREF:
			return *V_I1REF(pVariantOrg) == *V_I1REF(pVariantCpy);
...
)
{
	...
	dwDBType = dwDBType & (~DBTYPE_BYREF);
...
void * WSTR2DBTYPE_EX(
	WCHAR *		pwszSource,	//@parm [IN] String to be converted, must NOT be null
	DBTYPE		dbDestType,	//@parm [IN] DBTYPE to convert to
	ULONG *		pcb,		//@parm [OUT] Pointer to ULONG to store count of bytes.
	BOOL		fLaxConvert	//@param [IN] optional Whether to use strict error checking. DEFAULT = FALSE
)
{
	...
	ASSERT(pwszSource);
	ULONG cBytes = (ULONG)GetDBTypeSize(dbDestType);
	...
	switch(dbDestType)
	{
		...
		{	
			...
			cBytes = (ULONG) WideCharToMultiByte(CP_ACP, 0, pwszSource, -1, NULL, 0, NULL, NULL);
			pDestBuf = (BYTE *)PROVIDER_ALLOC(cBytes);
			if (!pDestBuf)
				break;

			// Now convert string and return
			if (!WideCharToMultiByte(CP_ACP, 0, pwszSource, -1, (CHAR *)pDestBuf, cBytes, NULL, NULL))
			{
	...
	else
	{
		...
		{				
				...
				eColsMemProvOwned == SUPPORTED_COLS_OWNED_BY_PROV)
			{
				...
				if (dwAccessorFlags & DBACCESSOR_PARAMETERDATA && fParamConv)
				{
					if (FAILED(hr = pIConvertType->CanConvert(rgDBBINDING[ulBindidx].wType|DBTYPE_BYREF,
						rgDBCOLUMNINFO[ulColidx].wType, DBCONVERTFLAGS_PARAMETER)))
						goto CLEANUP;
					if (S_FALSE == hr)
						fBYREFSupport = FALSE;
				}
				...
				{
					if (FAILED(hr = pIConvertType->CanConvert(rgDBCOLUMNINFO[ulColidx].wType,
			...
			if (eColsMemProvOwned == SUPPORTED_COLS_OWNED_BY_PROV &&
				!((rgDBBINDING[ulBindidx].wType & DBTYPE_BYREF) || 
				rgDBCOLUMNINFO[ulColidx].wType == DBTYPE_BSTR))
				rgDBBINDING[ulBindidx].dwMemOwner = DBMEMOWNER_CLIENTOWNED;
...
BOOL VerifyBindings(HRESULT hrReturned, DBCOUNTITEM cBindings, DBBINDING* rgBindings, void* pData)
{
	...
	if(hrReturned == S_OK)
		return cErrors == 0;
	if(hrReturned == DB_S_ERRORSOCCURRED)
		return (cErrors >= 0 && cErrors < cBindings) || (cStatusBound < cBindings);
	if(hrReturned == DB_E_ERRORSOCCURRED)
...
)
{
	...
	{		
		...
		if(!STATUS_IS_BOUND(*pBinding))
			hr = tempCol.MakeData(pwszData, ulRowNum, eValue, NONULLS);
		else
		...
		else		
		{
			...
			else if(dbBaseType == DBTYPE_VARIANT)
			{
				...
				pvData = DBTYPE2VARIANT(pDBTypeData, vt);
...
)
{
	...
	if ((cPropSets!=1 || rgPropSets==NULL || rgPropSets[0].rgProperties==NULL) ||
		(rgPropSets[0].rgProperties[0].dwPropertyID!=PropertyID))
		goto CLEANUP;

	// Get the value		
	VariantCopy(pVariant, &rgPropSets[0].rgProperties[0].vValue);
	fSuccess = TRUE;
...
HRESULT SetProperty(DBPROPID PropertyID, GUID guidPropertySet, ULONG* pcPropSets, DBPROPSET** prgPropSets, DBTYPE wType, void* pv, DBPROPOPTIONS dwOptions, const DBID& colid)
{
	...
	switch(wType)
	{
		...
		case DBTYPE_I4:
			ASSERT(pv);
			CreateVariant(pVariant, wType, (void*)*(LONG*)pv);
			break;
...
)
{
	...
	switch(StmtKd)
	{
		...
		case eINSERT:
			if (!pCTable)
				goto CLEANUP;
			if (FAILED(hr=pCTable->Insert(0,PRIMARY,FALSE,&sql)))
				goto CLEANUP;
			break;
...
WCHAR * MakeObjectName(WCHAR * pwszModuleName, ULONG ulMaxNameLen)
{
	...
	ulNameLen = wcslen(wszPrefix) + wcslen(wszLocaleUNDERSCORE) + wcslen(pwszPID) + 
				wcslen(wszLocaleUNDERSCORE) +	wcslen(pwszRandNumber);
	...
	wcscpy(pwszObjectName,wszPrefix);
	wcscat(pwszObjectName,wszLocaleUNDERSCORE);
	wcscat(pwszObjectName,pwszPID);
	wcscat(pwszObjectName,wszLocaleUNDERSCORE);
	wcscat(pwszObjectName,pwszRandNumber);
...
BOOL iswcharMappable(WCHAR wch)	
{
	...
	if (!COMPARE(cchBytesWritten, 2))  // one byte for source char, one byte for null terminator
		return FALSE;
...
{
	LONG fComp = CompareStringA(LOCALE_USER_DEFAULT, NORM_IGNOREKANATYPE | NORM_IGNOREWIDTH, 
		psz1, -1, psz2, -1); 
	...
	else
	{
		...
		switch(wType)
		{
			...
			case DBTYPE_VARNUMERIC:
			{
				...
				memcpy(rgbVal, pVarNum->val, cbData);
			...
			case DBTYPE_BYTES:
			{
				...
				// Copy the data
				memcpy(V_ARRAY(pVariant)->pvData, pvSource, cbData);
