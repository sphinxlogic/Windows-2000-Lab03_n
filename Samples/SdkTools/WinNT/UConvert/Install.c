...
*
* When NumberOnly is false, addstrings the registry value associated with
* each codepage - either an nls table filename or a dll filename.
*
* When NumberOnly is true, addstrings all codepage numbers for which there
* is a non-empty registry value.
*
* CRITERION for table being installed:
*  value exists in registry with numeric key and nonblank value
*
\***************************************************************************/
int ListInstalledTables (HWND hwndFill, UINT message, int NumberOnly)
{
  ...
  while (!RegEnumValue (hKey, iSubKey, szKeyname, &cBytesName, NULL, NULL, (LPBYTE)szValue, &cBytesValue)) {
    ...
    // Check there is a value - other entries represent codepages not installed

    if (lstrlen(szValue) > 0) {

      // Check szKeyName is numeric - other keys are not codepages

      fCodePage = TRUE;
      for (i=0; i < lstrlen(szKeyname)  &&  fCodePage; i++) {
        if (   szKeyname[i] < '0'
            || szKeyname[i] > '9') {
            fCodePage = FALSE;
        }
      ...
      if (fCodePage) {
        ...
        if (NumberOnly) {
          ...
          // Send key as codepage value

          SendMessage (hwndFill, message, 0 ,(LPARAM) szKeyname);

        } else {

          // Send value as supporting filename. To avoid duplicate dll names,
          // check first that it is not already present.

          if (SendMessage(
                hwndFill,
                message == LB_ADDSTRING ? LB_FINDSTRINGEXACT : CB_FINDSTRINGEXACT,
                0,
                (LPARAM)szValue) == LB_ERR) {
