    ...
    * Set radio buttons appropriately.
    ...
    case WM_COMMAND:
      switch (LOWORD (wParam)) {
        ...
        *
        * Ensure that when one button is checked, the other gets cleared,
        * however do allow for both to be cleared.
        * Codepage translation DLLS only work with both Precomposed and
        * Composite disabled.
        \******************************************************************/
        case DID_PRECOMPOSED:
          if (SendDlgItemMessage(hwnd, DID_PRECOMPOSED, BM_GETCHECK, 0, 0) == BST_CHECKED) {
            SendDlgItemMessage (hwnd, DID_COMPOSITE,   BM_SETCHECK, FALSE, 0);
          }
        break;

        case DID_COMPOSITE:
          if (SendDlgItemMessage(hwnd, DID_COMPOSITE, BM_GETCHECK, 0, 0) == BST_CHECKED) {
