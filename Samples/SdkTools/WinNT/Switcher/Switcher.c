...
void PaintMainWnd (HWND hWnd)
{
   ...
   //
   ptd = (ThreadData *)GetWindowLongPtr (hWnd, GWLP_USERDATA);
   myThread = ptd->index;
...
void CreateControls (ThreadData *ptd, HWND hWnd)
{
   ...
   //
   oldproc = GetWindowLongPtr (ptd->hWndEdit, GWLP_WNDPROC);
   SetWindowLongPtr (ptd->hWndEdit, GWLP_WNDPROC,  (LONG_PTR)EditProc);
   SetWindowLongPtr (ptd->hWndEdit, GWLP_USERDATA, oldproc);
                ...
                )
{
   ...
   switch (msg)
   {
      ...
      case WM_PAINT:
      {
         HDC hdc;
         PAINTSTRUCT ps;
         index = (int *)GetWindowLongPtr (hWnd, GWLP_USERDATA);
         hdc = BeginPaint (hWnd, &ps);
         //
