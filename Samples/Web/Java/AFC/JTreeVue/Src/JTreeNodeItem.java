...
class JTreeNodeItem extends UIItem implements JTreeConsts
{
	...
	public JTreeNodeItem(JTreeNodeInfo nodeinfo)
	{
		...
		prefix.setBackground(new FxColor(127,170,212));
		prefix.setForeground(new FxColor(0,42,127));
		...
		if ( len >= 9 ) {
			...
			if (str.equalsIgnoreCase("java.awt.")) {
				prefix.setName("AWT");
				prefix.setForeground(new FxColor(127,153,0));
			}
			else if ( len >= 10 ) {
				str = pkg.substring(0,10);
				if ( str.equalsIgnoreCase("com.ms.ui.") ) {
					prefix.setName("UI");
					prefix.setForeground(new FxColor(170,42,0));
				}
				else if ( str.equalsIgnoreCase("com.ms.fx.") ) {
					prefix.setName("Fx");
					prefix.setForeground(new FxColor(212,127,0));
				}
				else if ( len >= 11 ) {
					str = pkg.substring(0,11);
					if ( str.equalsIgnoreCase("com.ms.awt.") ) {
						prefix.setName("awt");
						prefix.setForeground(new FxColor(51,153,0));
					}
					else if ( len >= 12 ) {
