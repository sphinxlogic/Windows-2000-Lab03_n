...
public class TreePanel extends UIPanel implements SDKConsts
{
	...
	public void setSingleSelectMode()
	{
		...
		if ( (comps == null) || (comps.length == 0) ) {
			...
			// This feature will be fixed in the next version of the
			//   AFC Class Libraries
			comp0 = comps[0];
	...
	private void makeTitle()
	{
		...
		UIText txt = new UIText("Eras", UIStatic.LEFT); txt.setForeground(new FxColor(255,0,0));
		pnl.add(txt); pnl.add(new UIText(",", UIStatic.LEFT));
		txt = new UIText("Periods", UIStatic.LEFT); txt.setForeground(new FxColor(0,191,0));
		pnl.add(txt); pnl.add(new UIText(",", UIStatic.LEFT));
		txt = new UIText("Epochs", UIStatic.LEFT); txt.setForeground(new FxColor(255,255,0));
		pnl.add(txt); pnl.add(new UIText(":", UIStatic.LEFT));
		add(pnl, "North");
	}
}
...
class GeoNode extends UIItem implements SDKConsts
{
	...
	public GeoNode(int idx[], int depth)
	{
		...
		case -1:
			...
			duration = GeoROOT.DURATION; setForeground(new FxColor(0,0,255));
			break;
		case 0:
			setImage(SDKImages.get(GeoERA.IMAGE[idx[0]])); setName(GeoERA.NAME[idx[0]]);
			duration = GeoERA.DURATION[idx[0]]; setForeground(new FxColor(255,0,0));
			break;
		case 1:
			...
			setForeground(new FxColor(0,191,0));
			break;
		case 2:
			...
			setForeground(new FxColor(255,255,0));
			break;
		}
