...
class LCInfoPnl extends UIPanel implements LCConsts
{
	...
	public LCInfoPnl(LCParams lp, LCCallbacks lc)
	{
		...
		intrst.setForeground(c_static); freq.setForeground(c_static);
		setParams(lp);
		add(grp, "center");
	}
	...
	public void setParams(LCParams lp)
	{
		...
		switch ( calc ) {
		case BL_PV:
			pv.setForeground(c_changed);
			len.setForeground(c_dynamic);
			pmnt.setForeground(c_dynamic);
			break;
		case BL_LEN:
			pv.setForeground(c_dynamic);
			len.setForeground(c_changed);
			pmnt.setForeground(c_dynamic);
			break;
		case BL_PMNT:
			pv.setForeground(c_dynamic);
			len.setForeground(c_dynamic);
			pmnt.setForeground(c_changed);
			break;
		}
	}
