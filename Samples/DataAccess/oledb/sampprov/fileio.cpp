    ...
    )
{
    ...
    // Open the File
	open( ptstrFileName, ios::in | ios::out | ios::nocreate, filebuf::sh_read || filebuf::sh_write );
    if( !is_open() ) {
		open( ptstrFileName, ios::in | ios::nocreate, filebuf::sh_read );
		if (!is_open())
...
HRESULT CFileIO::GatherColumnInfo()
{
    ...
    for (cCols=1; cCols <= m_cColumns; cCols++, pcolinfo++)
        ...
        pcolinfo->columnid.uName.ulPropid = cCols;
