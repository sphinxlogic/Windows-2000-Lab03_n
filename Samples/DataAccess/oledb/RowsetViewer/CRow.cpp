...
WCHAR* CRow::GetObjectDesc()
{
	...
	if(!m_pwszObjectDesc && m_hSourceRow)
	{
		//0x00000000 = 1 Byte = 2 Asci Chars + "0x" + NULL Terminator
		SAFE_ALLOC(m_pwszObjectDesc, WCHAR, POINTER_DISPLAYSIZE);
		StringFormat(m_pwszObjectDesc, POINTER_DISPLAYSIZE, L"0x%p", (HANDLE)m_hSourceRow);
	}
...
HRESULT CRow::SetupColAccess(BINDCOLS eBindCols)
{
	...
	ASSERT(m_cBindings==0 && !m_rgBindings);
	SAFE_ALLOC(m_rgColAccess, DBCOLUMNACCESS, m_cColInfo);
	SAFE_ALLOC(m_rgBindings, DBBINDING, m_cColInfo);

	//Figure out how big to make the buffer...
	for(i=0; i<m_cColInfo; i++)
	{
		//Offset
		m_rgColAccess[i].cbMaxLen = GetMaxDisplaySize(GetOptions()->GetBindingType(m_rgColumnInfo[i].wType), m_rgColumnInfo[i].wType, m_rgColumnInfo[i].ulColumnSize, dwMaxLength);
	...
	for(i=0; i<m_cColInfo; i++) 
	{
		...
		DBBINDING*		pBinding	= &m_rgBindings[m_cBindings];
