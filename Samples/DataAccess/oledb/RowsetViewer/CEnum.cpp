...
WCHAR* CEnumerator::GetObjectDesc()
{
	if(!m_pwszObjectDesc && m_guidSource != GUID_NULL)
	{
		if(m_guidSource == CLSID_OLEDB_ENUMERATOR)
			m_pwszObjectDesc = wcsDuplicate(L"Root Enumerator");
		else
			m_pwszObjectDesc = GetProgID(m_guidSource);
	}
...
{ 
	if(m_pISourcesRowset)
		m_pCMainWindow->DisplayDialog(IDD_GETSOURCESROWSET, m_pCMainWindow->m_hWnd, CMainWindow::GetSourcesRowsetProc, this);
}
...
HRESULT CEnumerator::ParseDisplayName(WCHAR* pwszParseName, DWORD dwCLSCTX, REFIID riid, IUnknown** ppIUnknown, CBase** ppCSource, DWORD dwConnectOpts, WCHAR* pwszRemoteServer)
{
	...
	if(m_pIParseDisplayName && !(dwConnectOpts & CREATE_USESERVICECOMP))
	{
		...
		hr = m_pIParseDisplayName->ParseDisplayName(pIBindCtx, pwszParseName, &chEaten, &pIMoniker);
		TRACE_METHOD(hr, L"IParseDisplayName::ParseDisplayName(0x%p, \"%s\", &%d, &0x%p)", pIBindCtx, pwszParseName, chEaten, pIMoniker);
		
		if(SUCCEEDED(hr))
		{
			XTEST(hr = BindMoniker(pIMoniker, 0, riid, (void**)ppIUnknown));
			TESTC(TRACE_METHOD(hr, L"BindMoniker(0x%p, 0, %s, &0x%p)", pIBindCtx, GetInterfaceName(riid), ppIUnknown ? *ppIUnknown : NULL));
			pCSource = this;
		}
	}
...
HRESULT CEnumerator::CreateInstance(CAggregate* pCAggregate, REFCLSID clsid, DWORD dwCLSCTX, REFIID riid, IUnknown** ppIUnknown, WCHAR* pwszRemoteServer)
{
	...
	//Create the Trace Provider (if user requested).
	//NOTE: This function will be called before any provider, service, component is created 
	CTrace::CreateTraceProvider();
	CTrace::EnableProvider(pwszProgID);
...
HRESULT CEnumerator::Create(REFCLSID clsid)
{
	...
	m_guidSource = clsid;
	TESTC(hr = CreateInstance(NULL, clsid, CLSCTX_ALL, IID_IParseDisplayName, (IUnknown**)&pIParseDisplayName));

	//Now Delegate to our generic Enumerator method
	TESTC(hr = CreateObject(NULL, IID_IParseDisplayName, pIParseDisplayName));
...
HRESULT CEnumerator::EnumerateInfo(ULONG* pcEnumInfo, ENUMINFO** prgEnumInfo)
{
	...
	TESTC(hr = GetSourcesRowset(NULL, IID_IRowset, 0, NULL, (IUnknown**)&pIRowset));
	TESTC(hr = cRowset.CreateObject(this, IID_IRowset, pIRowset));
	...
	while(TRUE)
	{
		...
		{	
			//Get the Data
			TESTC(hr = cRowset.GetData(IID_IRowset, rghRows[i], hAccessor, (void*)&rgEnumInfo[cEnumInfo]));
			cEnumInfo++;
		}
...
CLEANUP:
	...
	*prgEnumInfo = rgEnumInfo;
	...
	TRACE_RELEASE(pIRowset,			L"IRowset");
	return hr;
}
