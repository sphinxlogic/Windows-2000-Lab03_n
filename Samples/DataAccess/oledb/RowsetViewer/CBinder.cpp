...
WCHAR* CBinder::GetObjectDesc()
{
	if(!m_pwszObjectDesc && m_guidSource != GUID_NULL)
	{
		if(m_guidSource == CLSID_RootBinder)
			m_pwszObjectDesc = wcsDuplicate(L"Root Binder");
		else
...
{ 
	...
	if(m_pIBindResource)
		m_pCMainWindow->DisplayDialog(IDD_BINDRESOURCE, m_pCMainWindow->m_hWnd, CMainWindow::BindResourceProc, this, IDM_IBINDRESOURCE_BIND);
}
...
HRESULT CBinder::CreateBinder(REFCLSID clsid)
{
	...
	XTESTC(hr = m_pCMainWindow->m_pCRootEnumerator->CreateInstance(NULL, clsid, CLSCTX_ALL, IID_IBindResource, (IUnknown**)&pIBindResource));

	//Delegate
	m_guidSource = clsid;
	TESTC(hr = CreateObject(NULL, IID_IBindResource, pIBindResource));
