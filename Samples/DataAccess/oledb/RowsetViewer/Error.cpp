...
#ifdef INTERNAL_ONLY
	//Services
		VALUE_PWCHAR(IID_IFoxService),
		VALUE_PWCHAR(IID_IServiceProperties),
		VALUE_PWCHAR(IID_IService),
		VALUE_PWCHAR(IID_IGetPool),
		VALUE_PWCHAR(IID_IPoolInfo),
		VALUE_PWCHAR(IID_IPoolMngrInfo),
		VALUE_PWCHAR(IID_IPoolNotify),
		VALUE_PWCHAR(IID_IUdlCacheMngr),
#endif //INTERNAL_ONLY
...
HRESULT DisplayErrorInfo(HRESULT hrActual, WCHAR* pwszFile, ULONG ulLine, INT* piResult)
{
	...
	if((hr = GetErrorInfo(0, &pIErrorInfo))==S_OK && pIErrorInfo)
	{
		...
		if(SUCCEEDED(hr = pIErrorInfo->QueryInterface(IID_IErrorRecords, (void**)&pIErrorRecords)))
		{
			//Multiple Errors
			XTESTC(hr = pIErrorRecords->GetRecordCount(&cErrorRecords));
		}
		else
		{
		...
		for(ULONG i=0; i<cErrorRecords; i++)
		{
			...
			if(pIErrorRecords)
			{
				...
				XTEST(hr = pIErrorRecords->GetBasicErrorInfo(i, &ErrorInfo));
