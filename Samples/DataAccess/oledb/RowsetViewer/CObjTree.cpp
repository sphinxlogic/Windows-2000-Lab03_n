...
BOOL CObjTree::OnDblclk(WPARAM fwKeys, REFPOINTS pts) 
{
	...
	if(hItem)
	{
		...
		if(pCBase)
		{
			...
			{
				//If there is no child window assoicated with this object, then
...
//
/////////////////////////////////////////////////////////////////////////////
BOOL CObjTree::RemoveObject(CBase* pCBase, BOOL fRemoveSubObjects, BOOL fObjectExists)
{
	//No-op
	...
	if(RemoveNode(pCBase->m_hTreeItem, fRemoveSubObjects, fObjectExists))
	{
		...
		if(hParentItem)
			GarbageCollectNode(hParentItem, fRemoveSubObjects, fObjectExists);
		return TRUE;
	}
...
//
/////////////////////////////////////////////////////////////////////////////
BOOL CObjTree::GarbageCollectNode(HTREEITEM hTreeItem, BOOL fRemoveSubObjects, BOOL fObjectExists)
{
	//No-op
	if(!m_hWnd || !hTreeItem)
		return TRUE;
	...
	while(hTreeItem)
	{
		//As soon as one of the nodes has an object, its no longer "un-used"
	...
	if(hHighestItem)
	{
		...
		return RemoveNode(hHighestItem, fRemoveSubObjects, fObjectExists);
	}
...
//
/////////////////////////////////////////////////////////////////////////////
BOOL CObjTree::RemoveNode(HTREEITEM hTreeItem, BOOL fRemoveSubObjects, BOOL fObjectExists)
{
	//No-op
	if(!m_hWnd || !hTreeItem)
		return TRUE;
	...
	while(hChildItem)
	{
		...
		CBase* pCChild = (CBase*)GetItemParam(hChildItem);

		//If we are just trying to remove this one item, and it has 
		//any (valid) children what-so-ever we cannot remove this item from the tree.
		if(pCChild)
		{
			//NOTE: We don't recurse into this node, if it doesn't
			//belong to us.  This way all sub objects of another window
			//are left alone...
			if(!fRemoveSubObjects || (pCChild->m_pCMDIChild != pCMDIChild))
				bSafeToRemove = FALSE;
		}
		
		if(bSafeToRemove)
		{
			//Recurse to determine any valid sub nodes...
			if(!RemoveNode(hChildItem, fRemoveSubObjects, fObjectExists))
				bSafeToRemove = FALSE;
		}

		//Do we need to exit if were not interesting in removing subobjects...
		if(!bSafeToRemove && !fRemoveSubObjects)
			break;
	...
	if(bSafeToRemove)
	{
		//Completely safe to delete this node...
	...
	else
	{
		if(!fObjectExists)
		{
			//The best we can do is indicate this object in the tree
			//is no longer usable...
			SetItemText(hTreeItem, L"");
			SetItemParam(hTreeItem, NULL);
		}
	}
