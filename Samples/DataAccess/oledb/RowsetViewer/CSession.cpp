...
HRESULT CSession::GetProviderTypes()
{
	...
	TESTC(hr = GetSchemaRowset(NULL, DBSCHEMA_PROVIDER_TYPES, 0, NULL, IID_IUnknown, 0, NULL, &pIUnknown));
	TESTC(hr = cRowset.CreateObject(this, IID_IUnknown, pIUnknown));
	...
	while(TRUE)
	{
		...
		{	
			//Get the Data
			TESTC(hr = cRowset.GetData(IID_IRowset, rghRows[i], hAccessor, (void*)&m_rgProvTypes[m_cProvTypes]));
			m_cProvTypes++;
		}
