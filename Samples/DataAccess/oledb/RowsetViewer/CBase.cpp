...
CBase::~CBase()
{
	...
	m_pCMainWindow->m_pCMDIObjects->m_pCObjTree->RemoveObject(this, TRUE/*fRemoveSubObjects*/, FALSE/*fObjectExists*/);
}
...
ULONG CBase::ObjectRelease()
{
	...
	//IUnknown::Release
	ulRefCount = CTrace::TraceRelease(pIUnknown, GetObjectName());
...
HRESULT CBase::ReleaseObject(ULONG ulExpectedRefCount)
{
	...
	if(ulExpectedRefCount==0)
	{
		...
		m_pCMainWindow->m_pCMDIObjects->m_pCObjTree->RemoveObject(this, FALSE/*fRemoveSubObjects*/, TRUE/*fObjectExists*/);
		m_guidSource		= GUID_NULL;
		SAFE_FREE(m_pwszObjectDesc);
...
BOOL CBase::IsSameObject(IUnknown* pIUnkObject)
{
	...
	//If we don't already have an object, then we can safely report that
	//this is the same object, (can reuse this object)
	if(!m_pIUnknown)
		return TRUE;

	//Obtain the IUnknown interface of the passed in object
	TESTC(TRACE_QI(pIUnkObject, IID_IUnknown, (IUnknown**)&pIUnknown));
	
	//According to COM, you are allowed to compare IUnknowns to determine
	//if two objects are the same, other interfaces cannot be compared...
	bEqual = (m_pIUnknown == pIUnknown);
...
{ 
	...
	m_pwszObjectDesc = fCopy ? wcsDuplicate(pwszObjectDesc) : pwszObjectDesc;

	//Update the object in the tree...
	if(m_pwszObjectDesc && m_hTreeItem)
...
HRESULT	CAggregate::SetInner(IUnknown* pIUnkInner)
{
	...
	TRACE_RELEASE(m_pIUnkInner, L"IUnknown");
	return TRACE_QI(pIUnkInner, IID_IUnknown, (IUnknown**)&m_pIUnkInner);
}
...
HRESULT	CAggregate::HandleAggregation(REFIID riid, IUnknown** ppIUnknown)
{
	...
	if(riid == IID_IUnknown)
	{
		IUnknown* pIUnkInner = *ppIUnknown;
	...
	else
	{
		...
		//than the inner non-delegating IUnknown, this will cuase our outer controlling object
		...
		ASSERT(!L"Provider Bug: Allowed Aggregation and riid!=IID_IUnknown!");
...
{
	if(!m_pwszObjectDesc && m_pIConnectionPoint)
	{
		IID iid;
		if(SUCCEEDED(GetConnectionInterface(&iid)))
			m_pwszObjectDesc = wcsDuplicate(GetInterfaceName(iid));
	}
...
#ifdef INTERNAL_ONLY
		case eCPoolInfo:
			hr = TRACE_QI(pIUnkObject, IID_IPoolInfo, &pIUnknown);
			break;
#endif //INTERNAL_ONLY
...
#ifdef INTERNAL_ONLY
		else if(SUCCEEDED(hr = TRACE_QI(pIUnkObject, IID_IPoolInfo, &pIUnknown)))
			eSource = eCPoolInfo;
#endif //INTERNAL_ONLY
