...
{	
	...
	for(ULONG iFile=0; iFile<cFiles; iFile++)
	{
		...
		if(StringCompare(wszExt, L".xml", FALSE/*fCaseSensitive*/))
		{
			//Load the XML file
			//TODO: Need to pass in FileName somehow...
			CMSPersistDlg cMSPersistDlg(this, TRUE/*fLoad*/);
			cMSPersistDlg.m_fXML		 = TRUE;
			cMSPersistDlg.SetFileName(wcsDuplicate(wszFileName));
			cMSPersistDlg.DoModal(m_hWnd);
		}
		else if(StringCompare(wszExt, L".adtg", FALSE/*fCaseSensitive*/))
		{
			//Load the ADTG file
			//TODO: Need to pass in FileName somehow...
			CMSPersistDlg cMSPersistDlg(this, TRUE/*fLoad*/);
			cMSPersistDlg.m_fXML		 = FALSE;
			cMSPersistDlg.SetFileName(wcsDuplicate(wszFileName));
			cMSPersistDlg.DoModal(m_hWnd);
		}
		else
...
BOOL CMainWindow::OnUpdateCommand(HMENU hMenu, UINT nID, DWORD* pdwFlags)
{
	...
	switch(nID)
	{
		...
		//----------------------------
#ifdef INTERNAL_ONLY
		ON_COMMAND_UI_ENABLED(IDM_IGETPOOL_GETPOOL,								SOURCE_GETINTERFACE(pCObject, IGetPool))
		ON_COMMAND_UI_ENABLED(IDM_IGETPOOL_GETPOOLSTATE,						SOURCE_GETINTERFACE(pCObject, IGetPool))
#else
		...
		ON_COMMAND_UI_ENABLED(IDM_IROWSET_CURSORENGINE,							SOURCE_GETINTERFACE(pCObject, IUnknown))
		ON_COMMAND_UI_ENABLED(IDM_MSPERSIST_ROWSET_SAVE,						GetActiveWindow(L"MDICHILD") && ((CMDIChild*)GetActiveWindow(L"MDICHILD"))->GetObject(eCRowset)) 
		ON_COMMAND_UI_ENABLED(IDM_MSPERSIST_ROWSET_LOAD,						TRUE) 
		...
		//----------------------------											
#ifdef INTERNAL_ONLY
		ON_COMMAND_UI_ENABLED(IDM_POOLMNGR_ENUMPOOLS,									TRUE) 
		ON_COMMAND_UI_ENABLED(IDM_POOLMNGR_REFRESH,										!m_pCServiceComp->m_listCPoolInfo.IsEmpty()) 
#else
		...
		ON_COMMAND_UI_ENABLED(IDM_ISTREAM_SETSIZE,								SOURCE_GETINTERFACE(pCObject, IStream))
		ON_COMMAND_UI_ENABLED(IDM_ISTREAM_COPYTO,								SOURCE_GETINTERFACE(pCObject, IStream))
		ON_COMMAND_UI_ENABLED(IDM_ISTREAM_COMMIT,								SOURCE_GETINTERFACE(pCObject, IStream))
		ON_COMMAND_UI_ENABLED(IDM_ISTREAM_REVERT,								SOURCE_GETINTERFACE(pCObject, IStream))
		ON_COMMAND_UI_ENABLED(IDM_ISTREAM_LOCKREGION,							SOURCE_GETINTERFACE(pCObject, IStream))
		ON_COMMAND_UI_ENABLED(IDM_ISTREAM_UNLOCKREGION,							SOURCE_GETINTERFACE(pCObject, IStream))
		ON_COMMAND_UI_ENABLED(IDM_ISTREAM_STAT,									SOURCE_GETINTERFACE(pCObject, IStream))
		...
		ON_COMMAND_UI_ENABLED(IDM_ILOCKBYTES_FLUSH,								SOURCE_GETINTERFACE(pCObject, ILockBytes))
		ON_COMMAND_UI_ENABLED(IDM_ILOCKBYTES_SETSIZE,							SOURCE_GETINTERFACE(pCObject, ILockBytes))
		ON_COMMAND_UI_ENABLED(IDM_ILOCKBYTES_LOCKREGION,						SOURCE_GETINTERFACE(pCObject, ILockBytes))
		ON_COMMAND_UI_ENABLED(IDM_ILOCKBYTES_UNLOCKREGION,						SOURCE_GETINTERFACE(pCObject, ILockBytes))
		ON_COMMAND_UI_ENABLED(IDM_ILOCKBYTES_STAT,								SOURCE_GETINTERFACE(pCObject, ILockBytes))
		//----------------------------
		//----------------------------
		...
		//----------------------------
#ifdef INTERNAL_ONLY
		ON_COMMAND_UI_ENABLED(IDM_IPOOLINFO_REFRESH,							SOURCE_GETINTERFACE(pCObject, IPoolInfo))
		ON_COMMAND_UI_ENABLED(IDM_IPOOLINFO_GETCOUNT,							SOURCE_GETINTERFACE(pCObject, IPoolInfo))
		ON_COMMAND_UI_ENABLED(IDM_IPOOLINFO_GETPOOLID,							SOURCE_GETINTERFACE(pCObject, IPoolInfo))
		ON_COMMAND_UI_ENABLED(IDM_IPOOLINFO_GETPROTOTYPE,						SOURCE_GETINTERFACE(pCObject, IPoolInfo)) 
		ON_COMMAND_UI_ENABLED(IDM_IPOOLINFO_SETPOOLTIMEOUT,						SOURCE_GETINTERFACE(pCObject, IPoolInfo))  
		ON_COMMAND_UI_ENABLED(IDM_IPOOLINFO_GETPOOLTIMEOUT,						SOURCE_GETINTERFACE(pCObject, IPoolInfo)) 
		ON_COMMAND_UI_ENABLED(IDM_IPOOLINFO_SETRETRYTIMEOUT,					SOURCE_GETINTERFACE(pCObject, IPoolInfo))  
		ON_COMMAND_UI_ENABLED(IDM_IPOOLINFO_GETRETRYTIMEOUT,					SOURCE_GETINTERFACE(pCObject, IPoolInfo))
#else
...
#ifdef INTERNAL_ONLY
	CPoolInfo*			pCPoolInfo				= SOURCE_GETOBJECT(pCObject, CPoolInfo);
#endif //INTERNAL_ONLY
...
#ifdef INTERNAL_ONLY
		ON_COMMAND(IDM_POOLMNGR_ENUMPOOLS,						OnEnumPools()) 
		ON_COMMAND(IDM_POOLMNGR_REFRESH,						OnEnumPools()) 

		ON_COMMAND(IDM_IPOOLINFO_REFRESH,						pCPoolInfo ? pCPoolInfo->DisplayObject() : 0) 
		ON_COMMAND(IDM_IPOOLINFO_GETPOOLTIMEOUT,				pCPoolInfo ? pCPoolInfo->GetPoolTimeOut(&dwValue) : 0) 
		ON_COMMAND(IDM_IPOOLINFO_GETRETRYTIMEOUT,				pCPoolInfo ? pCPoolInfo->GetRetryTimeOut(&dwValue) : 0) 
		
		case IDM_IPOOLINFO_GETCOUNT:
			if(pCPoolInfo)
			{
				CWaitCursor waitCursor;
		
				DWORD dwInUse, dwIdle;
				pCPoolInfo->GetCount(&dwInUse, &dwIdle);
			}
			return TRUE;

		case IDM_IPOOLINFO_GETPOOLID:
			if(pCPoolInfo)
			{
				CWaitCursor waitCursor;

				WCHAR* pwszPoolID = NULL;
				pCPoolInfo->GetPoolID(&pwszPoolID);
				SAFE_FREE(pwszPoolID);
			}
			return TRUE;

		case IDM_IPOOLINFO_GETPROTOTYPE:
			if(pCPoolInfo)
			{
				//Default output pointer to NULL, common case is your only interested in the string
				CInterfaceDlg interfaceDlg(IDD_INTERFACE, L"IGetPool::GetPrototype", IID_IDBInitialize); 
				interfaceDlg.m_fOutput = FALSE;
				...
				if(interfaceDlg.DoModal(GetFocus()) == IDOK)
				{
					...
					WCHAR* pwszPrototype = NULL;
					if(SUCCEEDED(pCPoolInfo->GetPrototype(&pwszPrototype, interfaceDlg.GetSelInterface(), interfaceDlg.ppUnknown())))
						HandleObjectType(pCPoolInfo, interfaceDlg.pUnknown(), interfaceDlg.GetSelInterface(), eCDataSource, 0, NULL, CREATE_NEWWINDOW);
					
					SAFE_FREE(pwszPrototype);
				}
			}
			return TRUE;

		case IDM_IPOOLINFO_SETPOOLTIMEOUT:
			if(pCPoolInfo)
			{
				//Display the dialog
				CEditValueDlg editValueDlg(IDD_SETVALUE, L"IGetPool::SetPoolTimeOut");
				editValueDlg.SetValue(L"0");
				editValueDlg.SetReqType(DBTYPE_UI4);
				...
				if(editValueDlg.DoModal(GetFocus()) == IDOK)
				{
					...
					if(SUCCEEDED(editValueDlg.GetVariant(DBTYPE_UI4, &cVariant)))
						pCPoolInfo->SetPoolTimeOut(V_UI4(&cVariant));
				}
			}
			return TRUE;

		case IDM_IPOOLINFO_SETRETRYTIMEOUT:
			if(pCPoolInfo)
			{
				//Display the dialog
				CEditValueDlg editValueDlg(IDD_SETVALUE, L"IGetPool::SetRetryTimeOut");
				editValueDlg.SetValue(L"0");
				editValueDlg.SetReqType(DBTYPE_UI4);
				...
				if(editValueDlg.DoModal(GetFocus()) == IDOK)
				{
					...
					if(SUCCEEDED(editValueDlg.GetVariant(DBTYPE_UI4, &cVariant)))
						pCPoolInfo->SetRetryTimeOut(V_UI4(&cVariant));
				}
			}
			return TRUE;

		case IDM_MSPERSIST_ROWSET_SAVE:
		case IDM_MSPERSIST_ROWSET_LOAD:
		{
			static CMSPersistDlg s_CMSPersistDlg(this, TRUE/*fLoad*/);
			s_CMSPersistDlg.m_fLoad			= (iID != IDM_MSPERSIST_ROWSET_SAVE);
			s_CMSPersistDlg.DoModal(GetFocus());
			break;
		}
...
//
/////////////////////////////////////////////////////////////////////////////
void CMainWindow::OnReleaseAll(CBase* pCBase)
{
	if(pCBase && pCBase->m_pIUnknown)
	{
		CWaitCursor waitCursor;
...
#ifdef INTERNAL_ONLY
/////////////////////////////////////////////////////////////////////////////
// CMainWindow::OnEnumPools
//
/////////////////////////////////////////////////////////////////////////////
void CMainWindow::OnEnumPools() 
{
	CWaitCursor waitCursor;

	m_pCServiceComp->EnumPools();
	UpdateControls();
}
		...
		case WM_COMMAND:
		{
			...
			switch (GET_WM_COMMAND_ID(wParam, lParam))
			{
				case IDOK:
				{
					...
					//Record
					if(!GetEditBoxValue(s_CComboRecord.m_hWnd, 0, LONG_MAX, (LONG*)&ulRecord, FALSE/*fAllowEmpty*/))
						return FALSE;
					...
					if(lcid == CB_ERR)
					{
						if(!GetEditBoxValue(s_CComboLCID.m_hWnd, 0, LONG_MAX, (LONG*)&lcid, FALSE/*fAllowEmpty*/))
							return FALSE;
					}
						...
						for(ULONG i=0; i<pCRootEnum->m_cEnumInfo; i++)
						{
							...
							if(pEnumInfo->eType ==  DBSOURCETYPE_ENUMERATOR)
							{
								...
								if(wcsstr(pwszInitString, wszSubName) || 
									(wcsstr(pwszInitString, L"Provider")==NULL && wcsstr(pwszInitString, L"provider")==NULL && wcscmp(pEnumInfo->wszName, L"MSDASQL Enumerator")==0) )
								{
						...
						if(cSession.m_pIDBSchemaRowset)
						{
							...
							while(TRUE)
							{
								...
								{	
									...
									TESTC(hr = cRowset.GetData(IID_IRowset, rghRows[i], hAccessor, wszBuffer));
...
CBase* CMainWindow::HandleObjectType(CBase* pCSource, IUnknown* pIUnknown, REFIID riid, SOURCE eSource, ULONG cPropSets, DBPROPSET* rgPropSets, DWORD dwFlags)
{
	...
	if(dwFlags & CREATE_DETERMINE_TYPE)
	{
		...
		//Since many OLE DB methods can return different objects depneding upon interface or properties
	...
	if(pCMDIChild && (dwFlags & CREATE_NEWWINDOW_IFEXISTS))
	{
		...
		//in the base class CDataAccess for simplity (no special casing).  Becuase of this you can't
	...
	if(pCSource && (dwFlags & CREATE_FINDWINDOW))
	{
		...
		if(pCParent)
		{
			...
			else
			{
				...
				if(pCParent->m_pCMDIChild)
					MDIActivate(pCParent->m_pCMDIChild->GetWnd());
				dwFlags &= ~CREATE_NEWWINDOW;

				//Since we found the object, we need to set the parent this the found
				//objects parent...
	...
	switch(eSource)
	{
		...
		case eCRowset:
		{
			...
			//NOTE: We delete the previous class, since we may be creating a different class
			SAFE_RELEASE(pCMDIChild->m_pCDataAccess);
			CRowset* pCRowset = new CRowset(NULL, pCMDIChild);
			pCObject = pCMDIChild->m_pCDataAccess = pCRowset;

#ifdef INTERNAL_ONLY
			//Do we need to invoke a service?
			if(dwFlags & CREATE_CURSORENGINE)
			{
				//Invoke the Cursor Engine on top of our existing rowset...
				TESTC(hr = pCRowset->InvokeCursorEngine(pIUnknown, cPropSets, rgPropSets, IID_IRowset, &pIUnknown));

				//Indicate this rowset is from CE
				pCRowset->SetObjectDesc(L"CE");
			}
#endif //INTERNAL_ONLY
...
#ifdef INTERNAL_ONLY
			//Need to release CE's rowset, now that its in our object...
			if(dwFlags & CREATE_CURSORENGINE)
				TRACE_RELEASE(pIUnknown, L"IUnknown");
#endif //INTERNAL_ONLY
		...
		case eCRow:
		{
			...
			//NOTE: We delete the previous class, since we may be creating a different class
			SAFE_RELEASE(pCMDIChild->m_pCDataAccess);
			CRow* pCRow = new CRow(NULL, pCMDIChild);
		...
		case eCDataset:
		{
			...
			//NOTE: We delete the previous class, since we may be creating a different class
			SAFE_RELEASE(pCMDIChild->m_pCDataAccess);
			CDataset* pCDataset = new CDataset(NULL, pCMDIChild);
		...
		{	
			//Stand Alone Object (no window required)
			CPoolInfo* pCPoolInfo = new CPoolInfo(this);
			pCObject = pCPoolInfo;

			//Update our object
			TESTC(hr = pCPoolInfo->CreateObject(pCSource, riid, pIUnknown));
			break;	   
		}
#endif //INTERNAL_ONLY
