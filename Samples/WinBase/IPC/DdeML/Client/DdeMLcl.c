...
INT    nCmdShow)
{
    ...
    while (GetMessage (&msg, NULL, 0, 0)){
        ((HOOKPROC)*lpMsgFilterProc)(MSGF_DDEMGR, 0, (LPARAM)&msg);
    }
 ...
 *                                                                          *
 ****************************************************************************/
LRESULT  APIENTRY MDIChildWndProc( HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
    MYCONVINFO *pmci;
    RECT rc;
    ...
    case WM_CREATE:
        ...
        if (pmci != NULL) {
            ...
            SetWindowLongPtr(hwnd, 0, (DWORD_PTR)pmci);
             ...
             */
            if (!pmci->fList)
                DdeSetUserHandle(pmci->hConv, QID_SYNC, hwnd);
        }
        goto CallDCP;
        break;
 ...
 *                                                                          *
 ****************************************************************************/
VOID NEAR PASCAL InitializeMenu ( HANDLE hmenu )
{
    ...
    if (fAny = (IsWindow(hwndActive) &&
            (pmci = (MYCONVINFO *)GetWindowLongPtr(hwndActive, 0)))) {
...
VOID NEAR PASCAL CloseAllChildren ()
{
    ...
    while ( hwndT = GetWindow (hwndMDIClient, GW_CHILD)){
        ...
        SendMessage(hwndMDIClient, WM_MDIDESTROY, (DWORD_PTR)hwndT, 0L);
    }
...

{
    ...
    switch (id){
        ...
        case IDM_DISCONNECT:
            if (hwndActive) {
                SendMessage(hwndMDIClient, WM_MDIDESTROY, (DWORD_PTR)hwndActive, 0L);
            }
            break;

        case IDM_TRANSACT:
            if (DoDialog(MAKEINTRESOURCE(IDD_TRANSACT), (DLGPROC)TransactDlgProc,
                    (DWORD_PTR)(LPTSTR)pmci->hConv, FALSE))
...
BOOL fList)
{
    ...
    // mci.y =
    // mci.ci =
    mcs.lParam = (DWORD_PTR)&mci;

    hwnd = (HWND)SendMessage (hwndMDIClient, WM_MDICREATE, 0,
             (DWORD_PTR)(LPMDICREATESTRUCT)&mcs);

    MyFree((PTSTR)mcs.szTitle);
