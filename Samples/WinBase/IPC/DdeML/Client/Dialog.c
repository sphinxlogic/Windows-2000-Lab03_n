    ...
    case WM_INITDIALOG:
        ...
        if (IsWindow(hwndActive)) {
            ...
            if (IsWindow(hwndXaction)) {
                pxact = (XACT *)GetWindowLongPtr(hwndXaction, GWL_USER);
        ...
        case IDBN_OPTIONS:
            {
                ...
                if (LOWORD(wParam) == IDBN_OPTIONS) {
                    DoDialog(MAKEINTRESOURCE(IDD_ADVISEOPTS),
                            (DLGPROC)AdvOptsDlgProc, (LONG_PTR)pxact, TRUE);
                    return 0;
                }
                ...
                if (pxact->wType == XTYP_POKE || pxact->wType == XTYP_EXECUTE) {
                    if (!DoDialog(MAKEINTRESOURCE(IDD_TEXTENTRY),
                            (DLGPROC)TextEntryDlgProc, (DWORD_PTR)pxact,
                            TRUE))
                        return 0;
                }
...
LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        for (i = 0; i < (INT)cOwned; i++) {
            if (aOwned[i].hszItem == pxact->hszItem &&
                ...
                SendDlgItemMessage(hwnd, IDLB_HANDLES, LB_ADDSTRING, 0, (LONG_PTR)szT);
            }
        }
    ...
    case WM_COMMAND:
        ...
        case IDBN_VIEW:     // view selected handle
            ...
            if (itm != LB_ERR) {
                SendDlgItemMessage(hwnd, IDLB_HANDLES, LB_GETTEXT, itm, (LONG_PTR)szT);
