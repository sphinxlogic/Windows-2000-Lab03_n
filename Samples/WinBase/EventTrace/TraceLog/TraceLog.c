	...
	char **argv;
{
    ...
    while (--argc > 0) {
        ...
        if (**targv == '-' || **targv == '/') {  // argument found
            ...
            else if (!_tcsicmp(targv[0], _T("-nf"))) {
                if (argc > 1) {
                    pLoggerInfo->LogFileMode |= EVENT_TRACE_FILE_MODE_NEWFILE;
                    pLoggerInfo->MaximumFileSize = _ttoi(targv[1]);
                    ++targv; --argc;
                    _tprintf(_T("Switch to new logfile size limit : %d\n"),
                        pLoggerInfo->MaximumFileSize);
                }
            }
	...
	case ACTION_LIST :
	{
        ...
        case ACTION_QUERY:
            ...
            if (Status == ERROR_SUCCESS && Action == ACTION_UPDATE) {
                pLoggerInfo->Wnode.Flags = WNODE_FLAG_TRACED_GUID; 
                _tcscpy(LogFileName, tstrLogFileName);
    ...
    )
{
    ...
    else if (LoggerInfo->LogFileMode & EVENT_TRACE_FILE_MODE_NEWFILE) {
        _tprintf(_T("Switch to new file\n"));
    }
