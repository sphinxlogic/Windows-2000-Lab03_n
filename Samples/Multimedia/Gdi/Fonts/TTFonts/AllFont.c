...
HDC       hdc;
  ...
  SendMessage (hwndDisplayFonts, WMU_CREATE, (DWORD_PTR)hdc, 0);
  ...
  hdc = GetPrinterDC ();
  if (hdc == NULL) {
    SetWindowLongPtr (hwndPrinterFonts, GWLU_PARFONTS, (LONG_PTR) NULL);
    DestroyWindow (hwndPrinterFonts);
    SendMessage (hwndMain, WM_COMMAND, IDU_NOPRINTER, 0);
  } else {
    SendMessage (hwndPrinterFonts, WMU_CREATE, (DWORD_PTR)hdc, 0);
    DeleteDC (hdc);
...
int nFaces;
  ...
  parFonts = (PARFONTS) GetWindowLongPtr (hwnd, GWLU_PARFONTS);
  nFaces   = (int)      GetWindowLongPtr (hwnd, GWLU_NFACES);
  ...
  switch (message) {
    ...
    *  Set the parFonts pointer into the extra bytes for use by other messages.
    ...
    case WMU_CREATE: {
      ...
      SetWindowLongPtr (hwnd, GWLU_PARFONTS, (LONG_PTR) parFonts);
      SetWindowLongPtr (hwnd, GWLU_NFACES,   nFaces);
    ...
    * Hittest based on the mouse position.  When finished, hide the window.
    ...
    case WM_LBUTTONDOWN: {
      ...
      SendMessage (hwndDlgLF, WMU_DEMOTOLF, 0,(LONG_PTR) &(parFonts[i].lf[j]));

      flyWinWin(hwndMain,  hwnd, hwndDlgTM, 40);
      SendMessage (hwndDlgTM, WMU_DEMOTOTM, 0,(LONG_PTR) &(parFonts[i].tm[j]));
      SetFocus (hwndMain);
