...
UINT UiMakeFilePatchesCabinetsTransformsAndStuffIntoPatchPackage ( MSIHANDLE hdbInput, LPTSTR szPatchPath, LPTSTR szTempFolder, LPTSTR szTempFName )
{
	...
	TCHAR rgchTempMspPath[_MAX_PATH];
...
static UINT IdsCreatePatchFilesMSTsCabinetsForFamily ( MSIHANDLE hview, MSIHANDLE hrec, LPARAM lp1, LPARAM lp2 )
{
	...
	int iDiskId = MsiRecordGetInteger(hrec, 2);
	Assert(iDiskId > 1);

	int iSeqNumStart = MsiRecordGetInteger(hrec, 3);
...
static UINT UiMakeFilePatches ( LPTSTR szFamily, int iDiskId, int* piSequenceNumCur )
{
	...
	TCHAR rgchWhere[_MAX_PATH];
	EvalAssert( IDS_OKAY == IdsMsiGetTableString(g_hdbInput, TEXT("Properties"), TEXT("Name"), TEXT("Value"), TEXT("IncludeWholeFilesOnly"), rgchWhere, _MAX_PATH) );
...
static UINT IdsMakeFilePatchesForUpgradedImage ( MSIHANDLE hview, MSIHANDLE hrec, LPARAM lp1, LPARAM lp2 )
{
	...
	TCHAR rgchSrcRoot[_MAX_PATH+_MAX_PATH];
	DWORD dwcch = _MAX_PATH+_MAX_PATH;
...
static TCHAR g_rgchFNameToCompare[_MAX_PATH] = TEXT("");
static TCHAR g_rgchSubFolderToCompare[_MAX_PATH] = TEXT("");
...
static UINT UiGenerateOnePatchFile    ( MSIHANDLE hdbInput, LPTSTR szFTK, LPTSTR szSrcPath, int iFileSeqNum, LPTSTR szTempFolder, LPTSTR szTempFName );
...
static UINT IdsHandleOneFilePatch ( MSIHANDLE hview, MSIHANDLE hrec, LPARAM lp1, LPARAM lp2 )
{
	...
	TCHAR rgchComponent[_MAX_PATH+_MAX_PATH];
	dwcch = _MAX_PATH;
...
//				g_szFamily, rgchFTK);
	...
	if (iSeqNumNew == MSI_NULL_INTEGER || iSeqNumNew <= 0)
		...
		if (!fNeedWholeFile)
			...
			UINT ui = UiGenerateOnePatchFile(hdbInput, rgchFTK,
						g_szUpgradedSrcFolder, iSeqNumNew,
...
static UINT IdsDoTargetFileComparesIfFileExists ( MSIHANDLE hview, MSIHANDLE hrec, LPARAM lp1, LPARAM lp2 )
{
	...
	TCHAR rgchPath[_MAX_PATH+_MAX_PATH];
...
static UINT IdsMarkIfFileMatches ( MSIHANDLE hview, MSIHANDLE hrec, LPARAM lp1, LPARAM lp2 )
{
	...
	if (!FDoFilesDiffer(szUpgradedImageSrcFile, rgch))
		{
		TCHAR rgchNewFTK[_MAX_PATH];
		lstrcpy(rgchNewFTK, TEXT("<matches> "));
		dwcch = _MAX_PATH - lstrlen(rgchNewFTK);
...
static void GetFileVersion ( LPTSTR szFile, DWORD* pdwHi, DWORD* pdwLow );
...
static BOOL FDoFilesDiffer ( LPTSTR szFile1, LPTSTR szFile2 )
{
	...
	if (dw1Low != dw2Low || dw1Hi != dw2Hi)
		...
		while (!fDone)
			{
			BYTE rgb1[256], rgb2[256];
			EvalAssert( ReadFile(hf1, (LPVOID)rgb1, 256, &dw1Low, NULL) );
			EvalAssert( ReadFile(hf2, (LPVOID)rgb2, 256, &dw2Low, NULL) );
			if (dw1Low != dw2Low)
				fDone = fRet = fTrue;
			else if (dw1Low == 0)
				fDone = fTrue;
...
static void GetFileVersion ( LPTSTR szFile, DWORD* pdwHi, DWORD* pdwLow )
{
	...
	*pdwHi = *pdwLow = 0L;
	...
	Assert(2048 > sizeof(VS_FIXEDFILEINFO));
	BYTE rgbBuf[2048];
	if (GetFileVersionInfo(szFile, 0L, 2048, (LPVOID)rgbBuf))
		{
...
static UINT UiGenerateOnePatchFile ( MSIHANDLE hdbInput, LPTSTR szFTK, LPTSTR szSrcPath, int iFileSeqNum, LPTSTR szTempFolder, LPTSTR szTempFName )
{
	...
	Assert(lstrlen(szTempFolder) < _MAX_PATH);
	TCHAR rgchPatchFile[_MAX_PATH];
...
static LPCSTR SzaGetUpgradedSymPaths ( LPTSTR szFTK )
{
	...
	TCHAR rgchSymPaths[_MAX_PATH*10];
	*rgchSymPaths = TEXT('\0');
...
static UINT IdsFillSymPaths ( MSIHANDLE hview, MSIHANDLE hrec, LPARAM lp1, LPARAM lp2 )
{
	...
	EvalAssert( FCatSymPath(g_hdbInput, rgchTarget, szFTK, szSymPaths, _MAX_PATH*10, fFalse) );
...
#define rgchSymPaths  rgchSrcRoot // reuse buffer
	*rgchSymPaths = TEXT('\0');
	EvalAssert( FCatSymPath(g_hdbInput, rgchTarget, szFTK, rgchSymPaths, _MAX_PATH+_MAX_PATH, fTrue) );
...
#define rgchIgnoreOffsets  rgchSrcRoot // reuse buffer
	if (cIgnoreRanges > 0)
		...
		uiRet = IdsMsiGetTableStringWhere(g_hdbInput, TEXT("`TargetFiles_OptionalData`"),
					TEXT("`IgnoreOffsets`"), rgchWhere, rgchIgnoreOffsets, _MAX_PATH);
		Assert(uiRet == MSI_OKAY);
		Assert(!FEmptySz(rgchIgnoreOffsets));

		TCHAR rgchIgnoreLengths[_MAX_PATH];
		uiRet = IdsMsiGetTableStringWhere(g_hdbInput, TEXT("`TargetFiles_OptionalData`"),
					TEXT("`IgnoreLengths`"), rgchWhere, rgchIgnoreLengths, _MAX_PATH);
		Assert(uiRet == MSI_OKAY);
...
#define rgchRetainOldOffsets  rgchSrcRoot // reuse buffer
	uiRet = IdsMsiGetTableStringWhere(g_hdbInput, TEXT("`TargetFiles_OptionalData`"),
				TEXT("`RetainOffsets`"), rgchWhere, rgchRetainOldOffsets, _MAX_PATH);
	Assert(uiRet == MSI_OKAY);
	if (!FEmptySz(rgchRetainOldOffsets))
		...
		TCHAR rgchRetainNewOffsets[_MAX_PATH];
		uiRet = IdsMsiGetTableStringWhere(g_hdbInput, TEXT("`FamilyFileRanges`"),
					TEXT("`RetainOffsets`"), rgchWhere, rgchRetainNewOffsets, _MAX_PATH);
		Assert(uiRet == MSI_OKAY);
		Assert(!FEmptySz(rgchRetainNewOffsets));

		TCHAR rgchRetainLengths[_MAX_PATH];
		uiRet = IdsMsiGetTableStringWhere(g_hdbInput, TEXT("`FamilyFileRanges`"),
					TEXT("`RetainLengths`"), rgchWhere, rgchRetainLengths, _MAX_PATH);
		Assert(uiRet == MSI_OKAY);
...
static UINT IdsResolveSrcFilePathFtk ( MSIHANDLE hdb, LPTSTR szBuf, LPTSTR szFtk, BOOL fLfn, LPTSTR szFullSubFolder )
{
	...
	UINT  uiRet = IdsMsiGetTableString(hdb, TEXT("`File`"), TEXT("`File`"),
					TEXT("`Component_`"), szFtk, rgchComponent, _MAX_PATH);
	Assert(uiRet == IDS_OKAY);
	Assert(!FEmptySz(rgchComponent));

	TCHAR rgchFName[_MAX_PATH];
	uiRet = IdsMsiGetTableString(hdb, TEXT("`File`"), TEXT("`File`"),
					TEXT("`FileName`"), szFtk, rgchFName, _MAX_PATH);
	Assert(uiRet == IDS_OKAY);
	Assert(!FEmptySz(rgchFName));
...
#undef rgchUpgradedImage
		...
		MSIHANDLE hrecNew = MsiCreateRecord(5);
		Assert(hrecNew != NULL);
		EvalAssert( MSI_OKAY == MsiRecordSetString( hrecNew, 1, szFTK) );
		EvalAssert( MSI_OKAY == MsiRecordSetInteger(hrecNew, 2, iSeqNumNew) );
		EvalAssert( MSI_OKAY == MsiRecordSetInteger(hrecNew, 3, iPatchSize) );
		EvalAssert( MSI_OKAY == MsiRecordSetInteger(hrecNew, 4, iAttributes) );
		EvalAssert( MSI_OKAY == MsiRecordSetStream( hrecNew, 5, g_szTempFolder) );

		EvalAssert( IDS_OKAY == IdsMsiSetTableRecord(hdbCopyOfUpgradedImage, TEXT("`Patch`"),
						TEXT("`File_`,`Sequence`,`PatchSize`,`Attributes`,`Header`"),
						TEXT("`File_`"), szFTK, hrecNew) );
		}
	else
		{
...
#define PATCH_OPTION_DEFAULT  (PATCH_OPTION_USE_BEST + PATCH_OPTION_FAIL_IF_BIGGER)
...
static ULONG UlGetApiPatchFlags ( MSIHANDLE hdbInput, BOOL fOption )
{
	...
	EvalAssert( IDS_OKAY == IdsMsiGetPcwPropertyString(hdbInput,
					(fOption) ? TEXT("ApiPatchingOptionFlags") : TEXT("ApiPatchingSymbolFlags"),
					rgch, 64) );
	...
	Assert(FValidHexValue(rgch));
	Assert(!fOption || FValidApiPatchOptionFlags(UlFromHexSz(rgch)));
...
static UINT UiCreateCabinet ( LPTSTR szFamily, int iSequenceNumStart, int iSequenceNumCur )
{
	...
	while (uiRet == ERROR_SUCCESS && iSequenceNumStart < iSequenceNumCur)
		...
		TCHAR rgchDdfLine[_MAX_PATH+_MAX_PATH];
...
static BOOL FRunMakeCab ( LPTSTR szFamily, LPTSTR szTempFolder )
{
	...
	TCHAR rgchCmdLine[_MAX_PATH+20];
...
static UINT UiStuffCabsAndMstsIntoPackage ( MSIHANDLE hdbInput, LPTSTR szPatchPath )
{
	...
	UINT ids = MsiOpenDatabase(szPatchPath,
					MSIDBOPEN_CREATE + MSIDBOPEN_PATCHFILE, &hdbPackage);
	if (ids != MSI_OKAY)
		return (ERROR_PCW_CANT_CREATE_PATCH_FILE);
...
LEarlyReturn:
	...
	if (MSI_OKAY != MsiSummaryInfoSetProperty(hSummaryInfo, PID_WORDCOUNT, VT_I4, 1, NULL, NULL))
		return (fFalse);
