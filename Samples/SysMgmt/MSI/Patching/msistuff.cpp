...
UINT UiOpenInputPcp ( LPTSTR szPcpPath, LPTSTR szTempFldrBuf, LPTSTR szTempFName, MSIHANDLE* phdbInput )
{
	...
	*phdbInput = NULL;
	if (FEmptySz(rgchPcpPath) || !FFileExist(rgchPcpPath))
		return (ERROR_PCW_PCP_DOESNT_EXIST);
	...
	if (!FValidSummInfoVersion(szTempFldrBuf, iSchemaMin, iSchemaMax))
		return (ERROR_PCW_PCP_BAD_FORMAT);
...
UINT CchMsiTableString ( MSIHANDLE hdb, LPTSTR szTable, LPTSTR szPKey, LPTSTR szFieldName, LPTSTR szPKeyValue )
{
	...
	TCHAR rgchWhere[_MAX_PATH];
	wsprintf(rgchWhere, TEXT("%s = '%s'"), szPKey, szPKeyValue);
	Assert(lstrlen(rgchWhere) < sizeof(rgchWhere)/sizeof(TCHAR));

	return (CchMsiTableStringWhere(hdb, szTable, szFieldName, rgchWhere));
}
...
UINT CchMsiTableStringWhere ( MSIHANDLE hdb, LPTSTR szTable, LPTSTR szFieldName, LPTSTR szWhere )
{
	...
	TCHAR rgchQuery[_MAX_PATH];
	wsprintf(rgchQuery, TEXT("SELECT %s FROM %s WHERE %s"), szFieldName, szTable, szWhere);
	...
	TCHAR rgchBuf[16];  // doc says we can pass "" but this seems safer
	DWORD dwcch;
	dwcch = 0;
...
UINT IdsMsiGetTableString ( MSIHANDLE hdb, LPTSTR szTable, LPTSTR szPKey, LPTSTR szFieldName, LPTSTR szPKeyValue, LPTSTR szBuf, UINT cch )
{
	...
	return (IdsMsiGetTableStringWhere(hdb, szTable, szFieldName, rgchWhere, szBuf, cch));
}
...
UINT IdsMsiGetTableInteger ( MSIHANDLE hdb, LPTSTR szTable, LPTSTR szPKey, LPTSTR szFieldName, LPTSTR szPKeyValue, int * pi )
{
	...
	return (IdsMsiGetTableIntegerWhere(hdb, szTable, szFieldName, rgchWhere, pi));
}
...
UINT IdsMsiSetTableRecordWhere ( MSIHANDLE hdb, LPTSTR szTable, LPTSTR szFields, LPTSTR szWhere, MSIHANDLE hrec )
{
	...
	TCHAR rgchQuery[_MAX_PATH];
	wsprintf(rgchQuery, TEXT("SELECT %s FROM %s WHERE %s"), szFields, szTable, szWhere);
	Assert(lstrlen(rgchQuery) < sizeof(rgchQuery)/sizeof(TCHAR));
...
UINT IdsMsiSetTableRecord ( MSIHANDLE hdb, LPTSTR szTable, LPTSTR szFields, LPTSTR szPrimaryField, LPTSTR szKey, MSIHANDLE hrec )
{
	...
	TCHAR rgchWhere[_MAX_PATH];
	wsprintf(rgchWhere, TEXT("%s = '%s'"), szPrimaryField, szKey);
	Assert(lstrlen(rgchWhere) < sizeof(rgchWhere)/sizeof(TCHAR));

	return (IdsMsiSetTableRecordWhere(hdb, szTable, szFields, rgchWhere, hrec));
}
...
UINT IdsMsiUpdateTableRecordSz ( MSIHANDLE hdb, LPTSTR szTable, LPTSTR szField, LPTSTR szValue, LPTSTR szPKeyField, LPTSTR szPKeyValue )
{
	...
	TCHAR rgchQuery[_MAX_PATH];
	wsprintf(rgchQuery, TEXT("UPDATE `%s` SET `%s` = '%s' WHERE `%s` = '%s'"), szTable, szField, szValue, szPKeyField, szPKeyValue);
	Assert(lstrlen(rgchQuery) < sizeof(rgchQuery)/sizeof(TCHAR));
...
UINT IdsMsiUpdateTableRecordInt ( MSIHANDLE hdb, LPTSTR szTable, LPTSTR szField, int iValue, LPTSTR szPKeyField, LPTSTR szPKeyValue )
{
	...
	TCHAR rgchQuery[_MAX_PATH];
	wsprintf(rgchQuery, TEXT("UPDATE `%s` SET `%s` = %d WHERE `%s` = '%s'"), szTable, szField, iValue, szPKeyField, szPKeyValue);
	Assert(lstrlen(rgchQuery) < sizeof(rgchQuery)/sizeof(TCHAR));
...
UINT IdsMsiUpdateTableRecordIntPkeyInt ( MSIHANDLE hdb, LPTSTR szTable, LPTSTR szField, int iValue, LPTSTR szPKeyField, int iPKeyValue )
{
	...
	TCHAR rgchQuery[_MAX_PATH];
	wsprintf(rgchQuery, TEXT("UPDATE `%s` SET `%s` = %d WHERE `%s` = %d"), szTable, szField, iValue, szPKeyField, iPKeyValue);
	Assert(lstrlen(rgchQuery) < sizeof(rgchQuery)/sizeof(TCHAR));
...
UINT IdsMsiDeleteTableRecords ( MSIHANDLE hdb, LPTSTR szTable, LPTSTR szField, LPTSTR szKey )
{
	...
	TCHAR rgchWhere[_MAX_PATH];
	wsprintf(rgchWhere, TEXT("%s = '%s'"), szField, szKey);

	return (IdsMsiDeleteTableRecordsWhere(hdb, szTable, rgchWhere));
}
...
UINT IdsMsiDeleteTableRecordsWhere ( MSIHANDLE hdb, LPTSTR szTable, LPTSTR szWhere )
{
	...
	TCHAR rgchQuery[_MAX_PATH];
	wsprintf(rgchQuery, TEXT("DELETE FROM %s WHERE %s"), szTable, szWhere);
			...
			LPTSTR szWhere, PIEMTPROC pIemtProc, LPARAM lp1, LPARAM lp2 )
{
	...
	TCHAR rgchQuery[_MAX_PATH + 128];
	if (szWhere != szNull)
		wsprintf(rgchQuery, TEXT("SELECT %s FROM %s WHERE %s"), szFields, szTable, szWhere);
	else
		wsprintf(rgchQuery, TEXT("SELECT %s FROM %s"), szFields, szTable);
	Assert(lstrlen(rgchQuery) < sizeof(rgchQuery)/sizeof(TCHAR));
...
UINT IdsMsiExistTableRecords ( MSIHANDLE hdb, LPTSTR szTable, LPTSTR szField, LPTSTR szValue, PBOOL pf )
{
	...
	TCHAR rgchWhere[_MAX_PATH];
	wsprintf(rgchWhere, TEXT("WHERE %s = '%s'"), szField, szValue);
	Assert(lstrlen(rgchWhere) < sizeof(rgchWhere)/sizeof(TCHAR));

	return (IdsMsiExistTableRecordsWhere(hdb, szTable, szField, rgchWhere, pf));
}
...
//	Assert(!FEmptySz(szWhere));
	...
	TCHAR rgchQuery[_MAX_PATH+_MAX_PATH];
	Assert(lstrlen(szField) + lstrlen(szTable) + lstrlen(szWhere) + lstrlen(TEXT("SELECT  FROM  ")) < sizeof(rgchQuery)/sizeof(TCHAR));
	wsprintf(rgchQuery, TEXT("SELECT %s FROM %s "), szField, szTable);
	lstrcat(rgchQuery, szWhere);
	Assert(lstrlen(szField) + lstrlen(szTable) + lstrlen(szWhere) + lstrlen(TEXT("SELECT  FROM  ")) == lstrlen(rgchQuery));
	Assert(lstrlen(rgchQuery) < sizeof(rgchQuery)/sizeof(TCHAR));
	Assert(lstrlen(rgchQuery) < 256); // performance much better if this is true
...
static BOOL FValidateInputMsiSchema ( MSIHANDLE hdb )
{
	...
	if (FTableExists(hdb, TEXT("UpgradedFiles_OptionalData"), fFalse))
		...
		if (FIntColumnExists(hdb, szTable, TEXT("IgnoreErrors"), fFalse))
			{
#ifdef DEBUG
			MessageBox(hwndNull, TEXT("The IgnoreErrors column has been dropped from the UpgradedFiles_OptionalData table; ignoring column in current PCP.  Use 'AllowIgnoreOnPatchError' column."), TEXT("Debug Schema Changed Msg"), MB_OK);
#endif /* DEBUG */
...
BOOL FTableExists ( MSIHANDLE hdb, LPTSTR szTable, BOOL fMsg )
{
	...
	if (fMsg)
		...
		MessageBox(hwndNull, rgch, szMsgBoxTitle, MB_OK | MB_ICONEXCLAMATION);
		}
#endif
...
static BOOL FSzColumnExists ( MSIHANDLE hdb, LPTSTR szTable, LPTSTR szColumn, BOOL fMsg )
{
	...
	TCHAR rgchTable[_MAX_PATH];
	wsprintf(rgchTable, TEXT("`%s`"), szTable);

	TCHAR rgchColumn[_MAX_PATH];
...
static UINT IdsCopyRecordsFromFileDataToUFOD ( MSIHANDLE hview, MSIHANDLE hrec, LPARAM lp1, LPARAM lp2 )
{
	...
	TCHAR rgchSymbolPaths[_MAX_PATH+_MAX_PATH];
	dwcch = _MAX_PATH+_MAX_PATH;
	...
	TCHAR rgchQuery[_MAX_PATH+_MAX_PATH+_MAX_PATH];
...
UINT UiValidateInputRecords ( MSIHANDLE hdb, LPTSTR szPcpPath, LPTSTR szPatchPath, LPTSTR szTempFolder, LPTSTR szTempFName )
{
	...
	if (FNoUpgradedImages(hdb))
		return (ERROR_PCW_NO_UPGRADED_IMAGES_TO_PATCH);
	
	if (!FCheckForProductCodeMismatches(hdb))
		return (ERROR_PCW_MISMATCHED_PRODUCT_CODES);
	if (!FCheckForProductVersionMismatches(hdb))
		return (ERROR_PCW_MISMATCHED_PRODUCT_VERSIONS);
...
#define FValidFamilyName(sz)   FValidName(sz,8)
#define FValidImageName(sz)    FValidName(sz,13)
...
static UINT IdsValidateFamilyRecords ( MSIHANDLE hview, MSIHANDLE hrec, LPARAM lp1, LPARAM lp2 )
{
	...
	TCHAR rgchFamily[_MAX_PATH];
	DWORD dwcch = _MAX_PATH;
...
static UINT IdsValidateFamilyRangeRecords ( MSIHANDLE hview, MSIHANDLE hrec, LPARAM lp1, LPARAM lp2 )
{
	...
	TCHAR rgchRetainOffsets[_MAX_PATH];
	dwcch = _MAX_PATH;
	...
	TCHAR rgchRetainLengths[_MAX_PATH];
	dwcch = _MAX_PATH;
...
static UINT IdsValidateUpgradedRecords ( MSIHANDLE hview, MSIHANDLE hrec, LPARAM lp1, LPARAM lp2 )
{
	...
	TCHAR rgchData[_MAX_PATH+_MAX_PATH];
	dwcch = _MAX_PATH+_MAX_PATH;
	...
	EvalAssert( MSI_OKAY == IdsMsiGetPropertyString(hdb, TEXT("ProductCode"), rgchData, _MAX_PATH) );
	...
	EvalAssert( MSI_OKAY == IdsMsiGetPropertyString(hdb, TEXT("ProductVersion"), rgchData, _MAX_PATH) );
	...
	EvalAssert( MSI_OKAY == IdsMsiGetPropertyString(hdb, TEXT("UpgradeCode"), rgchData, _MAX_PATH) );
...
//		}
	...
	else if (!FValidHexValue(rgchData) || !FValidProdValValue(rgchData))
		return (UiLogError(ERROR_PCW_TARGET_BAD_PROD_VALIDATE, rgchTarget, szNull));
...
static UINT IdsCheckFileExists ( MSIHANDLE hview, MSIHANDLE hrec, LPARAM lp1, LPARAM lp2 )
{
	...
	*g_szFName = TEXT('\0');
	...
	TCHAR rgchComponent[_MAX_PATH];
	TCHAR rgchFName[_MAX_PATH];
...
UINT IdsResolveSrcFilePathSzs ( MSIHANDLE hdb, LPTSTR szBuf, LPTSTR szComponent, LPTSTR szFName, BOOL fLfn, LPTSTR szFullSubFolder )
{
	...
	*szBuf = TEXT('\0');
	...
	TCHAR rgchDir[_MAX_PATH];
	UINT  ids = IdsMsiGetTableString(hdb, TEXT("`Component`"), TEXT("`Component`"), TEXT("`Directory_`"), szComponent, rgchDir, _MAX_PATH);
	Assert(ids == MSI_OKAY);
...
static UINT IdsResolveSrcDir ( MSIHANDLE hdb, LPTSTR szDir, LPTSTR szBuf, BOOL fLfn, LPTSTR szFullSubFolder )
{
	...
	TCHAR rgchData[_MAX_PATH];
	UINT  ids = IdsMsiGetTableString(hdb, TEXT("`Directory`"), TEXT("`Directory`"), TEXT("`Directory_Parent`"), szDir, rgchData, _MAX_PATH);
	Assert(ids == MSI_OKAY);
	...
	ids = IdsMsiGetTableString(hdb, TEXT("`Directory`"), TEXT("`Directory`"), TEXT("`DefaultDir`"), szDir, rgchData, _MAX_PATH);
	Assert(ids == MSI_OKAY);
...
static UINT IdsValidateUFileDataRecords ( MSIHANDLE hview, MSIHANDLE hrec, LPARAM lp1, LPARAM lp2 )
{
	...
	TCHAR rgchFTK[_MAX_PATH];
	uiRet = IdsMsiGetTableString(hdbInput, TEXT("`UpgradedImages`"),
				TEXT("`Upgraded`"), TEXT("`Family`"), rgchUpgraded, rgchFTK, _MAX_PATH);
	Assert(uiRet == IDS_OKAY);
	if (FEmptySz(rgchFTK))
		return (UiLogError(ERROR_PCW_UFILEDATA_BAD_UPGRADED_FIELD, rgchUpgraded, szNull));
...
static UINT IdsValidateTFileDataRecords ( MSIHANDLE hview, MSIHANDLE hrec, LPARAM lp1, LPARAM lp2 )
{
	...
	uiRet = IdsMsiGetTableString(hdbInput, TEXT("`TargetImages`"),
				TEXT("`Target`"), TEXT("`Family`"), rgchTarget, rgchFamily, _MAX_PATH);
	Assert(uiRet == IDS_OKAY);
...
static BOOL FCheckForProductCodeMismatches ( MSIHANDLE hdb )
{
	...
	TCHAR rgch[_MAX_PATH];
	EvalAssert( IDS_OKAY == IdsMsiGetPcwPropertyString(hdb, TEXT("AllowProductCodeMismatches"), rgch, _MAX_PATH) );
...
static BOOL FCheckForProductVersionMismatches ( MSIHANDLE hdb )
{
	...
	TCHAR rgch[_MAX_PATH];
	EvalAssert( IDS_OKAY == IdsMsiGetPcwPropertyString(hdb, TEXT("AllowProductVersionMajorMismatches"), rgch, _MAX_PATH) );
...
static int ICompareVersions ( LPTSTR szUpgradedPC, LPTSTR szTargetPC )
{
	...
	*szTargetNext++ = TEXT('\0');
	...
	if (lstrlen(szUpgradedPC) < lstrlen(szTargetPC))
		return (-1);
	if (lstrlen(szUpgradedPC) > lstrlen(szTargetPC))
		return (1);

	int i = lstrcmpi(szUpgradedPC, szTargetPC);
	if (i == 0)
...
static BOOL FValidPatchTableFormat  ( MSIHANDLE hdb, BOOL fPatch );
...
/* ********************************************************************** */
static UINT UiCreatePatchingTables ( MSIHANDLE hdb, LPSTR szFamily, LPTSTR szTempFolder, LPTSTR szTempFName )
{
	Assert(hdb != NULL);
	...
	*szTempFName = TEXT('\0');
	...
	Assert(FValidPatchTableFormat(hdb, fFalse));
...
static void UpdatePatchPackageTable ( MSIHANDLE hdb, MSIHANDLE hdbInput, LPTSTR szFamily )
{
	...
	TCHAR rgch[_MAX_PATH];
	EvalAssert( IDS_OKAY == IdsMsiGetPcwPropertyString(hdbInput, TEXT("PatchGUID"), rgch, _MAX_PATH) );
	Assert(!FEmptySz(rgch));
	CharUpper(rgch);
...
static void UpdateMediaTable ( MSIHANDLE hdb, MSIHANDLE hdbInput, LPTSTR szFamily )
{
	...
	EvalAssert( IDS_OKAY == IdsMsiGetTableString(hdbInput, TEXT("`ImageFamilies`"),
					TEXT("`Family`"), TEXT("`MediaSrcPropName`"), szFamily, rgch, _MAX_PATH) );
	Assert(!FEmptySz(rgch));
	EvalAssert( MsiRecordSetString(hrec,  4, rgch) == MSI_OKAY );

	EvalAssert( IDS_OKAY == IdsMsiGetTableString(hdbInput, TEXT("`ImageFamilies`"),
					TEXT("`Family`"), TEXT("`DiskPrompt`"), szFamily, rgch, _MAX_PATH) );
	EvalAssert( MsiRecordSetString(hrec,  5, rgch) == MSI_OKAY );

	EvalAssert( IDS_OKAY == IdsMsiGetTableString(hdbInput, TEXT("`ImageFamilies`"),
					TEXT("`Family`"), TEXT("`VolumeLabel`"), szFamily, rgch, _MAX_PATH) );
...
static UINT UiCreatePatchingTableExportFile ( MSIHANDLE hdbInput, BOOL fPatch, LPTSTR szTempFolder, LPTSTR szTempFName );
...
static UINT UiValidateAndLogPCWProperties ( MSIHANDLE hdbInput, LPTSTR szPcpPath, LPTSTR szPatchPath, LPTSTR szTempFolder, LPTSTR szTempFName )
{
	...
	*szTempFName = TEXT('\0');
	...
	TCHAR rgchLog[_MAX_PATH+64];

	TCHAR rgch[_MAX_PATH*5];
	UINT ids;

	UpdateStatusMsg(0, szNull, TEXT("PatchGUID"));
	ids = IdsMsiGetPcwPropertyString(hdbInput, TEXT("PatchGUID"), rgch, _MAX_PATH);
	Assert(ids == IDS_OKAY);
	CharUpper(rgch);
	if (FEmptySz(rgch))
		{
		EvalAssert( FWriteLogFile(TEXT("ERROR: Patch GUID is missing.\r\n")) );
		return (ERROR_PCW_MISSING_PATCH_GUID);
		}
	if (!FValidGUID(rgch, fFalse, fFalse, fFalse))
		{
		EvalAssert( FSprintfToLog(TEXT("ERROR: Bad Patch GUID = '%s'"), rgch, szEmpty, szEmpty, szEmpty) );
		return (ERROR_PCW_BAD_PATCH_GUID);
		}
	...
	ids = IdsMsiGetPcwPropertyString(hdbInput, TEXT("ListOfPatchGUIDsToReplace"), rgch, _MAX_PATH);
	Assert(ids == IDS_OKAY);
	CharUpper(rgch);
	if (FEmptySz(rgch))
		...
		{
		wsprintf(rgchLog, TEXT("ERROR: Bad ListOfPatchGUIDsToReplace = '%s'\r\n"), rgch);
		EvalAssert( FWriteLogFile(rgchLog) );
		return (ERROR_PCW_BAD_GUIDS_TO_REPLACE);
		}
	...
	ids = IdsMsiGetPcwPropertyString(hdbInput, TEXT("ListOfTargetProductCodes"), rgch, _MAX_PATH*5);
	Assert(ids == IDS_OKAY);
	CharUpper(rgch);
	if (FEmptySz(rgch))
		lstrcpy(rgch, TEXT("*"));
	if (!FValidGUID(rgch, fTrue, fTrue, fTrue))
		{
		wsprintf(rgchLog, TEXT("ERROR: Bad ListOfTargetProductCodes = '%s'\r\n"), rgch);
		EvalAssert( FWriteLogFile(rgchLog) );
		return (ERROR_PCW_BAD_TARGET_PRODUCT_CODE_LIST);
		}
	...
	UpdateStatusMsg(0, szNull, TEXT("ApiPatchingOptionFlags"));
	ids = IdsMsiGetPcwPropertyString(hdbInput, TEXT("ApiPatchingOptionFlags"), rgch, _MAX_PATH);
	Assert(ids == IDS_OKAY);
	if (FEmptySz(rgch))
		lstrcpy(rgch, TEXT("<blank>"));
	else if (!FValidHexValue(rgch) || !FValidApiPatchOptionFlags(UlFromHexSz(rgch)))
		{
		wsprintf(rgchLog, TEXT("ERROR: Bad ApiPatchingOptionFlags = '%s'\r\n"), rgch);
		EvalAssert( FWriteLogFile(rgchLog) );
		return (ERROR_PCW_BAD_API_PATCHING_OPTION_FLAGS);
		}
	wsprintf(rgchLog, TEXT("ApiPatchingOptionFlags             = '%s'\r\n"), rgch);
	EvalAssert( FWriteLogFile(rgchLog) );

	UpdateStatusMsg(0, szNull, TEXT("ApiPatchingSymbolFlags"));
	ids = IdsMsiGetPcwPropertyString(hdbInput, TEXT("ApiPatchingSymbolFlags"), rgch, _MAX_PATH);
	Assert(ids == IDS_OKAY);
	if (FEmptySz(rgch))
		lstrcpy(rgch, TEXT("<blank>"));
	else if (!FValidHexValue(rgch) || !FValidApiPatchSymbolFlags(UlFromHexSz(rgch)))
		{
		wsprintf(rgchLog, TEXT("ERROR: Bad ApiPatchingSymbolFlags = '%s'\r\n"), rgch);
		EvalAssert( FWriteLogFile(rgchLog) );
		return (ERROR_PCW_BAD_API_PATCHING_SYMBOL_FLAGS);
		}
	...
	ids = UiCreatePatchingTableExportFile(hdbInput, fTrue, szTempFolder, szTempFName);
	if (ids != IDS_OKAY)
		return (ids);
	ids = UiCreatePatchingTableExportFile(hdbInput, fFalse, szTempFolder, szTempFName);
	if (ids != IDS_OKAY)
		return (ids);
	*szTempFName = TEXT('\0');
	EvalAssert( FWriteLogFile(TEXT("\r\n")) );
...
{
	if (ul & ~(PATCH_OPTION_VALID_FLAGS))
		return (fFalse);

	if (ul & PATCH_OPTION_RESERVED1)
		return (fFalse);
...
static void LogSzProp ( MSIHANDLE hdbInput, LPTSTR szProp, LPTSTR szBuf, LPTSTR szBufLog )
{
	...
	EvalAssert( IDS_OKAY == IdsMsiGetPcwPropertyString(hdbInput, szProp, szBuf, _MAX_PATH) );
	if (FEmptySz(szBuf))
...
static UINT UiCreatePatchingTableExportFile ( MSIHANDLE hdbInput, BOOL fPatch, LPTSTR szTempFolder, LPTSTR szTempFName )
{
	...
	LPTSTR szCreateSqlCmd = TEXT("CREATE TABLE `Patch` ( `File_` CHAR(72) NOT NULL, `Sequence` INTEGER NOT NULL, `PatchSize` LONG NOT NULL, `Attributes` INTEGER NOT NULL, `Header` OBJECT NOT NULL PRIMARY KEY `File_`, `Sequence` )");
	...
	if (FTableExists(hdbInput, szTable, fFalse))
		{
		if (FValidPatchTableFormat(hdbInput, fPatch))
			{
	...
	TCHAR rgchMsiPath[_MAX_PATH];
	EvalAssert( IDS_OKAY == IdsMsiGetPcwPropertyString(hdbInput, TEXT("MsiFileToUseToCreatePatchTables"), rgchMsiPath, _MAX_PATH) );
	if (!FEmptySz(rgchMsiPath))
		{
		MSIHANDLE hdb;
		hdb = NULL;
		TCHAR rgchFullPath[_MAX_PATH];
...
LHandleRelativePath:
		...
		if (hdb != NULL)
			...
			if (FTableExists(hdb, szTable, fFalse))
				{
				if (!FValidPatchTableFormat(hdb, fPatch))
					{
					...
					{
					LPTSTR szFNameTmp = (fPatch) ? TEXT("export1.idt") : TEXT("export2.idt");
					...
					*szTempFName = TEXT('\0');
					...
					if (!FFileExist(szTempFolder))
						...
						*szTempFName = TEXT('\0');
						...
						Assert(FTableExists(hdbInput, szTable, fFalse));
						Assert(FValidPatchTableFormat(hdbInput, fPatch));
						EvalAssert( FExecSqlCmd(hdbInput, szEmptySqlCmd) );
		...
		else if (fPatch && !fUsingDefaultMsi)
			{
...
#define rgchPropSqlCmd rgchMsiPath // reuse buffer
	...
	szPcpSqlPropName = (fPatch) ? TEXT("SqlCmdToCreatePatchTable") : TEXT("SqlCmdToCreatePatchPackageTable");
	EvalAssert( IDS_OKAY == IdsMsiGetPcwPropertyString(hdbInput, szPcpSqlPropName, rgchPropSqlCmd, _MAX_PATH) );
	if (!FEmptySz(rgchPropSqlCmd))
		...
		else if (!FValidPatchTableFormat(hdbInput, fPatch))
			{
...
static BOOL FValidPatchTableFormat ( MSIHANDLE hdb, BOOL fPatch )
{
	...
	LPTSTR szTable = (fPatch) ? TEXT("Patch") : TEXT("PatchPackage");
...
MSIHANDLE HdbReopenMsi ( MSIHANDLE hdbInput, LPTSTR szImage, BOOL fUpgradedImage, BOOL fTargetUpgradedCopy )
{
	...
	TCHAR rgch[_MAX_PATH];
	EvalAssert( IDS_OKAY == IdsMsiGetTableString(hdbInput, szTable, szPKey, szMsiPath, szImage, rgch, _MAX_PATH) );
	Assert(!FEmptySz(rgch));
