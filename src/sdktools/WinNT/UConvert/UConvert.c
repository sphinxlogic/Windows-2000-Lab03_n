...
DWORD gMBFlags = 0;    // Default to none, since this is required for dll implemented translations
DWORD gWCFlags = 0;
        ...
        case MID_CONVERTNOW: {
          ...
          if (gTypeSource == TYPEUNICODE) {
            ...
            nBytesNeeded = WideCharToMultiByte(giDestinationCodePage, gWCFlags,
                             ...
                             NULL, 0,
                             gWCFlags & WC_DEFAULTCHAR ? glpDefaultChar    : 0,
                             gWCFlags & WC_DEFAULTCHAR ? &gUsedDefaultChar : NULL);

            /* Note that passing in the address of a default character causes  */
            /* WCTOMB to fail when using dll based translations, so we only    */
            /* pass that address when the user has requested default character */
            /* operation through the UI.                                       */
