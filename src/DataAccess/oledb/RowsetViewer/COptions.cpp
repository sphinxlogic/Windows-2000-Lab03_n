...
BOOL COptionsCommand::OnInitDialog() 
{
	...
	CheckDlgButton(IDB_RELEASEROWSET,			BST2STATE(pCOptionsSheet->m_dwCommandOpts & COMMAND_RELEASEROWSET));
...
BOOL COptionsCommand::OnApply() 
{
	...
	//Display ReleaseRowset
	ENABLE_BIT(pCOptionsSheet->m_dwCommandOpts, COMMAND_RELEASEROWSET, IsDlgButtonChecked(IDB_RELEASEROWSET));
...
BOOL COptionsAccessor::OnApply() 
{
	...
	//MaxLength
	if(!GetEditBoxValue(GetDlgItem(IDE_MAXLENGTH), 0, LONG_MAX-1, (LONG*)&pCOptionsSheet->m_dwMaxLength))
	{
...
BOOL COptionsRowset::OnApply() 
{
	...
	{
		if(!GetEditBoxValue(GetDlgItem(IDE_ROWSTOFETCH), 0, LONG_MAX-1, (LONG*)&pCOptionsSheet->m_cRowsToFetch))
		{
...
BOOL COptionsError::OnApply() 
{
	...
	if(IsDlgButtonChecked(IDB_BREAKID))
	{
		...
		if(!GetEditBoxValue(GetDlgItem(IDE_BREAKID), 0, LONG_MAX, (LONG*)&g_dwBreakID, FALSE))
			return FALSE;
	}
