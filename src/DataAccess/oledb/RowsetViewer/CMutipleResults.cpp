...
//
/////////////////////////////////////////////////////////////////
HRESULT CMultipleResults::GetResult(CAggregate* pCAggregate, REFIID riid, DBROWCOUNT* pcRowsAffected, IUnknown** ppIUnknown)
{
	HRESULT hr = E_FAIL;
	...
	XTEST(hr = m_pIMultipleResults->GetResult(pCAggregate, 0, riid, pcRowsAffected, ppIUnknown));
	TESTC(TRACE_METHOD(hr, L"IMultipleResults::GetResult(0x%p, 0, %s, &%ld, &0x%p)", pCAggregate, GetInterfaceName(riid), pcRowsAffected ? *pcRowsAffected : NULL, ppIUnknown ? *ppIUnknown : NULL));
