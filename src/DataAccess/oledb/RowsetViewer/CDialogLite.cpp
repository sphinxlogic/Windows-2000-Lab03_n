...
BOOL CWndLite::CreateIndirect(HWND hWndParent, UINT nID)
{
	...
	m_hWnd			= ::GetDlgItem(m_hWndParent, m_nID);
...
{
	//This method does not handle multiple levels...
...
CMDIChildLite* CMDIFrameLite::GetActiveWindow(WCHAR* pwszClassName)
{
	...
	if(hWndChild)
	{
		...
		if(pCMDIChildLite)
		{
			if(pwszClassName)
			{
				ASSERT(pCMDIChildLite->m_pwszClassName);
				if(wcscmp(pwszClassName, pCMDIChildLite->m_pwszClassName)==0)
...
CMDIChildLite* CMDIFrameLite::FindWindow(WCHAR* pwszClassName)
{
	...
	while(hWndChild)
	{
		...
		{
			//Return the first child of the specified type...
			if(wcscmp(pCMDIChildLite->m_pwszClassName, pwszClassName)==0)
				return pCMDIChildLite;
		}
...
//
/////////////////////////////////////////////////////////////////////
BOOL CEditBoxLite::Save(WCHAR* pwszFileName)
{
	CFileLite cFile;
...
//
/////////////////////////////////////////////////////////////////////
void CEditBoxLite::ReplaceAll(WCHAR* pwszString, BOOL bReplaceAll, BOOL bHighlight)
{
	if(pwszString)
	{
		INDEX iStartPos = 0;
...
//
/////////////////////////////////////////////////////////////////////
void CRichEditLite::ReplaceSel(WCHAR* pwszString, BOOL bCanUndo, DWORD dwMask, DWORD dwColor)
{
...
//
/////////////////////////////////////////////////////////////////////
INDEX CListViewLite::HitTest(REFPOINTS pts, UINT* pFlags)
{
...
//
/////////////////////////////////////////////////////////////////////
HTREEITEM CTreeViewLite::HitTest(REFPOINTS pts, UINT* pFlags)
{
...
//
/////////////////////////////////////////////////////////////////////
HRESULT CFileLite::Open(WCHAR* pwszFileName, DWORD dwDesiredAccess, DWORD dwShareMode, DWORD dwCreationDisposition, BOOL fDisplayDialog)
{
	ASSERT(m_hFile == NULL);
	...
	if(m_hFile==INVALID_HANDLE_VALUE)
	{
		if(fDisplayDialog)
		{
			wMessageBox(GetFocus(), MB_TASKMODAL | MB_ICONERROR | MB_OK, wsz_ERROR, L"Unable to open file \"%s\", make sure File Name and Path are valid", pwszFileName);
		}
		m_hFile = NULL;
	}
...
//
/////////////////////////////////////////////////////////////////////
DWORD CFileLite::Seek(DWORD dwOffset, DWORD dwOrigin)
{
	if(m_hFile)
		return SetFilePointer(m_hFile, dwOffset, NULL, dwOrigin);
...
HRESULT CFileLite::Read(ULONG cbBytes, WCHAR* pwszText, ULONG* pcbRead)
{
	...
	//NOTE:  Currently All input/output to the file, will be in ANSI.
	//We eventually will need to deal with reading and writing unicode files,
	//but for backward compatbility we currently read and write ANSI...
	static CHAR szBuffer[MAX_QUERY_LEN] = {0};
	
	//Delegate
	hr = Read(cbBytes/2, szBuffer, &cbRead);
	if(SUCCEEDED(hr))
	{
		ConvertToWCHAR(szBuffer, pwszText, MAX_QUERY_LEN);
		cbRead = cbRead*2;
	}
...
HRESULT CFileLite::Read(ULONG cbBytes, CHAR* pszText, ULONG* pcbRead)
{
	...
	if(m_hFile && ReadFile(m_hFile, pszText, cbBytes, &cbRead, NULL))
	{
		if(cbBytes == cbRead)
			hr = S_OK;
		else
...
//
/////////////////////////////////////////////////////////////////////
HRESULT CFileLite::Write(WCHAR* pwszText, ULONG* pcbWritten)
{
	ULONG cbWritten = 0;
	HRESULT hr = S_OK;
	...
	if(m_hFile && pwszText)
	{
		...
		//Delegate
		hr = Write(szBuffer, &cbWritten);
		if(SUCCEEDED(hr))
		{
			cbWritten = cbWritten*2;
		}
	}
...
//
/////////////////////////////////////////////////////////////////////
HRESULT CFileLite::Write(CHAR* pszText, ULONG* pcbWritten)
{
	ULONG cbWritten = 0;
	HRESULT hr = S_OK;
	...
	{
		//Output to the file
		if(WriteFile(m_hFile, pszText, strlen(pszText)*sizeof(CHAR), &cbWritten, NULL))
			hr = S_OK;
		else
			hr = E_FAIL;
	}
...
//
/////////////////////////////////////////////////////////////////////
HRESULT CFileLite::WriteQuoted(WCHAR* pwszText)
{
	if(pwszText == NULL)
	{
...
//
/////////////////////////////////////////////////////////////////////
HRESULT CFileLite::WriteFmt(WCHAR* pwszFmt, ...)
{
	va_list		marker;
	WCHAR		wszBuffer[MAX_QUERY_LEN];
