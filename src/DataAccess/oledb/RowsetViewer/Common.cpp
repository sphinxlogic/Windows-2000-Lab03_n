...
/////////////////////////////////////////////////////////////////////////////
#include "Headers.h"
#include "msdatrc.h"	//Trace Provider
#include <errno.h>		//ERANGE

//TODO: Had to remove IDataInit, IDBPrompt from msdarc.h, as they are already in the lib...

/////////////////////////////////////////////////////////////////////////////
// Externs
//
//////////////////////////////////////////////////////////////////////////////
extern IDataConvert* g_pIDataConvert = NULL;

#ifdef INTERNAL_ONLY 
	const IID IID_IPoolInfo=		{ /* e231fcd8-be3d-11d2-91a2-00c04fb6ee9d */
		0xe231fcd8, 0xbe3d, 0x11d2, {0x91, 0xa2, 0x00, 0xc0, 0x4f, 0xb6, 0xee, 0x9d}};
	
	const IID IID_IPoolMngrInfo =	{ /* 07e78e52-be3e-11d2-91a2-00c04fb6ee9d */
		0x07e78e52,	0xbe3e,	0x11d2, {0x91, 0xa2, 0x00, 0xc0, 0x4f, 0xb6, 0xee, 0x9d}};
	
	const IID IID_IUdlCacheMngr =	{ /* 7ee07400-12ce-11d3-b5b8-00a0c9081c1d */
		0x7ee07400, 0x12ce, 0x11d3, { 0xb5, 0xb8, 0x00, 0xa0, 0xc9, 0x08, 0x1c, 0x1d}};
	
	const IID IID_IGetPool =		{ /* 10c2eaa8-be3e-11d2-91a2-00c04fb6ee9d */
		0x10c2eaa8, 0xbe3e,	0x11d2,	{0x91, 0xa2, 0x00, 0xc0, 0x4f, 0xb6, 0xee, 0x9d}};

	const IID IID_IPoolNotify =		{ /* 69e59018-f1ce-11d2-91aa-00c04fb6ee9d */
		0x69e59018, 0xf1ce,	0x11d2,	{0x91, 0xaa, 0x00, 0xc0, 0x4f, 0xb6, 0xee, 0x9d}};
#endif //INTERNAL_ONLY
...
//
/////////////////////////////////////////////////////////////////////////////
BOOL StringCompare(WCHAR* pwsz1, WCHAR* pwsz2, BOOL fCaseSensitive)
{
	//Safe guard against NULL...
	if(pwsz1 && pwsz2)
		return fCaseSensitive ? wcscmp(pwsz1, pwsz2)==0 : wcsicmp(pwsz1, pwsz2)==0;
...
//
/////////////////////////////////////////////////////////////////////////////
INT ConvertToMBCS(WCHAR* pwsz, CHAR* psz, INT cchDst)
{ 
...
//
/////////////////////////////////////////////////////////////////////////////
INT ConvertToWCHAR(CHAR* psz, WCHAR* pwsz, INT cchDst)
{
	//Delegate
...
//
/////////////////////////////////////////////////////////////////////////////
INT ConvertToWCHAR(CHAR* psz, INT cchSrc, WCHAR* pwsz, INT cchDst)
{
	//NOTE: The user can call this function with the Destination as NULL to indicate
	//just provide the length of the required conversion
...
HRESULT PostProcessString(WCHAR* pwsz, DBLENGTH ulMaxSize, DWORD dwFlags)
{
	...
	LONG lValue = wcstol(pwsz, &pwszStop, 0);
	if(lValue == LONG_MAX || lValue == 0 || pwszStop==NULL || pwszStop[0]!=wEOL)
		return E_FAIL;
...
)
{
	...
	if((dwConvFlags & CONV_MSDADC_ONLY) || FAILED(hr))
	{
		...
		if(g_pIDataConvert == NULL)
			TESTC(hr = CoCreateInstance(CLSID_OLEDB_CONVERSIONLIBRARY, NULL, CLSCTX_ALL, IID_IDataConvert, (void**)&g_pIDataConvert));
	...
	switch(vt)
	{
		...
		{	
			...
			if(pwszStop==NULL || pwszStop[0]!=wEOL || 
				((lValue==LONG_MAX || lValue==LONG_MIN) && errno==ERANGE))
			{
				hr = E_FAIL;
				goto CLEANUP;
			}

			V_I4(pVariant) = lValue;
			break;
		}
...
//
///////////////////////////////////////////////////////////////
HRESULT FreeBindingData(DBCOUNTITEM cBindings, DBBINDING* rgBindings, void* pData, BOOL fSetData)
{
	...
	for(ULONG i=0; i<cBindings; i++)
	{
		...
		switch(dbStatus)
		{
			...
			default:
				...
				//hacing special logic all over, we AddRef the Stream an extra time before
...
DBLENGTH GetMaxDisplaySize(DBTYPE wBindingType, DBTYPE wBackendType, DBLENGTH ulColumnSize, DBLENGTH ulMaxSize)
{
	...
	//NOTE: For variable types, the bytes will be bounded by ulMaxSize (cbMaxLen), for 
	//fixed length type bindings, the size will be bounded by the type, not ulMaxSIze...
	DBLENGTH cbMaxLen = 0;
	...
	{
		case DBTYPE_VARIANT:
			cbMaxLen = sizeof(VARIANT);
			break;

		case DBTYPE_BYTES:
			//Make sure variable types are bounded by the maximum size...
			cbMaxLen = min(ulColumnSize, ulMaxSize);
			break;
		...
		case DBTYPE_WSTR:
		{
			...
			switch(wBackendType)
			{
				...
				case DBTYPE_BYTES:
					...
					cbMaxLen = min(ulColumnSize, ulMaxSize) * 2 + 1;
					break;

				case DBTYPE_STR:
				case DBTYPE_WSTR:
					//String -> String
					//cbMaxLen already contains the length
					cbMaxLen = min(ulColumnSize, ulMaxSize) + 1;
					break;
				...
				default:
 					//For everything else, BSTR, VARIANT, VECTOR, UDT, etc
			...
			//Make sure variable types are bounded by the maximum size...
			cbMaxLen = min(cbMaxLen, ulMaxSize);	
			break;
		}
		...
		default:
		{
			...
			//Otherwise try to use the type size...
			if(SUCCEEDED(GetDBTypeMaxSize(wBindingType, &ulTypeSize)))
			{
				//Use the size of the type specified...
				cbMaxLen = min(ulTypeSize, ulMaxSize);
			}
			else
			{
 				//For everything else, VARIANT, VECTOR, UDT, etc
				//Just default to our largest buffer size
				cbMaxLen = ulMaxSize;
			}
			break;
...
//
//////////////////////////////////////////////////////////////////
void InternalTraceFmt(WCHAR*	pwszFmt, ...)
{
	va_list		marker;
