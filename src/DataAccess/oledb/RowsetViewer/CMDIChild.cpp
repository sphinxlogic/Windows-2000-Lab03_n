...
////////////////////////////////////////////////////////////////
// CMDIChild::ReleaseHeldRows
//
/////////////////////////////////////////////////////////////////
HRESULT CMDIChild::ReleaseHeldRows()
{
...
//
/////////////////////////////////////////////////////////////////
HRESULT CMDIChild::ReleaseRows(INDEX iIndex, BOOL fOnlyValidRows)
{
	HRESULT hr = S_OK;
	...
	{
		//Find all Rows
		LV_GetAllItems(m_pCDataGrid->m_hWnd, &cRows, &rgItems, (LPARAM**)&rghRows);
	}
	...
	else
	{
		...
		rgItems[0] = iIndex;
		rghRows[0] = m_pCDataGrid->GetItemParam(iIndex);
	}
...
CLEANUP:
	...
	for(i=0; i<cRows; i++)
	{
		if(rgRefCounts[i] == 0)
		{
			...
			m_pCDataGrid->SetItemText(rgItems[i], 0, L"");
			m_pCDataGrid->SetItemParam(rgItems[i], 0, DB_NULL_HROW);
		}
	}
...
//
/////////////////////////////////////////////////////////////////
HRESULT CMDIChild::AddRefRows(INDEX iIndex)
{
	HRESULT hr = S_OK;
	CRowset*	pCRowset = (CRowset*)GetObject(eCRowset);
	...
	{
		//Find all Rows
		LV_GetAllItems(m_pCDataGrid->m_hWnd, &cRows, NULL, (LPARAM**)&rghRows);
	}
...
BOOL CMDIChild::OnCommand(UINT iID, HWND hWndCtrl)
{
	...
	switch(iID)
	{
		...
		case IDM_COMMAND_GETDBSESSION:
			if(pCCommand && pCCommand->m_pICommand)
			{
				...
				if(interfaceDlg.DoModal(hWnd) == IDOK)
				{
					...
					XTEST(hr = pCCommand->m_pICommand->GetDBSession(interfaceDlg.GetSelInterface(), &pIUnknown));
					TRACE_METHOD(hr, L"ICommand::GetDBSession(%s, &0x%p)", GetInterfaceName(interfaceDlg.GetSelInterface()), pIUnknown);
					
					//Update the Session Object
					if(SUCCEEDED(hr))
						m_pCMainWindow->HandleObjectType(pCCommand, pIUnknown, interfaceDlg.GetSelInterface(), eCSession, 0, NULL, CREATE_FINDWINDOW);
					TRACE_RELEASE(pIUnknown, L"Session");
				}
			}
			return TRUE;
		...
		case IDM_DATASET_GETCELLDATA:
  			if(pCDataset && pCDataset->m_pIMDDataset)
  			{
				...
				for(INDEX i=0; i<cRows; i++)
				{
					...
					DisplayData(IID_NULL, rghRows[i], rgItems[i]);
				}
				SAFE_FREE(rgItems);
				SAFE_FREE(rghRows);
  			}
		...
		case IDM_DATASET_GETSPECIFICATION:
  			if(pCDataset && pCDataset->m_pIMDDataset)
			{
				...
				if(interfaceDlg.DoModal(hWnd) == IDOK)
				{
					...
					IUnknown* pIUnknown = NULL;
					XTEST(hr = pCDataset->m_pIMDDataset->GetSpecification(interfaceDlg.GetSelInterface(), &pIUnknown));
					TRACE_METHOD(hr, L"IMDDataset::GetSpecification(%s, &0x%p)", GetInterfaceName(interfaceDlg.GetSelInterface()), pIUnknown);
					
					//What type of object, let our helper determine...
					//NOTE: Can pontentially return other object types: (ie: CREATE_DETERMINE_TYPE)
					if(SUCCEEDED(hr))
						m_pCMainWindow->HandleObjectType(pCDataset, pIUnknown, interfaceDlg.GetSelInterface(), eCCommand, 0, NULL, CREATE_FINDWINDOW | CREATE_DETERMINE_TYPE);
		...
		{
			CMultipleResults* pCMultipleResults = SOURCE_GETOBJECT(pCObject, CMultipleResults);
			if(pCMultipleResults)
			{
				CInterfaceDlg interfaceDlg(IDD_AGGREGATION, L"IMultipleResults::GetResult", IID_IRowset); 
				if(interfaceDlg.DoModal(hWnd) == IDOK)
				{
					CWaitCursor waitCursor;
					...
					//NOTE: Can pontentially return other object types: (ie: CREATE_DETERMINE_TYPE)
					if(SUCCEEDED(pCMultipleResults->GetResult(interfaceDlg.GetAggregate(), interfaceDlg.GetSelInterface(), &cRowsAffected, interfaceDlg.ppUnknown())))
						m_pCMainWindow->HandleObjectType(pCMultipleResults, interfaceDlg.pUnknown(), interfaceDlg.GetSelInterface(), eCRowset, 0, NULL, CREATE_NEWWINDOW_IFEXISTS | CREATE_DETERMINE_TYPE);
				}
			}
			return TRUE;
		}
		...
		case IDM_ITRANSACTIONLOCAL_GETOPTIONSOBJECT:
		{
			...
			{
				CWaitCursor waitCursor;
				ITransactionOptions* pITransactionOptions = NULL;

				//ITransactionLocal::GetOptionsObject
				XTEST(hr = pITransactionLocal->GetOptionsObject(&pITransactionOptions));
				TRACE_METHOD(hr, L"ITransactionLocal::GetOptionsObject(&0x%p)", pITransactionOptions);

				//Delegate
				if(SUCCEEDED(hr))
					m_pCMainWindow->HandleObjectType(pCObject, pITransactionOptions, IID_ITransactionOptions, eCTransactionOptions, 0, NULL, CREATE_NEWWINDOW_IFEXISTS);
				TRACE_RELEASE(pITransactionOptions, L"ITransactionOptions");
			}
			return TRUE;
		}
		...
		case IDM_ITRANSACTIONOBJECT_GETTRANSACTIONOBJECT:
		{
			...
			if(pITransactionObject)
			{
				...
				XTEST(hr = pITransactionObject->GetTransactionObject(1, &pITransaction));
				TRACE_METHOD(hr, L"ITransactionObject::GetTransactionObject(1, &0x%p)", pITransaction);

				//Delegate
				if(SUCCEEDED(hr))
					m_pCMainWindow->HandleObjectType(pCObject, pITransaction, IID_ITransaction, eCTransaction, 0, NULL, CREATE_NEWWINDOW_IFEXISTS);
		...
		case IDM_ITRANSACTIONJOIN_GETOPTIONSOBJECT:
		{
			...
			if(pITransactionJoin)
			{
				...
				XTEST(hr = pITransactionJoin->GetOptionsObject(&pITransactionOptions));
				TRACE_METHOD(hr, L"ITransactionJoin::GetOptionsObject(&0x%p)", pITransactionOptions);
		...
		case IDM_RELEASEROWS:
			if(pCRowset && pCRowset->m_pIRowset)
				ReleaseRows(LV_ALLSELITEMS, FALSE/*fOnlyValidRows*/);
			return TRUE;

		case IDM_ADDREFROWS:
			if(pCRowset && pCRowset->m_pIRowset)
				AddRefRows(LV_ALLSELITEMS);
			return TRUE;

		case IDM_RELEASEALLROWS:
			if(pCRowset && pCRowset->m_pIRowset)
				ReleaseRows(LV_ALLITEMS, FALSE/*fOnlyValidRows*/);
			return TRUE;
		...
		case IDM_GETROWFROMHROW:
			if(pCRowset && pCRowset->m_pIGetRow)
			{
				...
				if(interfaceDlg.DoModal(hWnd) == IDOK)
				{
					...
					if(SUCCEEDED(pCRowset->GetRowFromHROW(NULL, hRow, interfaceDlg.GetSelInterface(), &pIUnknown)))
					{
						//Create in new window
						CBase* pCObject = m_pCMainWindow->HandleObjectType(pCRowset, pIUnknown, interfaceDlg.GetSelInterface(), eCRow, 0, NULL, CREATE_NEWWINDOW | CREATE_NODISPLAY);
						if(pCObject)
						{
		...
		case IDM_IROW_GETSOURCEROWSET:
		{
			...
			if(pIRow)
			{
				...
				if(interfaceDlg.DoModal(hWnd) == IDOK)
				{
					...
					XTEST(hr = pIRow->GetSourceRowset(interfaceDlg.GetSelInterface(), &pIUnknown, &hRow));
					TRACE_METHOD(hr, L"IRow::GetSourceRowset(%s, &0x%p, &0x%p)", GetInterfaceName(interfaceDlg.GetSelInterface()), pIUnknown, hRow);
					...
					if(SUCCEEDED(hr))
						m_pCMainWindow->HandleObjectType(pCObject, pIUnknown, interfaceDlg.GetSelInterface(), eCRowset, 0, NULL, CREATE_FINDWINDOW | CREATE_RESTARTPOSITION);
		...
		case IDM_IGETSESSION_GETSESSION: 
		{
			...
			if(pIGetSession)
			{
				...
				if(interfaceDlg.DoModal(hWnd) == IDOK)
				{
					...
					IUnknown* pIUnknown = NULL;
					XTEST(hr = pIGetSession->GetSession(interfaceDlg.GetSelInterface(), &pIUnknown));
					TRACE_METHOD(hr, L"IGetSession::GetSession(%s, &0x%p)", GetInterfaceName(interfaceDlg.GetSelInterface()), pIUnknown);
					
					//Update the Session
					if(SUCCEEDED(hr))
						m_pCMainWindow->HandleObjectType(pCObject, pIUnknown, interfaceDlg.GetSelInterface(), eCSession, 0, NULL, CREATE_FINDWINDOW);
					TRACE_RELEASE(pIUnknown, L"Session");
				}
			}
			return TRUE;
		...
		case IDM_GETSPECIFICATION:
			if(pCRowset && pCRowset->m_pIRowsetInfo)
			{
				...
				if(interfaceDlg.DoModal(hWnd) == IDOK)
				{
					...
					IUnknown* pIUnknown = NULL;
					XTEST(hr = pCRowset->m_pIRowsetInfo->GetSpecification(interfaceDlg.GetSelInterface(), &pIUnknown));
					TRACE_METHOD(hr, L"IRowset::GetSpecification(%s, &0x%p)", GetInterfaceName(interfaceDlg.GetSelInterface()), pIUnknown);
					
					//What type of object, let our helper determine...
					//NOTE: Can pontentially return other object types: (ie: CREATE_DETERMINE_TYPE)
					if(SUCCEEDED(hr))
						m_pCMainWindow->HandleObjectType(pCRowset, pIUnknown, interfaceDlg.GetSelInterface(), eCSession, 0, NULL, CREATE_FINDWINDOW | CREATE_DETERMINE_TYPE);
					TRACE_RELEASE(pIUnknown, L"IUnknown");
				}
			}
			return TRUE;
		...
		case IDM_COMMAND_CLEARCOMMAND:
			if(pCCommand && pCSession && pCCommand->m_pIUnknown)
			{
				...
				pCSession->CreateCommand(NULL, IID_ICommand, (IUnknown**)&pICommand);
				pCCommand->CreateObject(pCSession, IID_ICommand, pICommand);
				
				TRACE_RELEASE(pICommand, L"ICommand");
			}
			return TRUE;
		...
		case IDM_IROWSETLOCATE_GETROWSBYBOOKMARK:
			if(pCRowset && pCRowset->m_pIRowsetLocate)
			{
				...
				for(i=0; i<cSelItems; i++)
				{
					...
					if(rghRows[i])
						DisplayData(IID_IRowset, rghRows[i], rgSelItems[i]);
				}
...
#ifdef INTERNAL_ONLY
		//Cursor Engine
		case IDM_IROWSET_CURSORENGINE:
			if(pCRowset && pCRowset->m_pIUnknown)
			{
				CWaitCursor waitCursor;

				//Invoke the Cursor engine...
				pCRowset->RestartPosition();
				pCMainWindow->HandleObjectType(pCRowset, pCRowset->m_pIUnknown, IID_IUnknown, eCRowset, 0, NULL, CREATE_NEWWINDOW | CREATE_CURSORENGINE);
			}
			return TRUE;
#endif //INTERNAL_ONLY
		...
		case IDM_IROWSETSCROLL_GETDATA:
			if(pCRowset && pCRowset->m_pIRowset)
			{
				...
				IID riidSource = IID_IRowset;
				if(iID == IDM_IROWSETLOCATE_GETDATA)
					riidSource = IID_IRowsetLocate;
				if(iID == IDM_IROWSETSCROLL_GETDATA)
					riidSource = IID_IRowsetScroll;
				...
				{
					//Display the Data for this row...
					DisplayData(riidSource, rghRows[i], rgItems[i]);
				}
				SAFE_FREE(rgItems);
				SAFE_FREE(rghRows);
			}
		...
		case IDM_GETVISIBLEDATA:
			if(pCRowset && pCRowset->m_pIRowsetResynch)
			{
				...
				{
					//Display the Data for this row...
					DisplayData(IID_IRowsetResynch, rghRows[i], rgItems[i]);
				}
				SAFE_FREE(rgItems);
				SAFE_FREE(rghRows);
			}
		...
		case IDM_RESYNCHROWS:
			if(pCRowset && pCRowset->m_pIRowsetResynch)
			{
				...
				{
					//Display the Data for this row...
					DisplayData(IID_IRowset, rghRows[i], rgItems[i]);
				}
		...
		case IDM_GETLASTVISIBLEDATA:
			if(pCRowset && pCRowset->m_pIRowsetRefresh)
			{
				...
				{
					//Display the Data for this row...
					DisplayData(IID_IRowsetRefresh, rghRows[i], rgItems[i]);
				}
				SAFE_FREE(rgItems);
				SAFE_FREE(rghRows);
			}
		...
		case IDM_GETORIGINALDATA:			
			if(pCRowset && pCRowset->m_pIRowsetUpdate)
			{
				...
				{
					//Display the Data for this row...
					DisplayData(IID_IRowsetUpdate, rghRows[i], rgItems[i]);
				}
				SAFE_FREE(rgItems);
				SAFE_FREE(rghRows);
			}
		...
		case IDM_ISERVICEPROVIDER:
			if(pCDataSource && pCDataSource->m_pIServiceProvider)
			{
				...
				if(interfaceDlg.DoModal(hWnd) == IDOK)
				{
					...
					XTEST(hr = pCDataSource->m_pIServiceProvider->QueryService(OLEDB_SVC_DSLPropertyPages, interfaceDlg.GetSelInterface(), (void**)&pIUnknown));
					TRACE_METHOD(hr, L"IServiceProvider::QueryService(OLEDB_SVC_DSLPropertyPages, %s, &0x%p)", GetInterfaceName(interfaceDlg.GetSelInterface()), pIUnknown);

					//TODO handle object in tree?
					TRACE_RELEASE(pIUnknown, L"IServiceProvider");
				}
			}
			return TRUE;
...
#ifdef INTERNAL_ONLY
		//DataSource - Pooling
		case IDM_IGETPOOL_GETPOOL:
			if(pCDataSource && pCDataSource->m_pIGetPool)
			{
				CWaitCursor waitCursor;
				IPoolInfo* pIPoolInfo = NULL;
				
				//IGetPool::GetPool
				if(SUCCEEDED(pCDataSource->GetPool(&pIPoolInfo)))
					m_pCMainWindow->HandleObjectType(pCDataSource, pIPoolInfo, IID_IPoolInfo, eCPoolInfo, 0, NULL, 0);
				TRACE_RELEASE(pIPoolInfo, L"IPoolInfo");
			}
			return TRUE;

		//DataSource - Pooling
		case IDM_IGETPOOL_GETPOOLSTATE:
			if(pCDataSource && pCDataSource->m_pIGetPool)
			{
				CWaitCursor waitCursor;

				DWORD dwPoolState = 0;
				pCDataSource->GetPoolState(&dwPoolState);
			}
			return TRUE;
#endif //INTERNAL_ONLY
...
HRESULT CMDIChild::UpdateWndTitle()
{
	...
	if(pCDataSource)
	{
		bstrProvider = pCDataSource->m_bstrProviderDesc &&  pCDataSource->m_bstrProviderDesc[0] ? pCDataSource->m_bstrProviderDesc : pCDataSource->m_bstrProviderName;
		bstrDesc	 = pCDataSource->m_bstrDataSource && pCDataSource->m_bstrDataSource[0] ? pCDataSource->m_bstrDataSource : pCDataSource->m_bstrDBMS;
	}
	...
	wSendMessageFmt(
				...
				bstrProvider ? bstrProvider : L"", 
				bstrDesc && bstrDesc[0] ? L"(" : L"",
				bstrDesc ? bstrDesc : L"",
				bstrDesc && bstrDesc[0] ? L")" : L""
				);
...
HRESULT CMDIChild::CreateEnumChild()
{
	...
	if(pCRowset && pCEnumerator)
	{
		...
		if(pCRowset->m_cBindings && pCRowset->m_rgBindings[0].iOrdinal==0)
			iColumn = 2;
		...
		EnumInfo.fIsParent = wcscmp(wszBuffer, L"True") ? TRUE : FALSE;
...
{
	INDEX	iResult = 0;
	CDataAccess* pCDataAccess = m_pCDataAccess;

	//Insert the Column Headers.
	//Only display the columns for which are bound in the Accessor
	
	//Insert Row Header
	m_pCDataGrid->InsertColumn(0, pCDataAccess->GetObjectType() == eCDataset ? L"  Cell Ordinal  " : L"  Row Handle  ");

	//Loop through the columns
	for(ULONG i=0; i<pCDataAccess->m_cBindings; i++)
	{
		DBCOLUMNINFO* pColInfo = pCDataAccess->GetColInfo(pCDataAccess->m_rgBindings[i].iOrdinal);

		//Get ColumnName
		iResult = m_pCDataGrid->InsertColumn(i+1, GetColName(pColInfo), pCDataAccess->GetColumnImage(pColInfo));
	}

	//AutoSize Columns, (including "Row-Handle" column)
	for(i=0; i<pCDataAccess->m_cBindings+1; i++)
		m_pCDataGrid->SetColumnWidth(i,	LVSCW_AUTOSIZE_USEHEADER);
...
)
{
	...
	FreeBindingData(pCDataAccess->m_cBindings, pCDataAccess->m_rgBindings, pData);
...
//
/////////////////////////////////////////////////////////////////
HRESULT CMDIChild::DumpData(HROW hRow, INDEX iIndex, void* pData)
{
	//NOTE: DumpData takes a "pData" argument, for those cases where the accessor and 
	//bindings remain the same but the buffer pointer changes.  This is used in multiple
	//parameter sets, and cell data with OLAP.
	
	DBSTATUS dbStatus = 0;
	DBLENGTH dbLength = 0;
	WCHAR	wszBuffer[MAX_COL_SIZE+1] = {0};
	DWORD	dwConvFlags = m_pCMainWindow->GetOptions()->m_dwConvFlags; 
	CDataAccess* pCDataAccess = m_pCDataAccess;

	//Add the "hRow" column
	if(hRow)
		StringFormat(wszBuffer, NUMELE(wszBuffer), L"0x%p", hRow);
	m_pCDataGrid->SetItemText(iIndex, 0, wszBuffer);
	m_pCDataGrid->SetItemParam(iIndex, 0, hRow);

	//Display all Columns
	for(ULONG i=0; i<pCDataAccess->m_cBindings; i++)
	{
		//Obtain STATUS, LENGTH, VALUE
		DBBINDING* pBinding		= &pCDataAccess->m_rgBindings[i];
		DBCOLUMNINFO* pColInfo	= pCDataAccess->GetColInfo(pBinding->iOrdinal);

		//Obtain the column data...
		pCDataAccess->GetColumnData(pBinding, pData, &dbStatus, &dbLength, NULL, wszBuffer, MAX_COL_SIZE, dwConvFlags | CONV_TYPENAME, pColInfo->wType);
	
		//Set item in ListView
	   	m_pCDataGrid->SetItemText(iIndex, i+1, wszBuffer);
		m_pCDataGrid->SetItemImage(iIndex, i+1, pCDataAccess->GetColumnImage(NULL, dbStatus));
	}
...
//
/////////////////////////////////////////////////////////////////
HRESULT CMDIChild::DisplayRows(DBROWOFFSET lOffset, DBROWCOUNT cRows, DBCOUNTITEM cRowsObtained, HROW* rghRows, BOOL fAdjustFetchPosition)
{
	HRESULT hr = S_OK;
	...
	INDEX cItems = m_pCDataGrid->GetItemCount();
	ASSERT(cItems >= 0);
	...
	if(cRowsObtained)
	{
		...
		if(fAdjustFetchPosition)
			m_pCDataGrid->SetItemImage((INDEX)m_lCurPos, 0, IMAGE_NORMAL);

		//Remove Previous Selection Bars
		for(INDEX iIndex=0; iIndex<cItems; iIndex++)
			m_pCDataGrid->SetItemState(iIndex, 0, ~LVIS_SELECTED, LVIS_SELECTED);
	}
	...
	for(DBCOUNTITEM iRow=0; iRow<cRowsObtained; iRow++)
	{
		...
		while(iIndex >= cItems)
		{
			m_pCDataGrid->InsertItem(iIndex, 0, L"", 0, IMAGE_NORMAL);
			cItems++;
		}
		...
		while(iIndex < 0)
		{
			m_pCDataGrid->InsertItem(0, 0, L"", 0, IMAGE_NORMAL);
			cItems++;
	...
	if(cRowsObtained)
	{
		...
		if(fAdjustFetchPosition)
		{
			...
			m_pCDataGrid->SetItemImage(lEndIndex, 0, fLastFetchForward ? IMAGE_ARROW_DOWN : IMAGE_ARROW_UP);

			//Display the new Selection Bar for this row
			m_pCDataGrid->SetItemState(lEndIndex, 0, LVIS_SELECTED, LVIS_SELECTED);
		...
		m_pCDataGrid->EnsureVisible(lEndIndex);
	}
...
//
/////////////////////////////////////////////////////////////////
HRESULT CMDIChild::GetNextRows(DBROWOFFSET lOffset, DBROWCOUNT cRows, BOOL fRetry)
{
	HRESULT hr		= S_OK;
	HRESULT hrGNR	= S_OK;
	CDataAccess* pCDataAccess = m_pCDataAccess;
...
HRESULT CMDIChild::ChangeSelectedRow()
{
	...
	m_iSelectedRow = m_pCDataGrid->GetSelectedRow(NULL, FALSE);

	//Create Dialog box, passing the "this" pointer
	//NOTE: m_pCSource and m_idSource are already setup...
	DisplayDialog(IDD_ROWCHANGE, m_hWnd, RowChangeProc, m_pCSource, m_idSource);
	return S_OK;
}
...
HRESULT CMDIChild::RestartPosition()
{
	...
	m_pCDataGrid->SetItemImage((INDEX)m_lCurPos, 0, IMAGE_ARROW_UP);
	m_pCDataGrid->EnsureVisible((INDEX)m_lCurPos);
...
HRESULT CMDIChild::RefreshData()
{
	...
	if(pCRow)
	{
		...
		TESTC(hr = DisplayData(IID_IRow, pCRow->m_hSourceRow, 0));
	}
	else
	{
		...
		DBROWCOUNT cRowsToFetch = m_pCDataGrid->m_lMaxRows+3;
...
//
/////////////////////////////////////////////////////////////////
HRESULT CMDIChild::GetListViewValues(HWND hWndNames, HWND hWndValues, CDataAccess* pCDataAccess, DBCOUNTITEM cBindings, DBBINDING* rgBindings, void* pData)
{
	ASSERT(hWndNames);
	ASSERT(hWndValues);
	ASSERT(pCDataAccess);
	...
	for(ULONG i=0; i<cBindings; i++)
	{
		...
		if(bChecked)
		{
			...
			TESTC(hr = pCDataAccess->SetColumnData(&rgBindings[i], pData, dbStatus, wcslen(wszBuffer), wszBuffer, CONV_NONE, wBackendType));
		...
		else
		{
			...
			if(STATUS_IS_BOUND(rgBindings[i]))
				BINDING_STATUS(rgBindings[i], pData) = DBSTATUS_S_IGNORE;
		}
	}
...
INT_PTR WINAPI CMDIChild::RowChangeProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message)
	{
		case WM_INITDIALOG:
		{
			...
			{
				//Dataset Object
				hr = pCDataset->GetCellData(pThis->m_iSelectedRow, pThis->m_iSelectedRow);
			}
			else if(pCRowset)
			{
				//Rowset
				...
				if(pThis->m_iSelectedRow != LVM_ERR)
					hRow = pThis->m_pCDataGrid->GetItemParam(pThis->m_iSelectedRow);
				...
				if(hRow || pThis->m_idSource != IDM_INSERTROW)
					hr = pCRowset->GetData(IID_IRowset, hRow);
			}
			...
			for(iBind=0; iBind<pCDataAccess->m_cBindings; iBind++)
			{
				//Get the Length, Status, Data for this Column
				DBBINDING* pBinding		= &pCDataAccess->m_rgBindings[iBind];
				DBCOLUMNINFO* pColInfo	= pCDataAccess->GetColInfo(pBinding->iOrdinal);
				wszBuffer[0] = wEOL;
				
				//Obtain the Data...
				if(pCRow || pCDataset || hRow)
		...
		case WM_COMMAND:
		{
			...
			switch (GET_WM_COMMAND_ID(wParam, lParam))
			{
				...
				case IDOK:
				{
					...
					DBCOUNTITEM cBindings	= pCDataAccess->m_cBindings;
					DBBINDING* rgBindings	= pCDataAccess->m_rgBindings;
					void* pData				= pCDataAccess->m_pData;
					...
					TESTC(hr = pThis->GetListViewValues(s_listNames.m_hWnd, s_listValue.m_hWnd, pCDataAccess, cBindings, rgBindings, pData));
					...
					switch(pThis->m_idSource)
					{
						case IDM_INSERTROW:
							if(pCRowset && pCRowset->m_pIRowsetChange)
							{
								...
								//Display Data Errors
								TESTC(hr = DisplayBindingErrors(hr, cBindings, rgBindings, pData));
					...
					//TODO - need to free in the case of errors as well.  But have to be careful
					//since the pData may not have been setup yet, or other errors where its undefined...
					TESTC(FreeBindingData(cBindings, rgBindings, pData, TRUE/*fSetData*/));

					if(pThis->m_idSource != IDM_INSERTROW)
					{
						//Need to indicate row was modified, use "changed" icon
						pThis->m_pCDataGrid->SetItemImage(pThis->m_iSelectedRow, 0, IMAGE_CHANGE);

						//Display the Data for this Row
						TESTC(hr = pThis->DisplayData(pCRow ? IID_IRow : IID_IRowset, hRow, pThis->m_iSelectedRow));
					}
...
INT_PTR WINAPI CMDIChild::ColumnChangeProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message)
	{
		case WM_INITDIALOG:
		{
			...
			ASSERT((DBCOUNTITEM)pThis->m_iSelectedCol < pCDataAccess->m_cBindings);
			DBBINDING* pBinding		= &pCDataAccess->m_rgBindings[pThis->m_iSelectedCol];
			DBCOLUMNINFO* pColInfo	= pCDataAccess->GetColInfo(pBinding->iOrdinal);
			void* pData				= pCDataAccess->m_pData;
			...
			iItemCount = pThis->m_pCDataGrid->GetItemCount();
			for(i=0; i<iItemCount; i++)
			{
				//Get row handle for the selected row
				HROW hRow = pThis->m_pCDataGrid->GetItemParam(i);

				if(pCRow)
				{
					ULONG cColAccess = 1;
					DBCOLUMNACCESS* rgColAccess = &pCRow->m_rgColAccess[pThis->m_iSelectedCol];
				...
				else if(pCRowset)
				{
					...
					//Much quicker to get and set using only the column we need.
					hr = pCRowset->GetData(IID_IRowset, hRow, hAccessor);
				}
		...
		case WM_COMMAND:
		{
			...
			switch (GET_WM_COMMAND_ID(wParam, lParam))
			{
				...
				case IDOK:
				{
					...
					ASSERT((ULONG)pThis->m_iSelectedCol < pCDataAccess->m_cBindings);
					DBBINDING* pBinding		= &pCDataAccess->m_rgBindings[pThis->m_iSelectedCol];
					DBCOLUMNINFO* pColInfo	= pCDataAccess->GetColInfo(pBinding->iOrdinal);
					void* pData				= pCDataAccess->m_pData;
					...
					for(i=0; i<iItemCount; i++)
					{
						...
						//Display the Data for this Row
						TESTC(hr = pThis->DisplayData(pCRow ? IID_IRow : IID_IRowset, hRow, i));
					}
		...
		case WM_COMMAND:
		{
			switch (GET_WM_COMMAND_ID(wParam, lParam))
			{
				...
				case IDOK:
				{
					...
					//GetAxisRowset
					if(SUCCEEDED(hr = pCDataset->GetAxisRowset(pCAggregate, iAxis, riid, cPropSets, rgPropSets, fOutput ? &pIUnknown : NULL)))
					{
						//Now that we have the rowset, display it...
						//NOTE: Can pontentially return other object types: (ie: CREATE_DETERMINE_TYPE)
						if(!pThis->m_pCMainWindow->HandleObjectType(pCDataset, pIUnknown, riid, eCRowset, 0, NULL, CREATE_NEWWINDOW | CREATE_DETERMINE_TYPE))
							TESTC(hr = E_FAIL);
					}
	...
	switch(message)
	{
				...
				case IDOK:
				{
					...
					switch(pThis->m_idSource)
					{
						case IDM_GETNEXTROWS:
						{
							...
							TESTC(hr = pThis->GetNextRows(s_lOffset, s_cRows));
							break;
						}
						...
						case IDM_IROWSETLOCATE_GETROWSAT:
						{
							...
							//Obtain all Selected Rows
							LV_GetSelItems(pThis->m_pCDataGrid->m_hWnd, &cSelItems, &rgSelItems, (LPARAM**)&rghRowsSel);
							...
							pThis->DisplayRows(rgSelItems[0] + s_lOffset, s_cRows, cRowsObtained, rghRows, FALSE/*fAdjustFetchPosition*/);
		...
		case WM_NOTIFY:
		{
			...
			if(pUpDown->hdr.idFrom == IDC_SPIN_CROWS)
			{
				...
				wSendMessageFmt(::GetDlgItem(hWnd, IDE_CROWS), WM_SETTEXT, 0, L"%ld", s_cRows);
			}
...
INT_PTR WINAPI CMDIChild::CreateAccessorProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message)
	{
		case WM_INITDIALOG:
		{
			...
			for(i=0; i<pCDataAccess->m_cBindings; i++)
			{
				DBORDINAL iOrdinal = pCDataAccess->m_rgBindings[i].iOrdinal;
				DBORDINAL iIndex = (iOrdinal && !pCDataAccess->m_rgColumnInfo[0].iOrdinal==0) ? iOrdinal-1 : iOrdinal;
			...
			switch (GET_WM_COMMAND_ID(wParam, lParam))
			{
				case IDOK:
				{
					...
					if(SUCCEEDED(hr))
					{
						...
						//FreeBindings
						FreeBindings(&pCDataAccess->m_cBindings, &pCDataAccess->m_rgBindings);
						
						//Use the new Bindings
						pCDataAccess->m_hAccessor	= hAccessor;
						pCDataAccess->m_cBindings	= cSelBindings;
						pCDataAccess->m_rgBindings	= rgSelBindings;
						...
						if(SUCCEEDED(hr = pThis->RestartPosition()))
	...
	switch(message)
	{
		case WM_INITDIALOG:
		{
			...
			for(ULONG i=0; i<g_cDBTypes; i++)
				s_CComboTypes.AddString(g_rgDBTypes[i].pwszName);
			...
			for(i=0; i<NUMELE(g_rgParamFlags); i++)
			{
...
//
/////////////////////////////////////////////////////////////////
INT_PTR WINAPI CMDIChild::ParamExecuteProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	static BOOL bBeginEdit = FALSE;
	...
	switch(message)
	{
		case WM_INITDIALOG:
		{
			...
			//Use Extended ListView Styles!
			SendMessage(hWndNames, LVM_SETEXTENDEDLISTVIEWSTYLE, LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES | LVS_EX_TWOCLICKACTIVATE | LVS_EX_CHECKBOXES, LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES | LVS_EX_TWOCLICKACTIVATE | LVS_EX_CHECKBOXES);
			SendMessage(hWndValues,	LVM_SETEXTENDEDLISTVIEWSTYLE, LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES | LVS_EX_TWOCLICKACTIVATE, LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES | LVS_EX_TWOCLICKACTIVATE);

			//Set image list to the Window 
			ListView_SetImageList(hWndNames, ImageList_LoadImage(GetAppLite()->m_hInstance, MAKEINTRESOURCE(IDB_STATE), 16, 16, CLR_DEFAULT , IMAGE_BITMAP, LR_DEFAULTCOLOR), LVSIL_STATE);
			ListView_SetImageList(hWndValues, ImageList_LoadImage(GetAppLite()->m_hInstance, MAKEINTRESOURCE(IDB_IMAGE), 16, 16, CLR_DEFAULT , IMAGE_BITMAP, LR_DEFAULTCOLOR), LVSIL_SMALL);

			DBPARAMS* pParamInfo = &pCCommand->m_ParamInfo;
			DBCOUNTITEM cBindings = pCCommand->m_cParamBindings;
			...
			//ListView Columns
			LV_InsertColumn(hWndNames,	0,		L"Parameter");
			LV_InsertColumn(hWndNames,	1,		L"Length");
			LV_InsertColumn(hWndNames,	2,		L"Status");
			LV_InsertColumn(hWndValues,	0,		L"Value");
			
			for(ULONG i=0; i<cBindings && pParamInfo->cParamSets; i++)
			{	
				ASSERT(rgBindings);
				DBBINDING* pBinding = &rgBindings[i];
				...
				//Value
				//Get the Length, Status, Data for this Column
				pCCommand->GetColumnData(pBinding, pParamInfo->pData, &dbStatus, &dbLength, NULL, wszBuffer, MAX_COL_SIZE, dwConvFlags, pBinding->wType);

				//Insert the Data into the list (make an "easy" edit)
				LV_InsertItem(hWndValues, i, 0, wszBuffer, (LPARAM)pBinding->wType, 0);

				//Insert Parameter Name
				StringFormat(wszBuffer, NUMELE(wszBuffer), L"Parameter %lu", i+1);
				LV_InsertItem(hWndNames, i, 0, wszBuffer);
				...
				if(LENGTH_IS_BOUND(*pBinding))
					...
					StringCopy(wszBuffer, L"Not Bound", MAX_COL_SIZE);
				LV_InsertItem(hWndNames, i, 1, wszBuffer);

				//Insert the Status into the List
				LV_InsertItem(hWndNames, i, 2, STATUS_IS_BOUND(*pBinding) ? GetStatusName(dbStatus) : L"Not Bound");

				//Set Item State to Checked/Unchecked
				LV_SetItemState(hWndNames, i, 0,  INDEXTOSTATEIMAGEMASK(STATE_CHECKED),  LVIS_STATEIMAGEMASK);
			}

			if(cBindings == 0 || pParamInfo->cParamSets == 0)
			{
				LV_InsertItem(hWndNames, 0, 0,	L"Parameter 1");
				LV_InsertItem(hWndNames, 0, 1,	L"0");
				LV_InsertItem(hWndNames, 0, 2,	L"DBSTATUS_S_OK");
				LV_InsertItem(hWndValues, 0, 0,	L"Enter Parameter Value...");
			}
		...
		case WM_COMMAND:
		{
			...
			switch (GET_WM_COMMAND_ID(wParam, lParam))
			{
				case IDOK:
				{
					...
					cItems = (ULONG)SendMessage(hWndNames, LVM_GETITEMCOUNT, 0, 0);
					cParams = cItems != LVM_ERR ? cItems : 0;
					
					//Obtain the Command Text
					pwszCommandText = pThis->m_pCQueryBox->GetSelectedText();
					...
					for(i=0; i<cParams; i++)
					{
						...
						rgParamInfo[i].wType		= pThis->GetOptions()->GetBindingType(DBTYPE_WSTR/*wNativeType*/);
						rgParamInfo[i].bPrecision	= 0;
						rgParamInfo[i].bScale		= 0;
					}

					//If the provider can derive parameter information then we can improve our 
					//parameter info, which will inturn provide a better default param accessor...
					//We need to make sure the command is prepared (if not already)...
					...
					//Setup Data Buffer
					TESTC(hr = pThis->GetListViewValues(hWndNames, hWndValues, pCCommand, pCCommand->m_cParamBindings, pCCommand->m_rgParamBindings, pParamInfo->pData));
				...
				case IDB_ADD:
				{
					...
					INDEX cItems = (INDEX)SendMessage(hWndNames, LVM_GETITEMCOUNT, 0, 0);
					if(cItems != LVM_ERR)
					{
						//Insert a new Parameter
						StringFormat(wszBuffer, NUMELE(wszBuffer), L"Parameter %lu", cItems+1);
						LV_InsertItem(hWndNames,	cItems, 0,	wszBuffer);
						LV_InsertItem(hWndNames,	cItems, 1,	L"0");
						LV_InsertItem(hWndNames,	cItems, 2,	L"DBSTATUS_S_OK");
						LV_InsertItem(hWndValues,	cItems, 0,	L"Enter Parameter Value...");

						//Set Item State to Checked/Unchecked
						LV_SetItemState(hWndNames, cItems, 0,  INDEXTOSTATEIMAGEMASK(STATE_CHECKED),  LVIS_STATEIMAGEMASK);
					}
					return 0;
				}
				...
				case IDB_DELETE:
				{
					...
					if(cItems != LVM_ERR)
					{
						...
						SendMessage(hWndNames, LVM_DELETEITEM, cItems-1, 0);
						SendMessage(hWndValues, LVM_DELETEITEM, cItems-1, 0);
					}
					return 0;
				}
				case IDM_DBSTATUS_S_OK:
				{
					...
					//Insert the Status into the List
					INDEX iSelRow = (INDEX)SendMessage(hWndNames, LVM_GETNEXTITEM, (WPARAM)-1, (LPARAM)LVNI_SELECTED);
					if(iSelRow == LVM_ERR)
						return 0;

					LV_InsertItem(hWndNames, iSelRow, 2, L"DBSTATUS_S_OK");
					return 0;
				}
				...
				case IDM_DBSTATUS_S_ISNULL:
				{
					...
					LV_InsertItem(hWndNames, iSelRow, 2, L"DBSTATUS_S_ISNULL");
					return 0;
				}
				...
				case IDM_DBSTATUS_S_DEFAULT:
				{
					...
					LV_InsertItem(hWndNames, iSelRow, 2, L"DBSTATUS_S_DEFAULT");
						return 0;
				}
				...
				case IDM_DBSTATUS_S_IGNORE:
				{
					...
					LV_InsertItem(hWndNames, iSelRow, 2, L"DBSTATUS_S_IGNORE");
						return 0;
				}
			}
			break;
		...
		{	
			...
			//Must have selected a row to change status
			INDEX iSel = (INDEX)SendMessage(hWndNames, LVM_GETNEXTITEM, (WPARAM)-1, (LPARAM)LVNI_SELECTED);
			if(iSel == LVM_ERR)
				return FALSE;
		...
		case WM_NOTIFY:
		{
			...
			switch(pDispInfo->hdr.code)
			{
				...
				{
					//Obtain the SelectedRow
					INDEX iSel = (INDEX)SendMessage(::GetDlgItem(hWnd, IDL_PARAMVALUES), LVM_GETNEXTITEM, (WPARAM)-1, (LPARAM)LVNI_SELECTED);
					if(iSel == LVM_ERR)
						return 0;
				...
				case LVN_ENDLABELEDIT:
				{
					...
					if(pDispInfo->item.pszText)
					{
						WCHAR wszBuffer[MAX_NAME_LEN]={0};
						ConvertToWCHAR(pDispInfo->item.pszText, wszBuffer, MAX_NAME_LEN);
						LV_SetItemText(hWndValues, pDispInfo->item.iItem, 0, wszBuffer);
					}
...
INT_PTR WINAPI CMDIChild::CommitTransactionProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch(message)
	{
		case WM_INITDIALOG:
		{
			...
			{
				iSel = (INDEX)wSendMessage(hWndXACTTC, CB_ADDSTRING,	0, g_rgXACTTC[i].pwszName);
		...
		case WM_COMMAND:
		{
			...
			switch (GET_WM_COMMAND_ID(wParam, lParam))
			{
				case IDOK:
				{
					...
					iSel = (INDEX)SendMessage(hWndXACTTC, CB_GETCURSEL, 0, 0);
					DWORD XACTTC = (DWORD)SendMessage(hWndXACTTC, CB_GETITEMDATA, iSel, 0);
					SendMessage(hWndXACTTC, WM_GETTEXT, MAX_NAME_LEN, (LPARAM)wszBuffer);
					...
					TRACE_METHOD(hr, L"ITransaction::Commit(%s, %s, 0)", fRetaining ? L"TRUE" : L"FALSE", wszBuffer);
		...
		case WM_COMMAND:
		{
			...
			switch (GET_WM_COMMAND_ID(wParam, lParam))
			{
				...
				case IDOK:
				{
					...
					if(SUCCEEDED(hr))
						hr = pThis->HandleRowset(pThis->m_pCSource, pIUnknown, riid, CREATE_NEWWINDOW_IFEXISTS, pCSession ? IID_IOpenRowset : IID_IScopedOperations, TableID.uName.pwszName);
...
INT_PTR WINAPI CMDIChild::RowOpenProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message)
	{
		case WM_INITDIALOG:
		{
			...
			INDEX iDefColumn = -1;
			for(ULONG i=0; i<pCRow->m_cColInfo; i++)
			{
				iSel = (INDEX)wSendMessage(hWndColumns, CB_ADDSTRING, 0, GetColName(&pCRow->m_rgColumnInfo[i]));
				SendMessage(hWndColumns, CB_SETITEMDATA, iSel, (LPARAM)&pCRow->m_rgColumnInfo[i].columnid);
				
				//Default the selected column to the first BLOB, Stream, or IUnknown object
				if(iDefColumn==-1 && ((pCRow->m_rgColumnInfo[i].wType == DBTYPE_IUNKNOWN) || (pCRow->m_rgColumnInfo[i].dwFlags & (DBCOLUMNFLAGS_ISLONG | DBCOLUMNFLAGS_ISDEFAULTSTREAM))))
					iDefColumn = i;
			}
		...
		case WM_COMMAND:
		{
			...
			switch (GET_WM_COMMAND_ID(wParam, lParam))
			{
				...
				case IDOK:
				{
					...
					if(!pThis->m_pCMainWindow->HandleObjectType(pCRow, pIUnknown, riid, GuidToSourceType(guidObjectType), 0, NULL, CREATE_NEWWINDOW))
						TESTC(hr = E_FAIL);
...
CBase* CMDIChild::GetObject(SOURCE eSource, BOOL fAlways)
{
	...
	if(ppCBase)
	{
		...
		if(pCBase && (eSource==eCUnknown || pCBase->GetObjectType()==eSource) && (fAlways || pCBase->m_pIUnknown))
			return pCBase;
	}
		...
		case WM_COMMAND:
		{
			...
			switch(GET_WM_COMMAND_ID(wParam, lParam))
			{
				...
				case IDB_ADD:
				{
					...
					if (0 >= cItems)
						LV_SetItemState(hWndIndexColumnDesc, 0, COLUMN_ID, LVIS_SELECTED, LVIS_SELECTED);						
					return 0;
				}
				...
				case IDOK:
				{
					...
					for (i=0; i<cIndexColumnDesc; i++)
					{
						...
						if (0 == wcscmp(wszBuffer, L"Descendent"))
		...
		case WM_COMMAND:
		{
			...
			switch (GET_WM_COMMAND_ID(wParam, lParam))
			{
				...
				case IDOK:
				{
					...
					// Column Size
					GetEditBoxValue(hWndSize, 0, LONG_MAX, &lValue, TRUE);
					ColumnDesc.ulColumnSize = lValue;
					
					// Column Precision
					GetEditBoxValue(hWndPrecision, 0, 255, &lValue, TRUE);
					ColumnDesc.bPrecision = (BYTE)lValue;
					
					// Column Scale
					GetEditBoxValue(hWndScale, 0, 255, &lValue, TRUE);
...
INT_PTR WINAPI CMDIChild::ISCO_Proc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message)
	{
		...
		{	
			...
			if(pThis->m_idSource == IDM_ISCOPEDOPERATIONS_COPY)
			{
		...
		case WM_COMMAND:
		{
			...
			switch (GET_WM_COMMAND_ID(wParam, lParam))
			{
				case IDB_SETPARAMS:
				{
					...
					//Setup Parameters
					pThis->DisplayDialog(IDD_PARAMEXECUTE, hWnd, ParamExecuteProc, pThis->m_pCSource, pThis->m_idSource);
					return 0;
				}
				...
				case IDB_SETPROPERTIES:
				{
					...
					if(pCRowset && pThis->GetOptions()->m_dwCommandOpts & COMMAND_RELEASEROWSET)
					{
						pCRowset->ReleaseObject();
						pThis->UpdateControls();
					}	
				...
				case IDOK:
				{
					...
					if(fUseParams)
						pParamInfo = &pCCommand->m_ParamInfo;
					...
					//Execute the Command
					TESTC(hr = pCCommand->Execute(pCAggregate, pwszCommandText, riid, pParamInfo, NULL, fOutput ? &pIUnknown : NULL));

					//Process the Rowset
					TESTC(hr = pThis->HandleRowset(pCCommand, pIUnknown, riid, CREATE_NEWWINDOW_IFEXISTS, IID_ICommand, pwszCommandText));
...
INT_PTR WINAPI CMDIChild::SetCommandTextProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message)
	{
		case WM_INITDIALOG:
		{
			...
			::CheckDlgButton(hWnd, IDB_RELEASEPREVROWSET,	BST2STATE(fRelPrevRowset));
...
//
/////////////////////////////////////////////////////////////////
HRESULT CMDIChild::GetChapteredChild(INDEX iSelectedCol, DBBINDING* pBinding, REFIID riid)
{
	ASSERT(pBinding);
	IUnknown* pIUnknown = NULL;
	HRESULT hr = S_OK;
	...
	if(pCRowset && pCRowset->m_pIRowsetInfo)
	{
		...
		//Obtain the hChapter from the rowset for the selected row...
	...
	switch(message)
	{
		case WM_INITDIALOG:
		{
			...
			for(ULONG i=0; i<pCRowset->m_cBindings; i++)
			{
				...
				DBORDINAL iOrdinal = pCRowset->m_rgBindings[i].iOrdinal;
				DBCOLUMNINFO* pColInfo = pCRowset->GetColInfo(iOrdinal);
			...
			//Set Default...
			SendMessage(hWndColumn, CB_SETCURSEL, iSavedCol < (INDEX)pCRowset->m_cBindings && iSavedCol!=CB_ERR ? iSavedCol : 0, 0);
		...
		case WM_COMMAND:
		{
			...
			switch (GET_WM_COMMAND_ID(wParam, lParam))
			{
				case IDOK:
				{
					...
					//cRows
					GetEditBoxValue(hWndcRows, LONG_MIN, LONG_MAX, &cSavedRows, FALSE);
					...
					//Release previously fetched rows, (if user requested)
					TESTC(hr = pThis->ReleaseHeldRows());
	...
	switch (message)
	{
		...
		{	
			...
			if(pIDBProperties)
			{
				...
				GetProperty(IID_IDBProperties, pIDBProperties, DBPROP_PROVIDERFILENAME,		DBPROPSET_DATASOURCEINFO, DBTYPE_BSTR, &bstrString);
				wSendMessage(::GetDlgItem(hWnd, IDT_PROVIDERFILENAME), WM_SETTEXT, 0, bstrString ? bstrString : L"");
				SAFE_BSTRFREE(bstrString);

				//DBPROP_PROVIDERFRIENDLYNAME
				GetProperty(IID_IDBProperties, pIDBProperties, DBPROP_PROVIDERFRIENDLYNAME,	DBPROPSET_DATASOURCEINFO, DBTYPE_BSTR, &bstrString);
				wSendMessage(::GetDlgItem(hWnd, IDT_PROVIDERDESC), WM_SETTEXT, 0, bstrString ? bstrString : L"");
				SAFE_BSTRFREE(bstrString);
				
				//DBPROP_PROVIDERVER
				GetProperty(IID_IDBProperties, pIDBProperties, DBPROP_PROVIDERVER,	DBPROPSET_DATASOURCEINFO, DBTYPE_BSTR, &bstrString);
				//DBPROP_PROVIDEROLEDBVER
				GetProperty(IID_IDBProperties, pIDBProperties, DBPROP_PROVIDEROLEDBVER,	DBPROPSET_DATASOURCEINFO, DBTYPE_BSTR, &bstrString2);
				wSendMessageFmt(::GetDlgItem(hWnd, IDT_PROVIDERVERSION), WM_SETTEXT, 0, L"Prov - %s   OLEDB - %s", bstrString ? bstrString : L"Unknown", bstrString2 ? bstrString2 : L"Unknown");
				SAFE_BSTRFREE(bstrString);
				SAFE_BSTRFREE(bstrString2);

				//DBMS / DBMSVER
				GetProperty(IID_IDBProperties, pIDBProperties, DBPROP_DBMSNAME,	DBPROPSET_DATASOURCEINFO, DBTYPE_BSTR, &bstrString);
				GetProperty(IID_IDBProperties, pIDBProperties, DBPROP_DBMSVER,	DBPROPSET_DATASOURCEINFO, DBTYPE_BSTR, &bstrString2);
				wSendMessageFmt(::GetDlgItem(hWnd, IDT_DBMS), WM_SETTEXT, 0, L"%s %s", bstrString ? bstrString : L"", bstrString2 ? bstrString2 : L"");
				SAFE_BSTRFREE(bstrString);
				SAFE_BSTRFREE(bstrString2);
				
				//DATASOURCE
				GetProperty(IID_IDBProperties, pIDBProperties, DBPROP_DATASOURCENAME, DBPROPSET_DATASOURCEINFO, DBTYPE_BSTR, &bstrString);
				wSendMessage(::GetDlgItem(hWnd, IDT_DATASOURCE), WM_SETTEXT, 0, bstrString ? bstrString : L"");
				SAFE_BSTRFREE(bstrString);
				...
				GetProperty(IID_IDBProperties, pIDBProperties, DBPROP_CURRENTCATALOG,		DBPROPSET_DATASOURCE, DBTYPE_BSTR, &bstrString);
				wSendMessage(::GetDlgItem(hWnd, IDT_CATALOG), WM_SETTEXT, 0, bstrString);
				SAFE_BSTRFREE(bstrString);
...
BOOL CDataGrid::OnColumnClick(INT idCtrl, NMLISTVIEW* pNMListView)
{
	...
	m_pCMDIChild->m_iSelectedCol = pNMListView->iSubItem-1;
	CRowset* pCRowset = (CRowset*)m_pCMDIChild->GetObject(eCRowset);

	//See if the selected column is a chapter column...
	if(pCRowset)
	{
		if((DBCOUNTITEM)m_pCMDIChild->m_iSelectedCol < pCRowset->m_cBindings)
		{
			DBBINDING* pBinding = &pCRowset->m_rgBindings[m_pCMDIChild->m_iSelectedCol];
			DBCOLUMNINFO* pColInfo = pCRowset->GetColInfo(pBinding->iOrdinal);
			if(pColInfo->dwFlags & DBCOLUMNFLAGS_ISCHAPTER)
			{
				//Obtain the Child rowset..
				m_pCMDIChild->GetChapteredChild(m_pCMDIChild->m_iSelectedCol, pBinding);
				return TRUE;
			}
		}
	}
...
{
	CRow* pCRow			= SOURCE_GETOBJECT(m_pCMDIChild->m_pCDataAccess, CRow);
	CDataset* pCDataset	= SOURCE_GETOBJECT(m_pCMDIChild->m_pCDataAccess, CDataset);
	...
	if(pCRow)
	{
		...
		m_pCMDIChild->m_idSource = IDM_IROW_SETCOLUMNS;
		m_pCMDIChild->m_pCSource = pCRow;
		m_pCMDIChild->ChangeSelectedRow();
	}
	else if(pCDataset)
	{
		//IMDDataset::GetCellData
		m_pCMDIChild->m_idSource = IDM_DATASET_GETCELLDATA;
		m_pCMDIChild->m_pCSource = pCDataset;
		m_pCMDIChild->ChangeSelectedRow();
	}
	else
	{
		CRowset* pCRowset =  SOURCE_GETOBJECT(m_pCMDIChild->m_pCDataAccess, CRowset);
		if(pCRowset)
		{
			...
			//See if the selected column is a chapter column...
			if((DBORDINAL)iSelCol < pCRowset->m_cBindings)
			{
				DBBINDING* pBinding = &pCRowset->m_rgBindings[iSelCol];
				DBCOLUMNINFO* pColInfo = pCRowset->GetColInfo(pBinding->iOrdinal);
				if(pColInfo->dwFlags & DBCOLUMNFLAGS_ISCHAPTER)
				{
					//Obtain the Child rowset..
			...
			HROW hRow;
			if(m_pCMDIChild->m_pCDataGrid->GetSelectedRow(&hRow, FALSE) != LVM_ERR)
			{
				CWaitCursor waitCursor;
				IUnknown* pIUnknown = NULL;
				...
				else
				{
					...
					m_pCMDIChild->m_idSource = IDM_SETDATA;
					m_pCMDIChild->m_pCSource = pCRowset;
					m_pCMDIChild->ChangeSelectedRow();
				}
...
//
/////////////////////////////////////////////////////////////////
HRESULT CDataGrid::ScrollGrid(LONG cItems)
{
	return m_pCMDIChild->GetNextRows(0, cItems, TRUE/*fRetry*/);
}
