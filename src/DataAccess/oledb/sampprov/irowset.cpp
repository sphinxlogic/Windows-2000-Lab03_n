    ...
    )
{
    ...
    return ResultFromScode( cErrorCount ? DB_S_ERRORSOCCURRED : S_OK );
}
    ...
    )
{
	...
	rgPropId[0]							= DBPROP_CANHOLDROWS;
	rgPropId[1]							= DBPROP_LITERALIDENTITY;
	...
	// Get the Prompt value
	if( V_BOOL(&prgPropertySets->rgProperties[0].vValue) == VARIANT_TRUE )
		fCanHoldRows = TRUE;

	// Get the Prompt value
	if( V_BOOL(&prgPropertySets->rgProperties[1].vValue) == VARIANT_TRUE )
