...
// Create all of the child windows that are desired.
      ...
      /* Need to resize the StatusBar */
         if (hwndStatus) {
            ...
            lHeight = GetWindowLongPtr (hwndStatus, GWLP_USERDATA);
            MoveWindow (hwndStatus, -1, iHeight - (int) lHeight,
                  iWidth + 2, (int)(lHeight + 2), TRUE);
         }/*endIf*/
         break;
      }/*endCase*/
...
LRESULT APIENTRY StatusBarWndProc (HWND hwnd, UINT uiMsg, WPARAM wParam, LPARAM lParam) {
   ...
   switch (uiMsg) {
      ...
      case WM_PAINT: {
         ...
         lHeight = (LONG)GetWindowLongPtr (hwnd, GWLP_USERDATA);
         pts[0].x = 3;
...
// Retrieve the height of the status bar.
   lHeight = (LONG)GetWindowLongPtr (hwndStatus, GWLP_USERDATA);
...
// Subclass the list box window proc so we can do the synchronization
   DefListBoxWndProc = (WNDPROC) GetWindowLongPtr (hwndFileList, GWLP_WNDPROC);
   SetWindowLongPtr (hwndFileList, GWLP_WNDPROC, (LONG_PTR) ListBoxWndProc);
...
// Subclass the list.
   SetWindowLongPtr (hwndFileList2, GWLP_WNDPROC, (LONG_PTR) ListBoxWndProc);
