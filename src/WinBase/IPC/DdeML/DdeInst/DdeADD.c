...
// Allocate a block of memory for the command to send to the Program Manager.
   ...
   // Initialize the DDE conversation here.
      ...
      if (DdeInitialize (&(ULONG)lIdInst, (PFNCALLBACK) GroupDDECallback,
...
// Create a DDEML data handle for the command.
            hData = DdeCreateDataHandle (lIdInst, (PUCHAR)szCommand,
...
// Create a DDEML data handle for the command.
            hData = DdeCreateDataHandle (lIdInst, (PUCHAR)szCommand, strlen (szCommand),
...
// Establish a new conversation.
      if (DdeInitialize (&(ULONG)lIdInst2, (PFNCALLBACK) GroupDDECallback,
            (DWORD) APPCMD_CLIENTONLY, 0L)) {
...
// Work around for limitation for in ProgMan (no more than 50 items per group)
         ...
         // Create a DDEML data handle for the command.
               hData = DdeCreateDataHandle (lIdInst2, (PUCHAR)szText2,
...
// Create a DDEML Data handle for the command string.
         hData = DdeCreateDataHandle (lIdInst2, (PUCHAR)szExecuteString,
