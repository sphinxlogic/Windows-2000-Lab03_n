...
LRESULT APIENTRY ClockWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam);

void NEAR PASCAL ClockCreate(HWND hWnd);
BOOL NEAR PASCAL ClockInit(HANDLE hInstance);
void NEAR PASCAL ClockPaint(HWND hWnd,  HDC hDC, INT hint);
void NEAR PASCAL ClockSize( HWND hWnd,INT newWidth,INT newHeight, WORD SizeWord);
...
void NEAR PASCAL DrawBorder(HWND hWnd,  HDC hDC);
void NEAR PASCAL DrawFace(HDC hDC);
void NEAR PASCAL DrawHand( HDC hDC, INT pos, HPEN hPen, INT scale, INT patMode);
void NEAR PASCAL DrawFatHand( HDC hDC, INT pos, HPEN hPen, BOOL hHand);
void NEAR PASCAL FormatInit( HANDLE hInstance, BOOL fModeChange);
...
void NEAR PASCAL ClockSize( HWND hWnd,
               INT       newWidth,
               INT       newHeight,
               WORD      SizeWord)
{
...
void NEAR PASCAL DrawHand( HDC hDC,
              INT          pos,
              HPEN         hPen,
              INT          scale,
              INT          patMode)
...
void NEAR PASCAL SetMenuBar( HWND hWnd )
{
    ...
    if( ClockDisp.bNoTitle ) {
        ...
        wID = SetWindowLongPtr( hWnd, GWL_ID, 0 );
    }
    else {
        /* put menu bar & caption back in */
        dwStyle = WS_TILEDWINDOW | dwStyle;
        SetWindowLongPtr( hWnd, GWL_ID, wID );
    }
...
PAINTSTRUCT ps;
    ...
    switch (message) {
        ...
        case WM_QUERYDRAGICON:
            return (INT_PTR)LoadIcon(hInst, "cckk");
