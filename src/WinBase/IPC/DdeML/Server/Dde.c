    ...
    for (i = 0; i < CTOPICS; i++) {
        if (DdeCmpStringHandles(topicList[i].hszTopic, hszTopic) == 0) {
            ...
            if (wType == XTYP_CONNECT) {
                return((HDDEDATA)(INT_PTR)ValidateContext((PCONVCONTEXT)lData1));
            }

            pItemList = topicList[i].pItemList;
            cItems = (USHORT)topicList[i].cItems;
            for (j = 0; j < cItems; j++) {
...
WORD iFmt)
{
    ...
    cbAlloc = 0;
    for (i = 0; i < CTOPICS; i++)
        cbAlloc += (USHORT)(_tcslen(topicList[i].pszTopic) + 1) * sizeof(TCHAR);  // 1 for tab
...
WORD iFmt)
{
    ...
    for (iItem = 0; iItem < cItems; iItem++)
        cbAlloc += (USHORT)(_tcslen(pItemList[iItem].pszItem) + 1) * sizeof(TCHAR); // 1 for tab
...
WORD iFmt)
{
    ...
    case XTYP_POKE:
        ...
        return((HDDEDATA)(INT_PTR)fSuccess);
        break;
