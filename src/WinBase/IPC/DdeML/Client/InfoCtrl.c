...
LRESULT  APIENTRY InfoCtrlWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);
...
DWORD_PTR dwUser)
{
    ...
    if (hwnd = CreateWindow(szClass, szNULL,
        ...
        cCreated++;
        SetWindowLongPtr(hwnd, GWL_USER, dwUser);
 ...
 *                                                                          *
 ****************************************************************************/
LRESULT  APIENTRY InfoCtrlWndProc(
HWND hwnd,
UINT msg,
WPARAM wParam,
LPARAM lParam)
{
    ...
    case WM_CREATE:
         ...
         * word.
         */
        SetWindowLongPtr(hwnd, GWL_INFODATA,
                (DWORD_PTR)(((LPCREATESTRUCT)lParam)->lpCreateParams));
        break;
    ...
    case WM_SIZE:
        ...
        if ((short)LOWORD(lParam) < 2 * cxMargin || (short)HIWORD(lParam) < 2 * cyMargin) {
            ...
            picd = (INFOCTRL_DATA *)GetWindowLongPtr(hwnd, GWL_INFODATA);
    ...
    case WM_DESTROY:
         ...
         */
        {
            ...
            SendMessage(GetParent(hwnd), ICN_BYEBYE, (WPARAM)hwnd,
                    GetWindowLongPtr(hwnd, GWL_USER));
            SetWindowLongPtr(hwnd, GWL_USER, 0);
            ...
            if (picd) {
                ...
                SetWindowLongPtr(hwnd, GWL_INFODATA, 0);
            }
        }
        break;
            ...
            if (picd->style & ICSTY_OWNERDRAW) {
                ...
                if (IntersectRect(&odps.rcPaint, &rc, &ps.rcPaint)) {
                    if (IntersectClipRect(ps.hdc, rc.left, rc.top, rc.right,
                        ...
                        odps.rcBound = rc;
                        odps.hdc = ps.hdc;
                        odps.dwUser = GetWindowLongPtr(hwnd, GWL_USER);
                        SendMessage(GetParent(hwnd), ICN_OWNERDRAW,
                        GetWindowLongPtr(hwnd, GWL_ID),(DWORD_PTR)&odps);
                    }
                }
            } else {
