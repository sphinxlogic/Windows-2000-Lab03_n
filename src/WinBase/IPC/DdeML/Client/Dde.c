...
XACT *pxact)
{
    ...
    hwnd = CreateInfoCtrl((LPTSTR)pData,
            ...
            ICSTY_SHOWFOCUS, 0, (DWORD_PTR)pxact);
    MyFree(pszItem);
    MyFree(pszFmt);
    MyFree(pData);
    return(hwnd);
...
XACT *pxact)
{
    ...
    if (pxact->wType == XTYP_ADVSTOP) {
        hwndInfoCtrl = FindAdviseChild((HWND)(DWORD_PTR)ci.hUser, pxact->hszItem,
                pxact->wFmt);
        if (hwndInfoCtrl) {
            SendMessage(hwndInfoCtrl, ICM_SETSTRING, ICSID_UL,
                    (INT_PTR)Type2String(pxact->wType, pxact->fsOptions));
     ...
     */
    if (!hwndInfoCtrl) {
        hwndInfoCtrl = CreateXactionWindow((HWND)(DWORD_PTR)ci.hUser, pxact);
        if (!hwndInfoCtrl) {
            MyFree(pxact);
            return 0;
        }
     ...
     */
    pxact->ret =  (INT_PTR)DdeClientTransaction((LPBYTE)pxact->hDdeData, 0xFFFFFFFF,
    ...
    SendMessage(hwndInfoCtrl, ICM_SETSTRING, ICSID_UR, (DWORD_PTR)szT);
    ...
    SendMessage(hwndInfoCtrl, ICM_SETSTRING, ICSID_LR, (DWORD_PTR)szT);

    if ((pxact->fsOptions & XOPT_ASYNC) && pxact->ret) {
        /*
         * asynchronous successful start - link transaction to window.
         */
        DdeSetUserHandle(pxact->hConv, pxact->Result, hwndInfoCtrl);
        ...
        SendMessage(hwndInfoCtrl, ICM_SETSTRING, ICSID_LL,
                (DWORD_PTR)State2String(ci.wConvst));
    } else {
        /*
...
XACT *pxact)
{
    ...
    if (pxact->ret) {
        ...
        if (pxact->wType == XTYP_REQUEST) {
            ...
            if (!DdeCmpStringHandles(pxact->hszItem, hszHuge) &&
                ...
                SendMessage(hwndInfoCtrl, ICM_SETSTRING, ICSID_CENTER,
                        (DWORD_PTR)TEXT("Invalid Huge Data"));
            } else {
                SendMessage(hwndInfoCtrl, ICM_SETSTRING, ICSID_CENTER,
                        (DWORD_PTR)psz);
            }
         ...
         */
        SendMessage(hwndInfoCtrl, ICM_SETSTRING, ICSID_LL,
                (DWORD_PTR)Error2String(DdeGetLastError(idInst)));
    }
...
ULONG_PTR lData2)
{
    ...
    if (hConv) {
        ...
        ci.cb = sizeof(CONVINFO);
        if (!DdeQueryConvInfo(hConv, QID_SYNC, &ci) || (!IsWindow((HWND)(DWORD_PTR)ci.hUser))) {
            /*
        ...
        if (pmci = (MYCONVINFO *)GetWindowLongPtr((HWND)(DWORD_PTR)ci.hUser, 0)) {
            if (pmci->ci.wStatus != ci.wStatus ||
                 ...
                 */
                InvalidateRect((HWND)(DWORD_PTR)ci.hUser, NULL, TRUE);
            }
    ...
    case XTYP_DISCONNECT:
         ...
         */
        SendMessage((HWND)(DWORD_PTR)ci.hUser, UM_DISCONNECTED, 0, 0);
        return 0;
        break;
    ...
    case XTYP_ADVDATA:
         ...
         */
        Delay(wDelay);
        hwnd = FindAdviseChild((HWND)(DWORD_PTR)ci.hUser, hsz2, wFmt);
        if (!IsWindow(hwnd)) {
            ...
            hwnd = CreateInfoCtrl(NULL,
                    (INT)SendMessage((HWND)(DWORD_PTR)ci.hUser, UM_GETNEXTCHILDX, 0, 0L),
                    (INT)SendMessage((HWND)(DWORD_PTR)ci.hUser, UM_GETNEXTCHILDY, 0, 0L),
                    200, 100,
                    (HWND)(DWORD_PTR)ci.hUser, hInst,
        ...
        if (hData) {
            ...
            SendMessage(hwnd, ICM_SETSTRING, ICSID_CENTER, (DWORD_PTR)pData);
            MyFree(pData);
            DdeFreeDataHandle(hData);
        }
        SendMessage(hwnd, ICM_SETSTRING, ICSID_LL, (DWORD_PTR)TEXT("Advised"));
    ...
    case XTYP_XACT_COMPLETE:
        ...
        if (DdeQueryConvInfo(hConv, lData1, &ci) &&
                IsWindow((HWND)(DWORD_PTR)ci.hUser) &&
                (pxact = (XACT *)GetWindowLongPtr((HWND)(DWORD_PTR)ci.hUser, GWL_USER))) {
            ...
            pxact->Result = lData2;
            pxact->ret = (DWORD_PTR)hData;
            CompleteTransaction((HWND)(DWORD_PTR)ci.hUser, pxact);
        }
        break;
    }
}
...
DWORD wFmt)
{
    ...
    while (hwnd && IsChild(hwndMDI, hwnd)) {
        pxact = (XACT *)GetWindowLongPtr(hwnd, GWL_USER);
        if (pxact &&
                (pxact)->wFmt == wFmt &&
...
HCONVLIST hConvList)
{
    ...
    while (hwnd) {
        if (GetWindowLongPtr(hwnd, GWLP_WNDPROC) == (LONG_PTR)MDIChildWndProc) {
            pmci = (MYCONVINFO *)GetWindowLongPtr(hwnd, 0);
