...
{
	TCHAR rgchMsg[_MAX_PATH];
	EvalAssert( FLoadString(ids, rgchMsg, _MAX_PATH) );

	TCHAR rgchTitle[_MAX_PATH];
	EvalAssert( FLoadString(IDS_TITLE, rgchTitle, _MAX_PATH) );
...
LFailureReturn:
		...
		DWORD  dwRet   = GetFullPathName(szTempFolder, _MAX_PATH, szBuf, &szFName);
		if (!FEmptySz(szFName))
			goto LGotFolderPath;
		}

	UINT uiRet;
	uiRet = GetTempPath(_MAX_PATH, szBuf);
	Assert(uiRet > 0);
	Assert(uiRet < _MAX_PATH);
	...
	if (!GetTempFileName(szBuf, TEXT("SPH"), 0, rgchPath))
		goto LUseWinDir;
	Assert(lstrlen(rgchPath) < _MAX_PATH);
	...
	if (FFileExist(rgchPath))
		{
LUseWinDir:
		uiRet = GetWindowsDirectory(szBuf, _MAX_PATH);
		Assert(uiRet > 0);
		Assert(uiRet < _MAX_PATH);
		szLast = SzLastChar(szBuf);
		Assert(szLast != szNull);
...
BOOL FFixupPathEx ( LPTSTR szIn, LPTSTR szOut )
{
	...
	return (NULL != _tfullpath(szOut, szIn, _MAX_PATH));
}
